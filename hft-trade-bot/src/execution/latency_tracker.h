...
inline const char* latency_stage_str(LatencyStage s) {
    switch (s) {
    case LatencyStage::SIGNAL_TO_ORDER:
        return "SIGNAL_TO_ORDER";
    case LatencyStage::ORDER_TO_ACK:
        return "ORDER_TO_ACK";
    case LatencyStage::ACK_TO_FILL:
        return "ACK_TO_FILL";
    case LatencyStage::SIGNAL_TO_FILL:
        return "SIGNAL_TO_FILL";
    case LatencyStage::ORDER_TO_FILL:
        return "ORDER_TO_FILL";
    case LatencyStage::MARKET_DATA_PROCESS:
        return "MARKET_DATA_PROCESS";
    case LatencyStage::RISK_CHECK:
        return "RISK_CHECK";
    case LatencyStage::STRATEGY_COMPUTE:
        return "STRATEGY_COMPUTE";
    }
    return "UNKNOWN";
}
...
class LatencyTracker {
  public:
    ...
    void record(LatencyStage stage, int64_t latency_ns) noexcept {
        ...
        while (us < current_min &&
               !stats.min_us.compare_exchange_weak(current_min, us, std::memory_order_relaxed,
                                                   std::memory_order_relaxed)) {
        }

        int64_t current_max = stats.max_us.load(std::memory_order_relaxed);
        while (us > current_max &&
               !stats.max_us.compare_exchange_weak(current_max, us, std::memory_order_relaxed,
                                                   std::memory_order_relaxed)) {
        }

        // Update histogram
        auto&  h   = histograms_[idx];
        size_t bin = static_cast<size_t>(static_cast<double>(us) / MAX_LATENCY_US *
                                         static_cast<double>(histogram_bins_));
    ...
    void reset() noexcept {
        for (size_t i = 0; i < NUM_STAGES; ++i) {
            ...
            for (auto& c : histograms_[i].bin_counts)
                c.store(0, std::memory_order_relaxed);
        }
    }
  ...
  private:
    ...
    size_t percentile_from_histogram(const HistogramData& h, double pct,
        ...
        for (size_t i = 0; i < histogram_bins_; ++i) {
            ...
            if (cumulative >= target) {
                ...
                double bin_width =
                    static_cast<double>(MAX_LATENCY_US) / static_cast<double>(histogram_bins_);
...
class ScopedLatencyMeasurement {
  public:
    ScopedLatencyMeasurement(LatencyTracker& tracker, LatencyStage stage)
        : tracker_(tracker), stage_(stage), start_ns_(now_ns()) {}

    ~ScopedLatencyMeasurement() { tracker_.record(stage_, now_ns() - start_ns_); }
