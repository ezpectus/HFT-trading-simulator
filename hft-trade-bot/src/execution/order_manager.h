...
inline const char* order_state_str(OrderStateV2 s) {
    switch (s) {
    case OrderStateV2::PENDING:
        return "PENDING";
    case OrderStateV2::ACK:
        return "ACK";
    case OrderStateV2::PARTIAL:
        return "PARTIAL";
    case OrderStateV2::FILLED:
        return "FILLED";
    case OrderStateV2::CANCELED:
        return "CANCELED";
    case OrderStateV2::REJECTED:
        return "REJECTED";
    case OrderStateV2::EXPIRED:
        return "EXPIRED";
    case OrderStateV2::MODIFY_PENDING:
...
class OrderManager {
  public:
    ...
    uint64_t create_order(const std::string& symbol, const std::string& exchange, Side side,
                          OrderType type, double quantity, double price = 0.0,
                          int64_t timeout_ns = 0) noexcept {
        ...
        while (slot > prev_max) {
            if (max_slot_used_.compare_exchange_weak(prev_max, slot, std::memory_order_relaxed,
    ...
    // On partial fill
    void on_partial_fill(uint64_t client_order_id, double fill_qty, double fill_price,
                         double fee = 0.0) noexcept {
        const auto* it = cid_find(client_order_id);
        if (!it) return;
        OrderRecord& rec = orders_[it->slot];
    ...
    int active_count() const noexcept { return active_count_.load(std::memory_order_relaxed); }
    ...
    uint64_t find_free_slot() noexcept {
        for (size_t i = 0; i < MAX_ORDERS; ++i) {
            if (orders_[i].state == OrderStateV2::FILLED ||
                ...
                orders_[i].state == OrderStateV2::EXPIRED || orders_[i].client_order_id == 0) {
                return i;
            }
        }
        return MAX_ORDERS;
