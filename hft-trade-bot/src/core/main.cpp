...
int main(int argc, char* argv[]) {
    ...
    if (config.is_production) {
        spdlog::info("  IPC: {} | FIX: {} | DB: {} | Redis: {} | Metrics: {}", config.ipc_enabled,
                     config.fix_enabled, !config.db_dsn.empty(), config.redis_enabled,
    ...
    kill_switch.set_notify_callback([&](KillSwitch::Reason reason) {
        const char* reason_str[] = {"MANUAL", "DAILY_LOSS", "MAX_DRAWDOWN", "MARGIN_CALL",
                                    "FILE_TRIGGER"};
    ...
    if (config.ipc_enabled) {
        ...
        shm_fill_producer =
            std::make_unique<ipc::ShmFillProducer>(config.ipc_fills_shm, config.ipc_fills_capacity);
        ...
        shm_signal_consumer = std::make_unique<ipc::ShmSignalConsumer>(config.ipc_signals_shm,
                                                                       config.ipc_signals_capacity);
        try {
            shm_signal_consumer->start([&](const ipc::SignalMsg& msg) {
                Signal sig;
                sig.symbol = (msg.symbol_id < config.symbols.size()) ? config.symbols[msg.symbol_id]
                                                                     : "UNKNOWN";
                sig.direction   = (msg.action == 1)   ? "LONG"
                                  : (msg.action == 2) ? "SHORT"
                                                      : "NEUTRAL";
            ...
            spdlog::warn(
                "SHM IPC: signal consumer failed to start (Python may not be running yet): {}",
                e.what());
    ...
    if (config.ipc_enabled) {
        try {
            ...
            spdlog::warn(
                "SHM IPC: market data consumer failed to start (Python may not be running yet): {}",
                e.what());
    ...
    receiver.on_arbitrage([&](const std::string& symbol, const std::string& buy_exchange,
                              const std::string& sell_exchange, double buy_price, double sell_price,
                              double spread_bps, double max_quantity) {
        arb_lock.lock();
        latest_arb = {symbol,     buy_exchange, sell_exchange, buy_price,
                      sell_price, spread_bps,   max_quantity};
        arb_lock.unlock();
    ...
    if (config.thread_pinning_enabled) {
        if (ThreadAffinity::pin_to_core(config.execution_core_id)) {
            ...
            spdlog::warn("Could not pin thread to core {} — running on default scheduler",
                         config.execution_core_id);
        }
    }
    ...
    while (g_running) {
        ...
        if (has_arb_opportunity.load() && can_trade) {
            ...
            if (executor.is_connected() && arb.max_quantity > 0.001) {
                ...
                executor.execute_arbitrage(arb.symbol, arb.buy_exchange, arb.sell_exchange, qty,
        ...
        while (can_trade) {
            ...
            // Risk check
            auto risk_result =
                risk_mgr.check_signal(ai_sig, current_balance, pos_mgr.position_count());
            if (risk_result.passed && !pos_mgr.has_position(ai_sig.symbol)) {
                ...
                spdlog::debug("AI signal rejected by risk: {} ({})", ai_sig.symbol,
                              risk_result.reason);
            }
        }
        ...
        if (config.signal_engine_v2_enabled && can_trade) {
            for (const auto& [symbol, sym_cstr, sym_id] : symbol_entries) {
                ...
                FastSignal fast_sig;
                if (engine_v3) {
                    fast_sig = engine_v3->analyze_incremental(sym_cstr, candles.data(),
                                                              candles.size(), ob, pressure, now_ns);
                } else {
                    fast_sig = engine_v2.analyze_incremental(sym_cstr, candles.data(),
                                                             candles.size(), ob, pressure, now_ns);
                }
                ...
                auto          risk_result =
                    risk_mgr.check_signal(sig, current_balance, pos_mgr.position_count());
                ...
                if (config.adaptive_order_enabled) {
                    ...
                    auto selection = adaptive_selector.select(
                        ...
                        pressure.obi_weighted, pressure.toxic_score, qty, top5_depth, now_ns);
                ...
                if (executor.is_connected()) {
                    ...
                    if (order_kind == FastOrder::OrderKind::MARKET) {
                        ...
                        if (limit_price > 0) {
                            ...
                            if (fast_sig.is_long()) {
                                ob_modified.bids.insert(ob_modified.bids.begin(),
                                                        {limit_price, qty});
                            } else {
                                ob_modified.asks.insert(ob_modified.asks.begin(),
                                                        {limit_price, qty});
                            }
                        }
            ...
            for (const auto& symbol : config.symbols) {
                ...
                if (fast_sig.direction != "NEUTRAL" &&
                    ...
                    spdlog::info("HFT v1 Signal: {} {} conf={:.1f} entry={:.2f} ({})",
                                 sig.direction, sig.symbol, sig.confidence, sig.entry_price,
                                 sig.reason);
        ...
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_print).count() >= 10) {
            ...
            spdlog::info("Status: balance={:.2f} equity={:.2f} positions={} unrealized={:+.2f} "
                         ...
                         balance.load(std::memory_order_relaxed) + unrealized, positions.size(),
                         unrealized, receiver.is_trading_active() ? "ACTIVE" : "STOPPED",
                         kill_switch.is_active() ? "TRIGGERED" : "ARMED");

            if (config.latency_histogram_enabled) {
                spdlog::info("  Latency — signal: [{}] risk: [{}] exec: [{}] loop: [{}]",
                             signal_latency_hist.format_stats(), risk_check_hist.format_stats(),
                             order_exec_hist.format_stats(), total_loop_hist.format_stats());
            }
        ...
        if (shm_market_data && receiver.has_shm_data()) {
            ...
            for (uint8_t sid = 0; sid < max_sym; ++sid) {
                ...
                if (shm_market_data->read_snapshot(sid, snap)) {
                    receiver.inject_snapshot(sid, snap.bid, snap.ask, snap.last, snap.volume);
                }
            }
        }
    }
