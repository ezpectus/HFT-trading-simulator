...
Config Config::load(const std::string& path) {
    ...
    if (auto t = root["trading"]) {
        ...
        if (t["signal_interval_seconds"])
            cfg.signal_interval_seconds = t["signal_interval_seconds"].as<int>();
    ...
    if (auto ex = root["exchange"]) {
        ...
        auto parse_exchange = [](const YAML::Node& node, Config::ExchangeConfig& ec) {
            ...
            if (node["api_secret"])
                ec.api_secret = expand_env(node["api_secret"].as<std::string>());
            if (node["passphrase"])
                ec.passphrase = expand_env(node["passphrase"].as<std::string>());
            ...
            if (auto rl = node["rate_limit"]) {
                if (rl["weight_per_min"])
                    ec.rate_limit_weight_per_min = rl["weight_per_min"].as<int>();
                if (rl["orders_per_min"])
                    ec.rate_limit_orders_per_min = rl["orders_per_min"].as<int>();
            }
        };
    ...
    if (auto sr = root["smart_order_router"]) {
        if (sr["strategy"]) {
            ...
            if (strat == "best_price")
                cfg.router_strategy = 0;
            else if (strat == "lowest_latency")
                cfg.router_strategy = 1;
            else if (strat == "lowest_fees")
                cfg.router_strategy = 2;
            else if (strat == "best_effective")
                cfg.router_strategy = 3;
            else if (strat == "depth_aware")
                cfg.router_strategy = 4;
        }
        if (sr["toxic_threshold"]) cfg.router_toxic_threshold = sr["toxic_threshold"].as<int>();
    }
