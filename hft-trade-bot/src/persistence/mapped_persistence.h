...
static constexpr size_t   MAPPED_FILE_SIZE =
    sizeof(MappedHeader) + sizeof(MappedAccount) + MAX_POSITIONS * sizeof(MappedPosition);
...
class MappedPersistence {
  public:
    ...
    ~MappedPersistence() { unmap_all(); }

    bool save_state(const std::vector<MappedPosition>& positions, const MappedAccount& account) {
        std::lock_guard<std::mutex> lock(mutex_);

        // Create or open file
        std::string path = base_dir_ + "/trading_state.bin";
#ifdef _WIN32
        HANDLE fd = CreateFileA(path.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
        ...
        // Write account
        auto* acct =
            reinterpret_cast<MappedAccount*>(static_cast<char*>(mapped) + sizeof(MappedHeader));
        *acct              = account;
        ...
        spdlog::debug("[MappedPersist] Saved {} positions, balance={:.2f}", header->position_count,
                      account.balance);
        return true;
    }
...
#ifdef _WIN32
        HANDLE fd = CreateFileA(tmp_path.c_str(), GENERIC_READ | GENERIC_WRITE, 0, nullptr,
