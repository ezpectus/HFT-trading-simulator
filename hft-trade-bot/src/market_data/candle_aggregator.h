...
class CandleAggregator {
  public:
    ...
    CandleAggregator(const std::string& symbol, const std::string& exchange,
        ...
        : symbol_(symbol), exchange_(exchange), mode_(CandleMode::TIME),
          interval_ns_(interval_seconds * 1'000'000'000LL),
          threshold_(static_cast<double>(interval_seconds)), callback_(std::move(callback)) {}

    // Volume-based constructor: fixed volume per bar
    CandleAggregator(const std::string& symbol, const std::string& exchange, double volume_per_bar,
                     CandleCallback callback = {}, CandleMode mode = CandleMode::VOLUME)
        : symbol_(symbol), exchange_(exchange), mode_(mode), interval_ns_(0),
          threshold_(volume_per_bar), callback_(std::move(callback)) {}

    // Tick-based constructor: fixed tick count per bar
    CandleAggregator(const std::string& symbol, const std::string& exchange, uint64_t ticks_per_bar,
                     CandleCallback callback = {}, bool tick_mode = true)
        : symbol_(symbol), exchange_(exchange), mode_(CandleMode::TICK), interval_ns_(0),
          threshold_(static_cast<double>(ticks_per_bar)), callback_(std::move(callback)) {}
