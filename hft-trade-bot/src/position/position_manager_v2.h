...
class PositionManagerV2 {
  public:
    ...
    void on_fill(const std::string& symbol, const std::string& exchange, Side side, double qty,
                 double price, double fee = 0.0, int leverage = 1,
                 uint16_t symbol_id = 0xFFFF) noexcept {
        std::lock_guard<Spinlock> lk(lock_);
        // Build key without heap allocation using stack buffer
        char key_buf[128];
        int  key_len =
            std::snprintf(key_buf, sizeof(key_buf), "%s:%s", symbol.c_str(), exchange.c_str());
        if (key_len <= 0) return;
    ...
    std::vector<CloseTrigger> check_sl_tp(const std::unordered_map<std::string, double>& prices,
                                          ...
                                          double take_profit_mult = 3.0) const noexcept {
        std::lock_guard<Spinlock> lk(lock_);
