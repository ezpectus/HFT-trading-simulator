...
class FixSession {
  public:
    ...
    FixSession(const std::string& sender_comp_id, const std::string& target_comp_id,
               const std::string& seq_file_path, int heart_bt_int = 30)
        : sender_comp_id_(sender_comp_id), target_comp_id_(target_comp_id),
          seq_file_path_(seq_file_path), heart_bt_int_(heart_bt_int) {
        load_seq_nums();
    }
    ...
    bool logon(const std::string& username = "", const std::string& password = "",
        ...
        auto msg  = FixEncoder::build_logon(sender_comp_id_.c_str(), target_comp_id_.c_str(),
                                            outgoing_seq_++, heart_bt_int_, username.c_str(),
                                            password.c_str(), reset_seq);
    ...
    bool logout(const std::string& text = "") {
        ...
        auto msg  = FixEncoder::build_logout(sender_comp_id_.c_str(), target_comp_id_.c_str(),
    ...
    void on_message(const char* data, size_t len) {
        ...
        if (incoming_seq > incoming_seq_) {
            ...
            auto resend = FixEncoder::build_resend_request(
                sender_comp_id_.c_str(), target_comp_id_.c_str(), outgoing_seq_++,
                static_cast<uint32_t>(incoming_seq_), static_cast<uint32_t>(incoming_seq - 1));
        ...
        if (decoder.is_logon()) {
            ...
            auto hb = FixEncoder::build_heartbeat(sender_comp_id_.c_str(), target_comp_id_.c_str(),
    ...
    bool send_new_order(const std::string& cl_ord_id, const std::string& symbol, char side,
                        double qty, char ord_type, double price = 0.0, char tif = '0',
                        double stop_px = 0.0) {
        if (state_.load(std::memory_order_acquire) != SessionState::LOGGED_IN) return false;
        auto msg = FixEncoder::build_new_order_single(
            sender_comp_id_.c_str(), target_comp_id_.c_str(), outgoing_seq_++, cl_ord_id.c_str(),
            symbol.c_str(), side, qty, ord_type, price, tif, stop_px);
        bool sent = send_cb_ ? send_cb_(msg.data(), msg.size()) : false;
        save_seq_nums();
        return sent;
    }
    ...
    bool send_cancel(const std::string& orig_cl_ord_id, const std::string& cl_ord_id,
        ...
        if (state_.load(std::memory_order_acquire) != SessionState::LOGGED_IN) return false;
        auto msg  = FixEncoder::build_order_cancel(sender_comp_id_.c_str(), target_comp_id_.c_str(),
                                                   outgoing_seq_++, orig_cl_ord_id.c_str(),
                                                   cl_ord_id.c_str(), symbol.c_str(), side);
        bool sent = send_cb_ ? send_cb_(msg.data(), msg.size()) : false;
        save_seq_nums();
        return sent;
    }
    ...
    bool         is_logged_in() const {
        return state_.load(std::memory_order_acquire) == SessionState::LOGGED_IN;
    }
            ...
            while (heartbeat_running_) {
                {
                    ...
                    hb_wake_cv_.wait_for(lk, std::chrono::seconds(heart_bt_int_), [this]() {
                        return !heartbeat_running_.load(std::memory_order_relaxed);
                    });
                }
                if (!heartbeat_running_ ||
                    state_.load(std::memory_order_acquire) != SessionState::LOGGED_IN)
                    break;
