...
class BybitAdapter : public ExchangeBase {
  public:
    ...
    OrderResult place_order(const std::string& symbol, const std::string& side,
                            const std::string& order_type, double qty, double price = 0.0,
                            const std::string& time_in_force = "",
                            const std::string& order_link_id = "", int64_t recv_window = 5000);
    ...
    std::string subscribe_orderbook(const std::string& symbol) const {
        return R"({"op":"subscribe","args":["orderbook.50.)" + symbol + R"("]})";
    }

    std::string subscribe_ticker(const std::string& symbol) const {
        return R"({"op":"subscribe","args":["tickers.)" + symbol + R"("]})";
    }

    std::string subscribe_trades(const std::string& symbol) const {
        return R"({"op":"subscribe","args":["publicTrade.)" + symbol + R"("]})";
    }
    ...
    std::string auth_message(const std::string& api_key, const std::string& timestamp,
                             const std::string& signature) const {
        return R"({"op":"auth","args":[")" + api_key + R"(",")" + timestamp + R"(",")" + signature +
               R"("]})";
    }
