...
bool SignalEngineV2::Params::validate() const {
    ...
    if (ema_fast_period <= 0 || ema_slow_period <= 0 || ema_signal_period <= 0) {
        std::snprintf(validation_error_, sizeof(validation_error_),
                      "EMA periods must be positive (got %d/%d/%d)", ema_fast_period,
                      ema_slow_period, ema_signal_period);
        return false;
    }
    if (ema_fast_period >= ema_slow_period) {
        std::snprintf(validation_error_, sizeof(validation_error_),
                      "ema_fast (%d) must be < ema_slow (%d)", ema_fast_period, ema_slow_period);
        return false;
    }
    ...
    if (rsi_oversold >= rsi_overbought) {
        std::snprintf(validation_error_, sizeof(validation_error_),
                      "rsi_oversold (%.1f) must be < rsi_overbought (%.1f)", rsi_oversold,
                      rsi_overbought);
        return false;
    }
    ...
    if (obi_levels_5 <= 0 || obi_levels_10 <= 0 || obi_levels_20 <= 0) {
        std::snprintf(validation_error_, sizeof(validation_error_), "OBI levels must be positive");
        return false;
    }
    ...
    if (body_direction_lookback <= 0) {
        std::snprintf(validation_error_, sizeof(validation_error_),
                      "body_direction_lookback must be positive (%d)", body_direction_lookback);
        return false;
    }
