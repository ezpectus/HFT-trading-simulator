...
class SignalEngineV2 {
  public:
    ...
    FastSignal analyze_raw(const char* symbol, const double* closes, size_t n_closes,
        ...
        FastSignal sig;
        int64_t    now_ms;
        if (!init_and_validate(sig, symbol, timestamp_ns, n_candles, n_closes, now_ms)) return sig;
        ...
        sig.vwap_score =
            compute_vwap_score_raw(highs, lows, closes, volumes, n_candles, current_price);
  ...
  private:
    inline bool init_and_validate(FastSignal& sig, const char* symbol, int64_t timestamp_ns,
        ...
        size_t min_candles =
            static_cast<size_t>(std::max(params_.ema_slow_period + params_.ema_signal_period,
                                         std::max(params_.adx_period, params_.atr_period)) +
                                2);
    ...
    inline bool check_cooldown(IndicatorCache& ic, int64_t timestamp_ns,
    ...
    inline void update_indicator_cache(IndicatorCache& ic, const Candle* candles, size_t n,
        ...
        if (!ic.initialized ||
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                ic.vwap.update(candles[i].high, candles[i].low, candles[i].close,
                               candles[i].volume);
            }
    ...
    inline void compute_cached_scores(FastSignal& sig, const IndicatorCache& ic,
        ...
        double macd_scale =
            std::fabs(ic.ema_slow.value()) > 1e-12 ? std::fabs(ic.ema_slow.value()) : 1.0;
        ...
        sig.rsi_score    = std::fmax(
            -1.0, std::fmin(1.0, rsi_range > 1e-12 ? (rsi_mid - rsi_val) / rsi_range : 0.0));
        sig.obi_score     = compute_obi_score(ob);
        ...
        if (band_width > 1e-12) {
            ...
            sig.vwap_score =
                std::fmax(-1.0, std::fmin(1.0, params_.vwap_dev_threshold > 1e-12
    ...
    inline double compute_obi_score(const OrderBook& ob) const noexcept {
        auto obi_res =
            compute_obi_all(ob, params_.obi_levels_5, params_.obi_levels_10, params_.obi_levels_20);
        double obi_combined =
            obi_res.obi_5 * 0.5 + obi_res.obi_10 * 0.3 + obi_res.obi_weighted * 0.2;
        return std::fmax(-1.0, std::fmin(1.0, std::fabs(obi_combined) > params_.obi_threshold
    ...
    inline double compute_vwap_score_raw(const double* highs, const double* lows,
                                         const double* closes, const double* volumes, size_t n,
    ...
    inline double compute_adx_raw(const double* highs, const double* lows, const double* closes,
                                  size_t n, double& adx_val) const noexcept {
        adx_val       = 0.0;
        ...
        for (size_t i = 1; i < n; ++i) {
            ...
            double tr  = std::fmax(highs[i] - lows[i], std::fmax(std::fabs(highs[i] - prev_c),
                                                                 std::fabs(lows[i] - prev_c)));
            if (count < adx_p) {
                tr_sum += tr;
                plus_dm_sum += pdm;
                minus_dm_sum += mdm;
    ...
    inline double compute_pressure_raw(const double* closes, const double* volumes, size_t n,
                                       const PressureResult& pressure) const noexcept {
        double buy_p = 0.0, sell_p = 0.0;
        int    lookback = std::min(params_.body_direction_lookback, static_cast<int>(n) - 1);
        for (int i = static_cast<int>(n) - lookback; i < static_cast<int>(n); ++i) {
            if (i < 0) continue;
        ...
        double raw_pressure =
            ...
            live_w;
        raw_pressure *= (1.0 - pressure.toxic_score * params_.toxic_penalty);
        return std::fmax(-1.0, std::fmin(1.0, std::fabs(raw_pressure) > params_.pressure_threshold
                                                  ? (raw_pressure > 0 ? 1.0 : -1.0)
                                                  : (params_.pressure_threshold > 1e-12
                                                         ? raw_pressure / params_.pressure_threshold
                                                         : 0.0)));
    ...
    inline double compute_atr_raw(const double* highs, const double* lows, const double* closes,
    ...
    inline void apply_adaptive_sl_tp(double atr, double price, double& sl_mult,
                                     double& tp_mult) const noexcept {
        if (!params_.adaptive_sl_tp || price <= 0.0) return;
        double atr_pct = atr / price;
        if (atr_pct < params_.low_vol_atr_pct) {
            sl_mult = params_.low_vol_sl_mult;
            tp_mult = params_.low_vol_tp_mult;
        } else if (atr_pct > params_.high_vol_atr_pct) {
            sl_mult = params_.high_vol_sl_mult;
            tp_mult = params_.high_vol_tp_mult;
        }
    }

    inline void finalize_signal(FastSignal& sig, double price, double atr, double sl_mult,
