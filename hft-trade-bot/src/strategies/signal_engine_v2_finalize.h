...
inline void set_long_signal(FastSignal& sig, double price, double atr, double sl_mult,
    ...
    std::snprintf(buf, sizeof(buf), "L comp=%+.2f E=%+.2f R=%+.2f O=%+.2f V=%+.2f A=%.0f P=%+.2f",
                  sig.composite_score, sig.ema_score, sig.rsi_score, sig.obi_score, sig.vwap_score,
                  adx_val, sig.pressure_score);
    sig.set_reason(buf);
}

inline void set_short_signal(FastSignal& sig, double price, double atr, double sl_mult,
                             double tp_mult, double adx_val,
                             const SignalEngineV2Params& p) noexcept {
