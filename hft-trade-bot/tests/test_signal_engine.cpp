...
// Tests: FFT, spectral_trend_score, fft_lowpass, EMA, RSI, OBI, VWAP, pressure, SignalEngine::analyze
//
...
#define ASSERT_TRUE(cond) \
    do { if(!(cond)) throw std::runtime_error(#cond " is false"); } while(0)

#define ASSERT_FALSE(cond) \
    do { if((cond)) throw std::runtime_error(#cond " is true"); } while(0)
...
TEST(test_fft_power_of_2) {
    ...
    for (size_t i = 0; i < 8; ++i) data[i] = static_cast<double>(i);
    fft(data);
...
TEST(test_fft_lowpass_smooths) {
    ...
    double mean = 0;
    for (auto v : closes) mean += v;
    mean /= closes.size();
    double orig_var = 0;
    for (auto v : closes) orig_var += (v - mean) * (v - mean);
    orig_var /= closes.size();
    double smooth_var = 0;
    for (auto v : smoothed) smooth_var += (v - mean) * (v - mean);
...
TEST(test_rsi_uptrend) {
    std::vector<double> closes;
    for (int i = 0; i < 30; ++i) closes.push_back(100 + i);
...
TEST(test_rsi_downtrend) {
    std::vector<double> closes;
    for (int i = 0; i < 30; ++i) closes.push_back(200 - i);
    auto rsi = compute_rsi(closes, 14);
