// Tests: Ring buffer push/pop, overflow, bulk operations
#include "../src/ipc/shm_protocol.h"
#include "../src/ipc/shm_ring_buffer.h"
#define DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN
#include "doctest.h"
#include <chrono>
#include <cstdio>
#include <cstring>
#include <thread>
#include <vector>

using namespace hft;
using namespace hft::ipc;

// Use a simple struct for testing
struct TestMsg {
    uint64_t id;
    double   value;
};

TEST_CASE("test_basic_push_pop") {
    // Create SHM ring buffer
    hft::ShmRingBuffer<TestMsg> rb("/hft_test_rb", 16, true);

    TestMsg msg{1, 42.0};
    REQUIRE(rb.try_push(msg));

    TestMsg out;
    REQUIRE(rb.try_pop(out));
    REQUIRE(out.id == 1);
    REQUIRE(out.value == 42.0);

    // Buffer should be empty now
    REQUIRE(!rb.try_pop(out));
    REQUIRE(rb.size() == 0);

    rb.unlink();
}

TEST_CASE("test_fill_buffer") {
    hft::ShmRingBuffer<TestMsg> rb("/hft_test_rb2", 8, true);

    // Fill to capacity
    for (uint64_t i = 0; i < 8; ++i) {
        REQUIRE(rb.try_push({i, static_cast<double>(i)}));
    }

    // Should be full
    REQUIRE(rb.size() == 8);
    REQUIRE(!rb.try_push({99, 99.0})); // Should fail

    // Pop all
    for (uint64_t i = 0; i < 8; ++i) {
        TestMsg out;
        REQUIRE(rb.try_pop(out));
        REQUIRE(out.id == i); // FIFO order
    }

    REQUIRE(rb.size() == 0);

    rb.unlink();
}

TEST_CASE("test_bulk_push_pop") {
    hft::ShmRingBuffer<TestMsg> rb("/hft_test_rb3", 64, true);

    std::vector<TestMsg> msgs;
    for (uint64_t i = 0; i < 32; ++i) {
        msgs.push_back({i, static_cast<double>(i * 10)});
    }

    uint64_t pushed = rb.bulk_push(msgs.data(), msgs.size());
    REQUIRE(pushed == 32);
    REQUIRE(rb.size() == 32);

    TestMsg  out[32];
    uint64_t popped = rb.bulk_pop(out, 32);
    REQUIRE(popped == 32);
    REQUIRE(rb.size() == 0);

    for (uint64_t i = 0; i < 32; ++i) {
        REQUIRE(out[i].id == i);
        REQUIRE(out[i].value == static_cast<double>(i * 10));
    }

    rb.unlink();
}

TEST_CASE("test_wraparound") {
    hft::ShmRingBuffer<TestMsg> rb("/hft_test_rb4", 4, true);

    // Push and pop multiple times to test wraparound
    for (int cycle = 0; cycle < 10; ++cycle) {
        for (uint64_t i = 0; i < 4; ++i) {
            REQUIRE(rb.try_push({static_cast<uint64_t>(cycle * 4 + i), 0.0}));
        }
        for (uint64_t i = 0; i < 4; ++i) {
            TestMsg out;
            REQUIRE(rb.try_pop(out));
            REQUIRE(out.id == static_cast<uint64_t>(cycle * 4 + i));
        }
    }

    rb.unlink();
}

TEST_CASE("test_signal_struct") {
    hft::ShmRingBuffer<SignalMsg> rb("/hft_test_sig", 16, true);

    SignalMsg sig{};
    sig.timestamp  = 123456789;
    sig.symbol_id  = 0; // BTC
    sig.action     = 1; // LONG
    sig.confidence = 0.85f;
    sig.price      = 50000.0f;
    sig.sl         = 49000.0f;
    sig.tp         = 52000.0f;
    sig.leverage   = 10;

    REQUIRE(rb.try_push(sig));

    SignalMsg out;
    REQUIRE(rb.try_pop(out));
    REQUIRE(out.timestamp == 123456789);
    REQUIRE(out.symbol_id == 0);
    REQUIRE(out.action == 1);
    REQUIRE(std::abs(out.confidence - 0.85f) < 1e-6);
    REQUIRE(std::abs(out.price - 50000.0f) < 1e-6);

    rb.unlink();
}

TEST_CASE("test_fill_struct") {
    hft::ShmRingBuffer<FillMsg> rb("/hft_test_fill", 16, true);

    FillMsg fill{};
    fill.timestamp   = 987654321;
    fill.symbol_id   = 1; // ETH
    fill.side        = 0; // BUY
    fill.qty         = 2.5f;
    fill.price       = 3000.0f;
    fill.fee         = 0.5f;
    fill.exchange_id = 1; // OKX

    REQUIRE(rb.try_push(fill));

    FillMsg out;
    REQUIRE(rb.try_pop(out));
    REQUIRE(out.timestamp == 987654321);
    REQUIRE(out.symbol_id == 1);
    REQUIRE(out.side == 0);
    REQUIRE(std::abs(out.qty - 2.5f) < 1e-6);

    rb.unlink();
}

TEST_CASE("test_header_magic") {
    hft::ShmRingBuffer<TestMsg> rb("/hft_test_magic", 8, true);

    // Verify magic number is set
    // The header should have been initialized with SHM_MAGIC
    // We can't directly access it, but the fact that it works means it's valid

    TestMsg msg{1, 1.0};
    REQUIRE(rb.try_push(msg));
    TestMsg out;
    REQUIRE(rb.try_pop(out));

    rb.unlink();
}
