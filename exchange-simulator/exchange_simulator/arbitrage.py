...
class ArbitrageDetector:
    ...
    def scan(self) -> list[ArbitrageOpportunity]:
        ...
        # Get all symbols from first exchange
        symbols = set()
        ...
        for symbol in symbols:
            ...
            # Find best arbitrage: lowest ask vs highest bid
            exchange_prices = []
            ...
            # Sort by best ask (ascending) for buy side
            sorted_by_ask = sorted(exchange_prices, key=lambda x: x["best_ask"])
            # Sort by best bid (descending) for sell side
            sorted_by_bid = sorted(exchange_prices, key=lambda x: x["best_bid"], reverse=True)

            # Try all pairs
            for buyer in sorted_by_ask:
                for seller in sorted_by_bid:
                    if buyer["exchange"] == seller["exchange"]:
                        continue
                    ...
                    buy_slippage = buy_price * self.slippage_bps / 10000
                    sell_slippage = sell_price * self.slippage_bps / 10000
