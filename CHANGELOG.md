...
- **`fill_rate` exceeded 100% (S394):** `ORDERS_SENT` was incremented on only 3 of 6 wire paths — v1 fallback submits, SL/TP `close_position`, and kill-switch `close_all` never counted, while `ORDERS_FILLED` counted every fill including those closes. All submission paths now increment. Deeper fix from live verification: the simulator's auto-arb fills on the shared account were broadcast and counted as HFT's — all HFT order paths now carry `client_order_id` (`hft_*` prefixes), the sim echoes it in fill frames, and FILLED/CANCELLED/REJECTED only count own-order ids while `apply_fill` still reconciles every real fill.
...
- **Null `client_order_id` threw and dropped foreign fills (S397):** sim auto-arb orders emit `client_order_id: null`; `o.value(..., "")` threw `json::type_error`, spamming `errors_total` (~930/3min live) and dropping those fills before position-book reconciliation. Null-safe extraction; regression test covers a `null` cid in a batch.
- **Code hygiene:** finding IDs (`S###`) removed from all source comments — they live in ledgers/changelog only (150 files swept).
...
### Fixed
- **Vendored `doctest.h` stub left a registered ctest target uncompilable; first real run surfaced 3 defects (S372):** the 156-line hand-rolled stub lacked `CHECK_THROWS*`/`WARN`/`INFO` and had broken `SUBCASE`/`REQUIRE` semantics — `test_doctest_order_type_selector` used `CHECK_THROWS_AS` and never compiled anywhere. The stub gained the THROWS/NOTHROW family; a new `HFT_TESTS_ONLY` CMake mode (deps QUIET, main exe skipped, dep-requiring tests gated on `TARGET`) lets the dep-free suite build on hosts without vcpkg. Executing the suite caught: `InlineRSI` returning ~100 on perfectly flat prices (now 50); `OnlineHMM` annualizing its per-observation volatility proxy (×252, ~16x mis-scale — VOLATILE always won, so trend/range detection was dead) plus an over-wide RANGING return variance (5e-5→1e-7); and a `test_doctest_pressure_model` fixture that claimed 1bps but built 2bps while expecting TIGHT. **ctest: 13/13 binaries, 221 cases — all green** (first local run ever). The stub was subsequently replaced by the genuine vendored doctest 2.4.11 header — which exposed that four more test targets had no `main` under any framework (now `DOCTEST_CONFIG_IMPLEMENT_WITH_MAIN` via CMake) and four `a && b` assertions that real doctest rejects (split into paired bounds).

## [Unreleased] — 2026-09-15 (Slop-fix R240–R247 — panel coverage sweep)

### Fixed
- **Panel lifecycle gate extended to the full externally-reachable cycle (no product defects — clean round R247):** the mount sweep now also re-renders every panel on a single-candle boundary context and through an unmount→remount cycle (localStorage cleared, simulating a new user), covering cleanup paths and single-element-series math. 271/271 clean.
- **Hooks-order violation found by extending the panel sweep to selection-switch and empty-data re-renders (S371):** `BayesianPricePredictor` ran a `useMemo` below its no-data early return — switching to a candle-less selection (or the pre-snapshot startup state) dropped a hook and crashed the panel subtree with "Rendered fewer hooks than expected". The memo now runs unconditionally; `eslint-plugin-react-hooks` `rules-of-hooks` was wired in as an error gate, closing the whole conditional-hook class statically.
- **Two numerical-stability defects found by extending the panel sweep to all control types plus a second-tick re-render (S370):** `BurgersEquation`'s solver implemented the documented Lax-Friedrichs scheme as unstable FTCS advection with an explicit Laplacian in the same update — a combination that amplifies grid-scale (Nyquist) noise, so the histogram initial condition blew the field up within one step and looped on an astronomical substep count; fixed via operator splitting (conservative LF advection + separate diffusion substep, each under its own CFL bound). `RecurrentNeuralNetwork`'s learning-rate input had no upper clamp — `lr=1` diverged training into `NaN` output; now clamped with a non-finite-loss early stop.
...
### Fixed
- **Render-phase state writes + broken Kalman update (S366):** two estimator panels ran their grid-search optimizers inside `useMemo` while writing the optimized sigmas back to state that was in the memo's own dep array — every input change ran the full search twice. Optimizers now run in `useEffect`. The formula-parser's error state was also written mid-render; it now travels inside the memo result (fixing a latent crash on the empty-values path). And the BSTS Kalman filter used a non-Kalman element-wise covariance update that lost positive-definiteness and produced NaN forecasts — corrected to `P − K(Z·P)`.
...
### Fixed
- **Portfolio API rejects non-finite inputs (S362):** `1e999` is valid JSON — it parsed to `inf`, flowed through the optimizer, and serialized as bare `NaN`/`Infinity` — an unparseable response frame the client silently dropped. All float inputs to `optimize_portfolio`/`vol_surface` now require finite values.
...
- **Sub-cent assets in the sim (S333):** prices were hard-rounded to 2 decimals everywhere, collapsing anything under ~$0.005 to a zero price. `round_price()` now keeps 8 decimals below $1 — memecoin-scale symbols produce real fills and PnL.
- **HFT bot connection race (S336):** the WebSocket connection handle was published without synchronization — a formal data race on weak-memory targets and a torn-read hazard on reconnect. Both WS clients now guard it under the existing client mutex.
...
- **ai-signal-bot cleanup (S322/S328):** the three exchange WS feed runners shared a copy-pasted ~60-line loop — now one shared `_run_feed` with per-venue URL/subscribe builders; the dead parallel `backtest_engine`/`pnl_calculator` stack (~580 lines) was deleted and `compare_backtests` now uses the canonical `BacktestResult` type instead of a diverging twin.
...
- **Testing guide corrected (S354):** `docs/TESTING.md` had drifted from the real test tree — wrong file counts in five places, four test files named that don't exist, and coverage claims for checks the tests never perform (e.g., alert-metric names are not cross-validated). Counts and coverage now match the tree (305 unit files + 5 e2e).
- **Risk-management examples made runnable (S355):** `docs/RISK_MANAGEMENT.md` documented a `RiskAnalyzer`/`var_stress_test.py` module that doesn't exist and used four wrong method names — every code sample failed on copy-paste. Examples now use the real `src/risk/` API; the phantom section was removed.
...
- **Closed-trade stats no longer stuck at zero (S342):** the bot now ingests the simulator's authoritative `trade_history` (per-exchange cursor) — closed trades write `CLOSED` rows, feed the performance tracker and trade CSV, so `trades_closed`/`win_rate`/`total_pnl` on the dashboard and the `ai_signal_bot_win_rate`/`pnl_total` Prometheus gauges show real numbers instead of permanent zeros.
- **Sentiment and market-making strategies actually respond (S343):** simulator `news_event` broadcasts are routed to the sentiment strategy (intensity→magnitude, direction→sign, deduplicated across the event window) — it was `enabled: true` yet could only ever return NEUTRAL. Market-making inventory now tracks real account position deltas via `on_fill` (opens, closes, flips). `update_toxicity` stays unwired — no order-flow source exists.
- **Live exchange feed no longer runs unread + real last price (S345):** `RealMarketDataFeed` sockets open lazily on the first `get_ticker`/`get_orderbook`/`get_candles` call instead of at adapter connect — order-only usage pays no sockets. `@aggTrade` is parsed and merged with `bookTicker`, so `last` is a real trade price rather than a permanent 0.0.
- **Walk-forward validation is genuinely out-of-sample (S346):** `--optimize` now grid-searches parameters on a 60% fit segment and validates on the untouched 40% tail — previously "out-of-sample" results were in-sample (params picked on the same data). `walk_forward` treats `train_size` as strictly-past context (warmup) and `test_size` as the evaluated segment.
- **Circuit-breaker metrics actually populate (S344):** `ai_signal_bot_circuit_breaker_trips_total` increments on real trips (new `on_trip` hook), and the `circuit_breaker_state` gauge updates every status tick even with zero WebSocket clients — it previously went stale exactly when the breaker tripped.
- **Simulator broadcast metrics now reflect real traffic (S347):** every production broadcast (market data, fill batches, audit events) feeds `messages_total`/`bytes_sent`/`broadcast_latency_p95_ms`/`message_size_*` — previously they only counted connect-time sends and the tick loop was invisible. `delta_update_ratio` now tracks real full-vs-delta orderbook decisions, and `clients_connected` reads the live client set at scrape. The `compression_ratio` gauge was removed — permessage-deflate is handled inside the websockets transport, so the app could never observe real compressed sizes.
- **Speed/config control acks are delivered reliably (S348):** `replay_state`/`speed_set`/`config_updated` responses are now awaited sends with negotiated encoding — previously they were unreferenced `create_task` sends that could be GC'd mid-flight, and msgpack clients received unparseable JSON text.
- **Correct close reasons in fill broadcasts (S349):** each triggered close now carries its own reason on the order — a tick closing multiple positions no longer labels them all with the last trade's reason, so AlertWebhook's liquidation/sl_tp/fill classification and the trade CSV statuses are accurate.
- **HFT SHM integration test actually compiles (S350):** `test_integration_shm.cpp` was written against an API that never existed and broke every POSIX build — rewritten to test real producer→consumer SHM paths plus the wire-size contract shared with the Python side.
- **SHM protocol enums corrected and wired (S351):** `ExchangeId` values now match the documented wire mapping (Simulator=3 — the enum previously claimed the opposite) and are used at the producer/decoder sites; the unused `SymbolId`/`AlignedOrderBookLevel`/`RoutingDecision` declarations were removed.
- **`/health` signal-engine bit is real liveness (S352):** `signal_engine_active` was structurally always-true — it now reflects actual engine evaluation within the last 60s, so a stalled engine loop reports unhealthy instead of hiding.
...
### Changed
- `exchange_simulator/Dockerfile` + `Dockerfile.prod`: Healthcheck upgraded from TCP socket to HTTP `urllib.request.urlopen('http://localhost:8765/health')`
- `ai-signal-bot/Dockerfile` + `Dockerfile.prod`: Healthcheck upgraded from TCP socket to HTTP `urllib.request.urlopen('http://localhost:8766/health')`
- `docker-compose.yml`: Exchange-simulator + ai-signal-bot healthchecks upgraded from TCP to HTTP /health
- `docker-compose.prod.yml`: Same TCP→HTTP healthcheck upgrade for exchange-simulator + ai-signal-bot
- `helm/templates/exchange-simulator.yaml`: Liveness + readiness probes upgraded from `tcpSocket` to `httpGet /health`
- `helm/templates/ai-signal-bot.yaml`: Liveness + readiness probes upgraded from `tcpSocket` to `httpGet /health`
- `terraform/environments/dev/main.tf`: Removed `default = "ChangeMeInProduction123!"` from `db_password` variable — now required via `-var` or tfvars
- `hft-trade-bot/config/config.yaml`: Added comment documenting localhost as dev default — prod config uses `${VAR}` env var syntax

### Fixed
- CODE_AUDIT §8.9 — Docker healthchecks TCP → HTTP /health endpoint
- CODE_AUDIT §8.14 — Helm probes отсутствуют → all templates have httpGet liveness+readiness
- CODE_AUDIT §8.44 — Dockerfile healthcheck TCP → HTTP urllib.request
- CODE_AUDIT §8.96 — hft-trade-bot config hardcoded localhost → documented as dev default
- CODE_AUDIT §8.162 — Terraform hardcoded RDS password → default removed, required via -var
- CODE_AUDIT §8.401 — terraform db_password default in plaintext → same as §8.162
- CODE_AUDIT §8.423 — hft-trade-bot no Dockerfile.prod → [N/A] already exists (stale audit item)
...
### Changed
- `src/communication/health_check.py`: Added `DeprecationWarning` emission — had deprecation docstring but no actual warning, zero imports outside tests
- CODE_AUDIT §1.7 marked [FIXED] — `communication/health_check.py` deprecated, `monitoring/health_server.py` and `observability/health_checks.py` both used in production
- CODE_AUDIT §1.6 marked [N/A] — MetricsCollector and MetricsExporter serve different purposes (WS client metrics vs Prometheus exporter)
