...
**Audit notes** (see `docs/AUDIT_FINDINGS.md`): the signal-bot's daily-drawdown gate runs on daily *equity* change (realized + unrealized) rather than realized PnL only, and halt signals now gate both order paths (R200/R201). Backtests are causal (signal on bar i−1, fill at bar i close) — still treat them as directional, not predictive.
