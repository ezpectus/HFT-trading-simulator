...
export default function BybitOrderBook({ orderbookData, currentPrice }) {
  const { theme } = useExchange()
  
  if (!orderbookData || !orderbookData.bids || !orderbookData.asks) {
    return (
      <BybitCard className="p-3 text-center text-xs" style={{ color: theme.textSecondary }}>
        Loading...
      </BybitCard>
    )
  }
  ...
  return (
    <BybitCard className="flex flex-col h-full">
      {/* Header - compact */}
      <div 
        className="px-2 py-1 text-xs font-medium border-b flex justify-between items-center"
        style={{ 
          color: theme.text,
      ...
      <div 
        className="px-2 py-0.5 text-xs flex justify-between border-b"
        style={{ 
      ...
      <div className="flex-1 overflow-y-auto">
        {[...asks].reverse().map((ask, i) => {
          ...
          return (
            <div 
              key={`ask-${i}`}
              className="px-2 py-0.5 text-xs flex justify-between items-center relative"
        ...
        {/* Current price */}
        <div 
          className="px-2 py-1 text-xs text-center border-y"
          style={{ 
            color: theme.text,
            borderColor: theme.border,
      ...
      >
        <div className="flex justify-between">
          <span>B: {bids[0]?.price?.toFixed(2) || '0.00'}</span>
          <span>A: {asks[0]?.price?.toFixed(2) || '0.00'}</span>
        </div>
      </div>
    </BybitCard>
  )
