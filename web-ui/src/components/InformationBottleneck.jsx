    ...
    <div className="p-4 space-y-3">
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs">
        <label className="flex items-center gap-1">
          ...
          <input type="number" value={nClusters} onChange={e => setNClusters(Math.max(2, Math.min(8, +e.target.value)))} className="w-12 px-1 bg-slate-800 border border-slate-600  text-slate-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-slate-400">β (trade-off):</span>
          <input type="number" step="0.5" value={beta} onChange={e => setBeta(Math.max(0.1, +e.target.value))} className="w-12 px-1 bg-slate-800 border border-slate-600  text-slate-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-slate-400">Bins:</span>
          <input type="number" value={nBins} onChange={e => setNBins(Math.max(3, Math.min(20, +e.target.value)))} className="w-12 px-1 bg-slate-800 border border-slate-600  text-slate-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-slate-400">Lag:</span>
          <input type="number" value={lag} onChange={e => setLag(Math.max(1, +e.target.value))} className="w-12 px-1 bg-slate-800 border border-slate-600  text-slate-200" />
        </label>
        <label className="flex items-center gap-1">
