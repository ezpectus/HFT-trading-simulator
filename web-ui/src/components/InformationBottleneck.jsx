...
export default function InformationBottleneck({ candles, symbol, exchange }) {
  ...
  if (!data) {
    return <div className="p-4 text-sm text-gray-400">Need at least {lookback + lag + 1} candles for {symbol} on {exchange}</div>
  }

  const W = 800, H = 250, P = 30
  const sigColor = data.signal === 'BUY' ? '#0ecb81' : data.signal === 'SELL' ? '#f6465d' : '#94a3b8'
  const clusterColors = ['#0ecb81', '#f6465d', '#f0b90b', '#a855f7', '#06b6d4', '#ec4899', '#14b8a6', '#f97316']
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm font-bold text-gray-200">Information Bottleneck — {symbol}</span>
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs">
        <label className="flex items-center gap-1">
          ...
          <input type="number" value={nClusters} onChange={e => setNClusters(Math.max(2, Math.min(8, +e.target.value)))} className="w-12 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">β (trade-off):</span>
          <input type="number" step="0.5" value={beta} onChange={e => setBeta(Math.max(0.1, +e.target.value))} className="w-12 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">Bins:</span>
          <input type="number" value={nBins} onChange={e => setNBins(Math.max(3, Math.min(20, +e.target.value)))} className="w-12 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">Lag:</span>
          <input type="number" value={lag} onChange={e => setLag(Math.max(1, +e.target.value))} className="w-12 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
      ...
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Rate-Distortion Curve: I(X;T) vs I(T;Y) (varying β)</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          ...
          {data.rdCurve.map((d, i) => (
            <g key={i}>
              <circle cx={sxRD(d.I_XT)} cy={syRD(d.I_TY)} r={Math.abs(d.beta - beta) < 0.01 ? 6 : 3} fill={Math.abs(d.beta - beta) < 0.01 ? '#f0b90b' : '#06b6d4'} />
              <text x={sxRD(d.I_XT) + 5} y={syRD(d.I_TY) - 5} fill="#848e9c" fontSize={7}>β={d.beta}</text>
            </g>
          ))}

          <text x={W - P} y={H - 5} textAnchor="end" fill="#5e6673" fontSize={9}>I(X;T) — Rate (complexity)</text>
          <text x={5} y={P + 10} fill="#5e6673" fontSize={9}>I(T;Y) — Distortion (relevance)</text>
        </svg>
      </div>

      {/* Convergence */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Blahut-Arimoto Convergence (β={beta})</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />

          <path d={data.history.map((h, i) => `${i === 0 ? 'M' : 'L'} ${sxIter(i)} ${syInfo(h.I_XT)}`).join(' ')} fill="none" stroke="#06b6d4" strokeWidth={2} />
          <path d={data.history.map((h, i) => `${i === 0 ? 'M' : 'L'} ${sxIter(i)} ${syInfo(h.I_TY)}`).join(' ')} fill="none" stroke="#f0b90b" strokeWidth={2} />

          <text x={W - P} y={20} textAnchor="end" fill="#06b6d4" fontSize={9}>I(X;T) = {data.I_XT.toFixed(4)} bits</text>
          <text x={W - P} y={34} textAnchor="end" fill="#f0b90b" fontSize={9}>I(T;Y) = {data.I_TY.toFixed(4)} bits</text>
        </svg>
      </div>

      {/* Cluster assignments */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-2">Cluster Assignments (last 60 bars)</div>
      ...
      {/* Cluster details */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-2">Cluster Statistics</div>
        <div className="space-y-1">
          {data.clusters.map((c, t) => (
            <div key={t} className="flex items-center gap-3 text-xs">
              <span className="w-3 h-3 " style={{ background: clusterColors[t] }} />
              <span className="text-gray-400 w-16">Cluster {t}</span>
