...
export default memo(function SessionReplay({ accounts, fills, signals, candles, prices, orderbooks, symbol, exchange }) {
  ...
  return (
    <div className="bg-bg-700  p-2.5">
      ...
      {importMsg && (
        <div className={`mb-2 px-2 py-1  text-[9px] ${importMsg.ok ? 'bg-accent-green/20 text-accent-green' : 'bg-accent-red/20 text-accent-red'}`}>
          {importMsg.text}
        </div>
      )}
      ...
      {!isRecording ? (
        <div className="space-y-1.5 mb-2">
          ...
          <button
            onClick={handleStart}
            className="w-full flex items-center justify-center gap-1 py-1 text-[10px]  bg-accent-red/20 text-accent-red hover:bg-accent-red/30"
          >
            <StopCircle size={10} />
            Start Recording
          </button>
        ...
        <div className="mb-2 space-y-1">
          ...
          <button
            onClick={handleStop}
            className="w-full flex items-center justify-center gap-1 py-1 text-[10px]  bg-accent-green/20 text-accent-green hover:bg-accent-green/30"
          >
            <StopCircle size={10} />
            Stop & Save
          </button>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-1 py-0.5 text-[9px]  bg-bg-600 text-gray-400 hover:bg-bg-500 mb-2"
      >
        <Upload size={10} />
        Import Recording
      </button>
        ...
        <div className="border-t border-bg-600 pt-2">
          ...
          <div className="space-y-0.5 max-h-[120px] overflow-y-auto scrollbar-thin">
            {savedRecordings.map(rec => (
              <div key={rec.id} className="flex items-center gap-1.5 px-1.5 py-1  bg-bg-600/50 group">
                <button
      ...
      {viewing && currentSnapshot && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setViewing(null)}>
          <div className="bg-bg-800  p-4 max-w-4xl w-full max-h-[90vh] overflow-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-bg-700  p-3 mb-3">
              <div className="flex items-center gap-2 mb-2">
                ...
                <select value={playSpeed} onChange={e => setPlaySpeed(Number(e.target.value))} className="bg-bg-600 text-[9px] text-gray-300  px-1 py-0.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {Object.entries(currentSnapshot.accounts).map(([exId, acc]) => (
                <div key={exId} className="bg-bg-700  p-2">
                  ...
                  {acc.positions && (acc.positions?.length ?? 0) > 0 && (
            ...
            {currentSnapshot.fills.length > 0 && (
              <div className="mt-2 bg-bg-700  p-2">
