...
export default function ItoCalculusGenerator({ candles, symbol, exchange }) {
  ...
  const sigColor = data.signal === 'GENERATOR_POSITIVE' ? '#0ecb81' : data.signal === 'GENERATOR_NEGATIVE' ? '#f6465d' : '#94a3b8'
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm font-bold text-gray-200">{'It\u00f4 Calculus Generator \u2014'} {symbol}</span>
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs">
        ...
        <label className="flex items-center gap-1">
          <span className="text-gray-400">Test function:</span>
          <select value={funcType} onChange={e => setFuncType(e.target.value)} className="bg-bg-700 border border-bg-500  text-gray-200 px-1">
      ...
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">{'Infinitesimal Generator A\u00b7f(x) = \u03bc(x)f\u2032(x) + (1/2)\u03c3\u00b2(x)f\u2033(x) \u2014'} {data.tf.name}</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          ...
          {/* A.f */}
          <path d={data.Af.map((v, i) => `${i === 0 ? 'M' : 'L'} ${sxAf(i)} ${syAf(v)}`).join(' ')} fill="none" stroke="#f0b90b" strokeWidth={2.5} />

          {/* Zero line for A.f */}
          <line x1={P} y1={H / 2} x2={W - P} y2={H / 2} stroke="#5e6673" strokeWidth={1} strokeDasharray="3,3" />

          <text x={W - P} y={20} textAnchor="end" fill="#06b6d4" fontSize={9}>f(x) (scaled)</text>
          <text x={W - P} y={34} textAnchor="end" fill="#f0b90b" fontSize={9}>{'A\u00b7f(x) (generator)'}</text>
        </svg>
      </div>

      {/* Expected hitting time */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">{'Expected Hitting Time E[\u03c4] to Mean (solving A\u00b7T = -1)'}</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />
      ...
      {/* Stationary distribution */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">{'Stationary Distribution \u03c0(x) (invariant measure of generator)'}</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />

          <path d={data.stationary.map((v, i) => `${i === 0 ? 'M' : 'L'} ${sxAf(i)} ${syStat(v)}`).join(' ')} fill="rgba(34,197,94,0.15)" stroke="#0ecb81" strokeWidth={2} />

          <text x={W - P} y={20} textAnchor="end" fill="#0ecb81" fontSize={9}>{'\u03c0(x) stationary (A\u00b7\u03c0 = 0)'}</text>
        </svg>
      </div>

      <div className="grid grid-cols-5 gap-2 text-xs">
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">{'A\u00b7f(current)'}</div>
        ...
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">{'\u03b8 (target)'}</div>
        ...
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">{'E[\u03c4] to mean'}</div>
          <div className="text-gray-300 font-mono">{data.currentHittingTime.toFixed(2)}</div>
        </div>
      </div>
