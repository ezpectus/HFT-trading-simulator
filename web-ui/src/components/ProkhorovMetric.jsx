...
export default function ProkhorovMetric({ candles, symbol, exchange }) {
  ...
  const sigColor = data.signal === 'DISTRIBUTION_SHIFT' ? '#f6465d' : data.signal === 'DISTRIBUTION_DRIFT' ? '#f0b90b' : '#0ecb81'
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm font-bold text-gray-200">Prokhorov Metric (Weak Convergence) — {symbol}</span>
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs">
        ...
        <label className="flex items-center gap-1">
          ...
          <input type="number" value={lookback} onChange={e => setLookback(Math.max(80, +e.target.value))} className="w-16 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
      </div>

      {/* CDF comparison with Prokhorov tube */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Empirical CDFs: Baseline vs Current with Prokhorov tube (eps={data.eps.toFixed(6)})</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          ...
          {/* Prokhorov tube */}
          <path d={data.tubeUpper.map((t, i) => `${i === 0 ? 'M' : 'L'} ${sxCDF(t.x)} ${syCDF(t.cdf)}`).join(' ')} fill="none" stroke="#f0b90b" strokeWidth={1} strokeDasharray="3,2" opacity={0.5} />
          <path d={data.tubeLower.map((t, i) => `${i === 0 ? 'M' : 'L'} ${sxCDF(t.x)} ${syCDF(t.cdf)}`).join(' ')} fill="none" stroke="#f0b90b" strokeWidth={1} strokeDasharray="3,2" opacity={0.5} />

          {/* Baseline CDF */}
          <path d={data.cdfBaseline.map((c, i) => `${i === 0 ? 'M' : 'L'} ${sxCDF(c.x)} ${syCDF(c.cdf)}`).join(' ')} fill="none" stroke="#06b6d4" strokeWidth={2} />

          {/* Current CDF */}
          <path d={data.cdfCurrent.map((c, i) => `${i === 0 ? 'M' : 'L'} ${sxCDF(c.x)} ${syCDF(c.cdf)}`).join(' ')} fill="none" stroke="#f6465d" strokeWidth={2} />

          <text x={W - P} y={20} textAnchor="end" fill="#06b6d4" fontSize={9}>baseline CDF</text>
          <text x={W - P} y={34} textAnchor="end" fill="#f6465d" fontSize={9}>current CDF</text>
          <text x={W - P} y={48} textAnchor="end" fill="#f0b90b" fontSize={9}>Prokhorov tube (+/-eps)</text>
        </svg>
      </div>

      {/* Distance metrics over time */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Distribution Distance Metrics Over Time: Prokhorov, Wasserstein-1, KS</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />
          ...
          <path d={data.comparisons.map((c, i) => `${i === 0 ? 'M' : 'L'} ${sxT(i)} ${syKS(c.ks)}`).join(' ')} fill="none" stroke="#0ecb81" strokeWidth={1.5} opacity={0.7} />
          ...
          <text x={W - P} y={48} textAnchor="end" fill="#0ecb81" fontSize={9}>Kolmogorov-Smirnov</text>
        </svg>
      </div>

      <div className="grid grid-cols-4 gap-2 text-xs">
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">Prokhorov d_P</div>
          <div className="text-purple-400 font-mono">{data.currentDP.toFixed(6)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">Wasserstein W_1</div>
          <div className="text-cyan-400 font-mono">{data.currentW1.toFixed(6)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">KS statistic</div>
          <div className="text-emerald-400 font-mono">{data.currentKS.toFixed(4)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">Trend</div>
          <div className="font-mono" style={{ color: data.isIncreasing ? '#f6465d' : '#0ecb81' }}>{data.isIncreasing ? 'increasing' : 'stable'}</div>
        </div>
      </div>
