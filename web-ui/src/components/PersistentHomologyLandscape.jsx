...
export default function PersistentHomologyLandscape({ candles, symbol, exchange }) {
  ...
  const sigColor = data.signal === 'TOPOLOGICAL_CHANGE' ? '#f6465d' : '#0ecb81'
  const layerColors = ['#06b6d4', '#f0b90b', '#a855f7', '#0ecb81', '#f6465d']
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm font-bold text-gray-200">Persistent Homology Landscape — {symbol}</span>
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs">
        <label className="flex items-center gap-1">
          <span className="text-gray-400">Embed dim:</span>
          <input type="number" value={embedDim} onChange={e => setEmbedDim(Math.max(2, Math.min(6, +e.target.value)))} className="w-12 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
      ...
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Persistence Landscape λ_k(t) (vectorized topology)</div>
      ...
      {/* Persistence diagram */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Persistence Diagram (birth vs death)</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />

          {/* Diagonal */}
          <line x1={sxPD(0)} y1={syPD(0)} x2={sxPD(maxPD)} y2={syPD(maxPD)} stroke="#5e6673" strokeWidth={1} strokeDasharray="3,3" />
          ...
          <text x={W - P} y={34} textAnchor="end" fill="#5e6673" fontSize={9}>diagonal = noise</text>
        </svg>
      </div>

      {/* Sliding window L2 norm */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Sliding Window: Landscape L2 Norm (topological complexity)</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />

          {/* Mean + 2σ threshold */}
          <line x1={P} y1={syNorm(data.normMean + 2 * data.normStd)} x2={W - P} y2={syNorm(data.normMean + 2 * data.normStd)} stroke="#f6465d" strokeWidth={1.5} strokeDasharray="4,3" />
          <line x1={P} y1={syNorm(data.normMean)} x2={W - P} y2={syNorm(data.normMean)} stroke="#5e6673" strokeWidth={1} strokeDasharray="2,2" />

          {data.slidingNorms.map((n, i) => (
            <line key={i} x1={sxNorm(i)} y1={H - P} x2={sxNorm(i)} y2={syNorm(n)} stroke={n > data.normMean + 2 * data.normStd ? '#f6465d' : '#a855f7'} strokeWidth={2} opacity={0.7} />
          ))}

          <text x={W - P} y={20} textAnchor="end" fill="#a855f7" fontSize={9}>L2 norm (windowed)</text>
          <text x={W - P} y={34} textAnchor="end" fill="#f6465d" fontSize={9}>μ+2σ threshold</text>
        </svg>
      </div>

      {/* Top features */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-2">Top Persistent Features (H₀)</div>
          ...
          {data.topFeatures.slice(0, 5).map((f, i) => (
            <div key={i} className="flex items-center gap-3 text-xs">
              ...
              <div className="flex-1 bg-bg-900  h-3 relative">
                <div className="h-full " style={{ width: `${Math.min(100, f.persistence / data.topFeatures[0].persistence * 100)}%`, background: layerColors[i] }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid grid-cols-5 gap-2 text-xs">
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">L2 norm</div>
