...
export default function AlmgrenChriss({ candles, symbol, exchange, currentPrice }) {
  ...
  if (!result) {
    return <div className="p-4 text-sm text-gray-400">Invalid parameters for Almgren-Chriss model</div>
  }
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm font-bold text-gray-200">Almgren-Chriss Optimal Execution — {symbol}</span>
      </div>

      <div className="flex items-center gap-3 flex-wrap text-xs">
        <label className="flex items-center gap-1">
          <span className="text-gray-400">Order Size:</span>
          <input type="number" value={orderSize} onChange={e => setOrderSize(Math.max(1, +e.target.value))} className="w-20 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">T (days):</span>
          <input type="number" step="0.1" value={timeHorizon} onChange={e => setTimeHorizon(Math.max(0.1, +e.target.value))} className="w-16 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">Steps:</span>
          <input type="number" value={nSteps} onChange={e => setNSteps(Math.max(2, +e.target.value))} className="w-16 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">λ (risk aversion):</span>
          <input type="number" step="0.0000001" value={riskAversion} onChange={e => setRiskAversion(Math.max(1e-10, +e.target.value))} className="w-24 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">η (temp impact):</span>
          <input type="number" step="0.01" value={eta} onChange={e => setEta(Math.max(0.001, +e.target.value))} className="w-16 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
        <label className="flex items-center gap-1">
          <span className="text-gray-400">γ (perm impact):</span>
          <input type="number" step="0.001" value={gamma} onChange={e => setGamma(Math.max(0, +e.target.value))} className="w-16 px-1 bg-bg-700 border border-bg-500  text-gray-200" />
        </label>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 bg-bg-700  p-3">
          <div className="text-xs text-gray-400 mb-1">Optimal Execution Trajectory</div>
          <svg width={W} height={H} className="bg-bg-900 ">
            ...
            <path d={twapPath} fill="none" stroke="#848e9c" strokeWidth={1.5} strokeDasharray="4,3" />
            <text x={W - P} y={H - 5} textAnchor="end" fill="#5e6673" fontSize={10}>Time (days)</text>
            <text x={5} y={P + 10} fill="#5e6673" fontSize={10}>Shares</text>
            <text x={W - P} y={20} textAnchor="end" fill="#06b6d4" fontSize={10}>Almgren-Chriss</text>
            <text x={W - P} y={34} textAnchor="end" fill="#848e9c" fontSize={10}>TWAP (dashed)</text>
          </svg>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        {/* Efficient frontier */}
        <div className="flex-1 bg-bg-700  p-3">
          <div className="text-xs text-gray-400 mb-1">Efficient Frontier (Cost vs Risk)</div>
          <svg width={fW} height={fH} className="bg-bg-900 ">
            <line x1={P} y1={fH - P} x2={fW - P} y2={fH - P} stroke="#1e2530" />
            <line x1={P} y1={P} x2={P} y2={fH - P} stroke="#1e2530" />
            <path d={frontierPath} fill="none" stroke="#f0b90b" strokeWidth={2} />
            {result.frontier[optimalIdx] && (
              <circle cx={fxScale(result.frontier[optimalIdx].stdDev)} cy={fyScale(result.frontier[optimalIdx].cost)} r={4} fill="#0ecb81" />
            )}
            <circle cx={fxScale(result.twapStdDev)} cy={fyScale(result.twapCost)} r={4} fill="#848e9c" />
            <text x={fW - P} y={fH - 5} textAnchor="end" fill="#5e6673" fontSize={9}>Risk (σ)</text>
            <text x={5} y={P + 8} fill="#5e6673" fontSize={9}>Cost</text>
          </svg>
          <div className="text-xs text-gray-500 mt-1">● Optimal (green) vs TWAP (gray)</div>
        </div>

        {/* Trade schedule */}
        <div className="flex-1 bg-bg-700  p-3">
          <div className="text-xs text-gray-400 mb-1">Execution Schedule (first 5 + last)</div>
          <div className="space-y-1 text-xs font-mono max-h-48 overflow-auto">
            {result.trades.slice(0, 5).map((t, i) => (
              <div key={i} className="flex justify-between text-gray-300">
                <span className="text-gray-500">t={t.t.toFixed(3)}</span>
      ...
      <div className="grid grid-cols-5 gap-2 text-xs">
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">E[cost] (AC)</div>
          <div className="text-cyan-400 font-mono">{result.expectedCost.toFixed(4)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">σ[cost] (AC)</div>
          <div className="text-amber-400 font-mono">{result.stdDev.toFixed(4)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">E[cost] (TWAP)</div>
          <div className="text-gray-300 font-mono">{result.twapCost.toFixed(4)}</div>
        </div>
        <div className="bg-bg-700  p-2">
