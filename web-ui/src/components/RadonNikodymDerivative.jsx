...
export default function RadonNikodymDerivative({ candles, symbol, exchange }) {
  ...
  const sigColor = data.signal === 'REGIME_CHANGE_STRONG' ? '#f6465d' : data.signal === 'REGIME_CHANGE' ? '#f0b90b' : '#0ecb81'
  ...
  return (
    <div className="p-4 space-y-3">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-sm font-bold text-gray-200">Radon-Nikodym Derivative (Regime Detection) — {symbol}</span>
      ...
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Radon-Nikodym Derivative dQ/dP(x): ratio of current to baseline density</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          ...
          <text x={W - P} y={34} textAnchor="end" fill="#5e6673" fontSize={9}>RN=1 (equal measures)</text>
        </svg>
      </div>

      {/* Log-RN over time */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Log Radon-Nikodym Derivative Over Time (measure change trajectory)</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H / 2} x2={W - P} y2={H / 2} stroke="#1e2530" />
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />
          ...
          {data.comparisons.map((c, i) => c.significant ? (
            <circle key={i} cx={sxT(i)} cy={syT(c.logRN)} r={4} fill="#f6465d" />
          ) : null)}

          <text x={W - P} y={20} textAnchor="end" fill="#06b6d4" fontSize={9}>log(dQ/dP) cumulative</text>
          <text x={W - P} y={34} textAnchor="end" fill="#f6465d" fontSize={9}>significant (p&lt;0.05)</text>
        </svg>
      </div>

      {/* KL divergence over time */}
      <div className="bg-bg-700  p-3">
        <div className="text-xs text-gray-400 mb-1">Symmetric KL Divergence (Jensen-Shannon-like) Over Time</div>
        <svg width={W} height={H} className="bg-bg-900 ">
          <line x1={P} y1={H - P} x2={W - P} y2={H - P} stroke="#1e2530" />
          <line x1={P} y1={P} x2={P} y2={H - P} stroke="#1e2530" />

          <path d={data.comparisons.map((c, i) => `${i === 0 ? 'M' : 'L'} ${sxT(i)} ${syKL(c.klSym)}`).join(' ')} fill="none" stroke="#0ecb81" strokeWidth={2} />

          <text x={W - P} y={20} textAnchor="end" fill="#0ecb81" fontSize={9}>KL_sym = (KL(P||Q) + KL(Q||P)) / 2</text>
        </svg>
      </div>

      <div className="grid grid-cols-5 gap-2 text-xs">
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">log(dQ/dP)</div>
          <div className="text-cyan-400 font-mono">{data.current.logRN.toFixed(4)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">LR stat</div>
        ...
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">KL(P||Q)</div>
          <div className="text-emerald-400 font-mono">{data.klPQ.toFixed(6)}</div>
        </div>
        <div className="bg-bg-700  p-2">
          <div className="text-gray-400">KL(Q||P)</div>
