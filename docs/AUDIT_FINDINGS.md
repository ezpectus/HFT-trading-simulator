...
**S281 (Low) — Open.** `ws_prometheus.py:129-130` counts orders with `o.status.value == "filled"`/`"rejected"` — lowercase — but `OrderStatus` stores `"FILLED"`/`"REJECTED"` (models.py:41-42; `ws_broadcast.py:276` compares `"FILLED"` correctly). `exchange_orders_filled_total` and `exchange_orders_rejected_total` are eternal zeros on `/metrics`. The comment admits it was ported from never-started `health.py` — ported with a case bug.
