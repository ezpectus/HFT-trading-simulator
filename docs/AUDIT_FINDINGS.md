...
**S293 (Info) — Open.** `SentimentConfig.follow_threshold` (sentiment.py:78, read at :198/:202 for the follow LONG/SHORT branches) is never wired: no `sentiment_follow_threshold` on `SignalBotConfig`, no yaml key, and `bot_helpers.py:54-56` passes only `fade_threshold`+`decay_rate`. The knob sits next to wired siblings in the same dataclass looking tunable — it's hardcoded 0.3 forever.
