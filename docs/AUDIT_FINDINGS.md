...
- **S117 (Medium) — Fixed in R40.** `src/portfolio/` (4 modules, ~630 lines) + `src/pricing/volatility_surface.py` (214 lines, SVI/SABR) have **zero live importers** — consumed only by their own ~41 tests. Same unwired-feature class as `database/` (S109); decide together.
...
**S272 (Medium) — Fixed in R159.** 17 of ~25 `MetricsExporter` update methods (`ai-signal-bot/src/monitoring/metrics.py`) are never called in prod — the gauges/counters/histograms they back render as eternal zeros across three dashboards. Dead setters: `record_signal` (:235), `record_fill` (:241), `record_order_sent`/`record_order_rejected` (:246/:253), `update_pnl` (:264), `update_positions` (:272), `update_ws_status` (:278), `update_shm_buffer` (:283), `observe_signal_latency`/`observe_order_latency`/`observe_shm_round_trip`/`observe_position_hold_time` (:288–303), `record_error` (:340), `set_bot_drawdown`/`set_bot_win_rate`/`set_bot_pnl_total`/`set_bot_uptime` (:345–360). Blast radius: `trading-performance.json` is a zero-board — `trading_fills_total`, `trading_signals_total`, `trading_total_equity`, `ai_signal_bot_drawdown`, `ai_signal_bot_win_rate` all flat (only `sharpe_ratio` lives, via run.py:641); `ai_signal_bot_metrics.json` shows `errors_total`/`uptime_seconds` permanently 0; `latency-monitoring.json`'s `trading_signal_latency_seconds` histogram is empty. Subtlety: the publisher increments `ai_signal_bot_signals_sent_total` (live) while the dashboard queries `trading_signals_total` (dead `record_signal`) — two signal counters, one flat. The hft half of `trading-overview.json` is the S246 blast radius.

**S273 (Low) — Fixed R167.** Both `terraform/environments/{dev,prod}/terraform.tfvars.example` set `db_password` — but the env roots declare **zero** `variable` blocks and no database resource exists (modules are eks/s3/vpc only). `terraform plan` fails with "Value for undeclared variable"; the dev example even ships a password-shaped literal `ChangeMeInProduction123!`.
...
**S277 (Low) — Fixed R167.** Strategy Marketplace is a storage island: `useStrategyMarketplace.ts` + `StrategyMarketplace.jsx` store/import/export `StrategyPackage` objects under `trading-sim-strategy-marketplace`, but nothing bridges them to execution — every onClick in the component is upload/download/delete/filter (zero run/load/apply actions), and `StrategyBacktest.jsx:8,41` loads a *different* localStorage key (`trading-sim-strategies`, written by `StrategyBuilder.jsx:24`). The `rules` format (`volume_spike`, `price_change_5`, …) matches `backtestEngine`'s condition vocabulary one-for-one — the data model is executable, the plumbing just ends. A "marketplace" whose goods can only be collected as JSON cards.

**S278 (Low) — Fixed R167.** `useSessionRecorder.ts:134` — `stopRecording` computes `totalTrades` by summing `acc.trade_history.length` across **every** snapshot, but trade history is cumulative (each snapshot carries the full list). Result: `metadata.totalTrades` is inflated by ~N_snapshots× — a 5-trade session with 100 snapshots reports "500 trades". Displayed verbatim in `SessionReplay.jsx:152` ("…N trades | X% DD") and `:175`.
...
## Round 126 — exchange_simulator ws-layer + engine extras (3 findings: S279–S281)

Scope: the never-leaf-read remainder of `exchange_simulator/` — `ws_message_handler.py` (609: inbound commands, auth, dedup, rate limits), `ws_broadcast.py` (540: the wire producer), `websocket_server.py` (268), `ws_metrics/ws_prometheus/ws_constants` (305), `models.py` (488), `arbitrage.py`, `options_simulator.py`, `exchange_liquidation.py`, `audit_logger.py`, `data_export.py`, `config_validator.py`, `visualizer*` — ~4.5k lines.

**S279 (High) — Fixed in R147.** `trade_csv_logger.py` is **gitignored and untracked** (`git check-ignore` confirms — it lives only in this working copy; the Docker build context `./exchange_simulator` can't see it). `websocket_server.py:37` guards the import with `except ImportError → TradeCsvLogger = None` — so in every clean clone, CI, and container `self.trade_logger` is None. But `ws_broadcast.py:284` calls `self.trade_logger.log_fill(...)` and `:389` `self.trade_logger.log_batch(...)` **without a None-guard** (the same call in `ws_message_handler.py:337` IS guarded). Since `_open_new_position` assigns SL/TP to every position, the first engine-side close (SL/TP, liquidation, partial-liquidation, arb auto-exec) raises `AttributeError` inside `_broadcast_loop` — which has zero error containment — the broadcast task dies, the market feed stops permanently, and `/health` keeps answering `healthy` because `_running` stays True. Classic works-on-my-machine: it can't reproduce locally because the file happens to exist here.

**S280 (Medium) — Fixed in R162.** `_broadcast_loop` (`ws_broadcast.py:208-235`) has no try/except anywhere — any exception in `next_candle()`/`_process_exchange_events`/arbitrage/serialization kills the task silently. Concrete trigger: `_handle_update_config` (`ws_message_handler.py:534-538`) writes `updates["volatility"][symbol]` straight into `market._volatility` with no type/range check — a string value → `sigma = "abc" / sqrt_cpy` → TypeError in the tick path → feed dead. In dev the control plane is unauthenticated (no `EXCHANGE_CONTROL_TOKEN`), so any connected client can send it; in prod it's an authenticated DoS. `fee`/`slippage`/`leverage` poisoning is caught by the message-level handler (TypeError in its except list) — `volatility` executes in the tick path where nothing catches.

**S281 (Low) — Fixed in R160.** `ws_prometheus.py:129-130` counts orders with `o.status.value == "filled"`/`"rejected"` — lowercase — but `OrderStatus` stores `"FILLED"`/`"REJECTED"` (models.py:41-42; `ws_broadcast.py:276` compares `"FILLED"` correctly). `exchange_orders_filled_total` and `exchange_orders_rejected_total` are eternal zeros on `/metrics`. The comment admits it was ported from never-started `health.py` — ported with a case bug.

**Retracted:** dedup-resubmit sends `{"type":"fill"}` for any stored status (even REJECTED/PENDING) — the client (`useExchangeData.js:51` `trackOrderStatus` + `resolveAck` passing the real `order.status`) handles it correctly; unknown `set_speed` values default to 1.0 — harmless; GTD-cancelled orders in `filled_orders` are filtered by the caller.
...
## Round 132 — run.py + config loader + src/communication internals (4 findings: S290–S293)

Scope: `run.py` (749-line composition root — init, main loop, SHM channel, alerting wiring, paper/live exec, equity snapshot, deprecated backtest-mode), `config/__init__.py` (452 lines, 70-property loader + validate), and all 12 `src/communication/` files (2,669 lines): signal_publisher, ws_client, circuit_breaker, metrics_server, shm_ring_buffer, shm_signal_producer, shm_fill_consumer, shm_kill_switch_consumer, shm_market_data_writer, backtest_requests, portfolio_requests, analysis_requests.

**S291 (High) — Fixed in R151.** One malformed WS message kills the market-data listener permanently — no restart anywhere. `ws_client.py:206` does `candle["symbol"]`; a `KeyError` escapes the local except (`JSONDecodeError`/`ValueError` only, :174), escapes `listen()` (`ConnectionClosed`/`OSError`/`TimeoutError` only, :176-185), escapes `_listen_loop` (`OSError`/`RuntimeError`/`ConnectionError`/`TimeoutError` only, run.py:442) → the task dies and `_on_task_done` (run.py:262-269) only logs. `_connected` stays `True`, so health stays green while `_generate_signals` keeps iterating frozen `candle_history` deques — the bot trades on dead data forever. The same escape path applies to exceptions raised by the `_on_message` handler (:173). The reconnect/backoff/watchdog hardening is defeated by any non-IO exception type.

**S290 (Medium) — Fixed in R159.** The `no_fills` alert rule can never fire — `run.py:397` calls `self.tracker.uptime_seconds()` on a `@property` (tracker.py:28-30), so every evaluation raises `TypeError`. `check_rules` swallows it per-rule (alerting.py:131), sets `last_fired`, and logs an error once per 300s cooldown — the "No orders executed in 10+ min" WARNING is dead code that spams the log. The wiring test hides it: `test_shm_alerting_wiring.py:209,238,264` feeds `tracker=SimpleNamespace(uptime_seconds=lambda: 0)` — a callable where production has a property — so the suite passes while the real rule crashes.

**S292 (Medium) — Fixed in R159.** `metrics.record_kill_switch(name)` at run.py:356 exists on `MetricsExporter` (metrics.py:258) but not on `MetricsCollector` (metrics_server.py — 11 methods, this isn't one). Without `--metrics`/`metrics.enabled` the publisher keeps the fallback collector (the run.py:225 swap never runs) → the first kill-switch activation raises `AttributeError` inside the unguarded `callback()` (shm_kill_switch_consumer.py:75) → the poll task dies. The `_hft_kill_active` latch is already set so signal push pauses, but the consumer is dead for any subsequent events and `_on_task_done` just logs once. A duck-type hole in the metrics swap.

**S293 (Info) — Fixed R168.** `SentimentConfig.follow_threshold` (sentiment.py:78, read at :198/:202 for the follow LONG/SHORT branches) is never wired: no `sentiment_follow_threshold` on `SignalBotConfig`, no yaml key, and `bot_helpers.py:54-56` passes only `fade_threshold`+`decay_rate`. The knob sits next to wired siblings in the same dataclass looking tunable — it's hardcoded 0.3 forever.

**Retracted/folded:** `portfolio_requests._calibrate`'s first `fitted` comprehension is dead work only when `eval_strikes` is provided (immediately rebuilt at median maturity); `implied_vol(t*365)` units are correct (the API takes days); `MetricsCollector`'s unused half (`set_pnl_total`/`set_drawdown`/`set_win_rate`/`record_error`/`record_circuit_breaker_trip` — zero callers) plus `render()` never being served folds into S272 (same dead-metrics class); `_daily_loss`'s `if not balance` treats `0.0` as absent — marginal, noted only; `shm_fill_consumer` docstring says "PostgreSQL" while the DB is SQLite — trivial drift, noted only.

**Verified clean:** `shm_ring_buffer.py` layout is byte-identical to the C++ `ShmRingBuffer` (192B header, head@64, tail@128, same magic; power-of-2 capacity enforced; header validated on open); SPSC try_push/try_pop correct with 64-batched flush; `shm_market_data_writer` is a real seq-lock (odd seq = write in progress, barriers); all 70 `SignalBotConfig` properties have live consumers; `SignalPublisher` does real auth (`secrets.compare_digest`), per-client sliding-window rate limiting on 9 compute endpoints, bounded signal history, lock-guarded client set; `backtest_requests` is honest (`data_source: "synthetic"` labeled, client candles validated, params clamped, GBM seeded); `portfolio_requests`/`analysis_requests` bound and validate every input and run heavy math in `asyncio.to_thread`; paper/live order paths are real (risk-based sizing, notional cap, lazy cached adapter, `client_order_id` dedup); `_on_shm_fills` persists with bounds-checked symbol IDs under exception guards; `_snapshot_equity` computes a real Sharpe from equity history; `CircuitBreaker` is a correct state machine with a probe budget.

Commit: b44ca41

---

## R133 — scripts/ remainder + helm/ + terraform/ (S294–S297)

**S294 (Low) — Fixed R168.** ~~pre-commit gate never builds/tests C++ in installed mode.~~ `pre-commit-check.py:835` sets `run_build = args.full or args.all`; `check_cpp_build_and_test` is invoked only under `has_cpp and (run_build or args.full or args.all)` (:878). The installed hooks call `--staged` (`pre-commit-hook.sh`) or `--staged --quick` (`pre-commit-hook.bat`) — neither sets the flag — so a staged `.cpp` change gets clang-format and a green gate with zero cmake/ctest. The header advertises `ctest (C++)` (:7) and the CI-map claims `test-cpp → cmake build + ctest` (:29); the equivalence exists only in `--full`, which nothing invokes. Distinct from S216's phantom cargo claim — here a real function is unreachable in the installed path. CI still catches breakage, so severity Low.

**S295 (Medium) — Fixed in R160.** `deploy.sh` native health gate could never pass. `health_check` curls `http://localhost:8080/ready` (:210), but `start_native` launches bare `python run.py` (:159) with no `--metrics` flag and `metrics.enabled: false` in `settings.yaml` — `run.py:199` therefore never starts `HealthServer`. The gate retries 30× and exits 1 (:245) even when every process is alive; native deploy cannot complete, ever. Companions: `:3000/health` in native mode is a vacuous vite-preview SPA-fallback 200 (the :225 comment about nginx's exact `/health` holds only in Docker); pid files are written (:150/:161/:172/:181) but `stop_deployment` never reads them — it uses the `pkill` patterns already recorded in S215; `status` (:355) greps `ps aux` for `ai_signal_bot`, the same unmatchable pattern, so status reports nothing while the bot runs.

**S296 (Medium) — Fixed in R160.** `deploy.bat` was can't-fail theater in both directions. The health loop (:168–206) iterates 30 times, curls all 4 services, logs warn/info — but has no healthy counter, no break, and no failure path: it falls through to unconditional `Health checks completed` (:206) and `Deployment completed successfully` (:229) even if zero services ever answered (the .sh twin honestly `exit 1`s at :245). `stop_deployment` (:101–104) runs `taskkill /FI "WINDOWTITLE eq exchange_simulator*"` etc., but the processes were launched with `start /B` (:132/:139/:154) — no window, no title — so the filters match nothing: both `python.exe` instances and `node.exe` are never killed. Only `hft_trade_bot.exe` dies (matched by `/IM`, :103). Every native deploy/restart stacks duplicate sim/bot/vite processes onto ports 8765/8766/3000.

**S297 (Info) — Fixed in R178.** ~~rollback restores 2 of 4 backed-up artifacts.~~ Backup takes config tar + exchange `data` + ai-signal-bot `data` + `audit` logs (`deploy.sh:48–59`, `deploy.bat:49–56`); `rollback` restores only config and exchange data (`deploy.sh:284–304`, `deploy.bat:241–253`). `ai_data_$TIMESTAMP` and `audit_$TIMESTAMP` accumulate on disk every deploy and are never read by any path — a rollback silently drops the AI bot's SQLite/WAL signal/trade database while reporting `Rollback completed`. Two write-only backup streams in both scripts.

**Dedup against prior rounds:** `pkill -f "ai_signal_bot"` not matching `python run.py`, `python -m exchange_simulator` run from inside the package, `docker-compose` v1, and the dead `ENVIRONMENT` flag are already S215; `ci-equivalence.py`'s phantom `test-rust → check_rust_build_and_test` row, the never-validated `CI_TO_PRECOMMIT` mapping, the `health-check.py` flat-layout blind spot, and the phantom `install-hooks.sh` are S216/S262; `scripts/ci/` orphanage is S226; `tfvars.example` `db_password` is S273.

**Verified clean:** `docker-smoke-test.{sh,bat}` — real ports, proper errorlevel/exit propagation, and compose's `--metrics` flag makes the `:9090/health` check valid; `commit-msg-hook.{sh,bat}` and the pre-commit hook shims correctly proxy exit codes; `health-check.py` is an honest report-only dashboard (always exit 0, never a gate); helm templates — `shareProcessNamespace` SHM sidecar, real probes on correct ports, vendored `files/alerts.yml`/`alertmanager.yml` and all 5 dashboards byte-identical to `monitoring/` originals (no drift), grafana provisioning paths correct, ingress routes WS paths to the right services, network policy sane; terraform — textbook VPC (public→IGW, private→NAT; the only `0.0.0.0/0`s are route-table entries), EKS openness already recorded as S204, S3 module clean.

---

## R134 — .github/workflows non-CI + root files + web-ui configs (S298–S302)

**S298 (Medium) — Fixed in R160.** `build-all.bat` was permanently red and silently skipped the simulator test suite. Three of its gate checks are dead at birth: `:51` runs `python -c "import exchange_simulator"` with cwd inside `exchange_simulator/` — the package cannot import itself (same broken-from-inside defect as S215/S220) → `[FAIL]` plus the entire `pytest tests/` branch is skipped via the `else` at :56-64, so the exchange suite never executes. `:94` imports `src.strategies.cross_exchange_arb.CrossExchangeArbEngine` and `:98` imports `src.strategies.marketplace.StrategyMarketplace` — neither module exists anywhere in the repo (marketplace lives only in `web-ui/`). Every invocation exits 1 while reporting a "Full Pipeline" that can never be green; the cry-wolf result trains users to ignore the gate entirely.

**S299 (Low) — Fixed R170.** `:97` unconditionally passes `-DCMAKE_TOOLCHAIN_FILE=%VCPKG_ROOT%\scripts\buildsystems\vcpkg.cmake` — with the var unset the path degrades to `\scripts\...` → cmake configure fails → `exit /b 1`. The sibling `build-all.bat:143` correctly guards `if defined VCPKG_ROOT`, and CMakeLists autodetects vcpkg anyway — the flag is unnecessary. `:140` tells the user to "Run start.bat" — no such file exists.

**S300 (Low) — Fixed R170.** `notify` (`:170-178`) runs `if: always()` and computes success via `!contains(needs.*.result, 'failure')` — `skipped` counts as success, so every `master` push (deploy and health-check are tag-only, `:108`) announces "Deployment SUCCESS" for a deployment that was skipped. The scp step (`:118`) copies `.env.prod.example` to `/opt/hft/` but no step ever materializes `.env.prod` — `env_file: .env.prod` (compose :137/:177) fails one layer earlier than the `:?`-interpolation failure S263 recorded; the release deploy is dead on a clean server regardless. `production-branch: main` + `production-deploy` gated to `main-or-tag` (:41-42) on a `master`-default repo → master pushes produce previews only.

**S301 (Info) — Fixed in R178.** ~~`nightly-backtest.yml` opens a fresh issue every failed night + dead pytest install.~~ `Create issue on regression` (`:224`, `if: failure()`) never checks for an existing open issue — a persistent failure files a new issue every night forever. `:37` installs `pytest pytest-asyncio` that no step invokes (everything is `python -c`). The pipeline itself is real — `src.strategies.strategies` re-exports both strategies and `Backtester.run(candles, strategy, symbol, warmup)` matches — but it inlines its own walk-forward rather than calling `scripts/walk_forward_ci.py`, leaving S214's orphan script orphaned.

**S302 (Info) — Fixed in R162. Makefile residue: `dev-exchange` is the 5th broken-from-inside site + 4 EOL `docker-compose` calls.** `Makefile:15` `dev-exchange` runs `cd exchange_simulator && python -m exchange_simulator` — the guaranteed ModuleNotFoundError already recorded for deploy.sh:148 (S215) and no-docker.* (S220); the documented dev target was born dead. `:12,:50,:53,:87` (`dev`/`docker-up`/`docker-down`/`docker-hub`) all call the EOL `docker-compose` v1 binary.

**Dedup:** notify vars-vs-secrets gates — S264; `:?`/env_file/`--env-file` interpolation — S263; `python -m` from inside package — S215/S220; Makefile `test-cpp` ctest-swallow — S208.

**Verified clean:** `release.yml` (real changelog→release, correct prev-tag via `HEAD^`); `codeql.yml` (deliberate cpp-only matrix, documented); nightly imports/signatures all resolve; `docker-compose.prod.yml` publishes all 5 health endpoints the deploy job curls; scp source list covers every prod bind-mount (`monitoring/`, both configs); both web-ui Dockerfiles declare and forward all 4 `VITE_*` ARGs that `deploy.yml` passes; `nginx.conf` serves a real `/health` with honest security-header comments and correct SW-cache split; `e2e/screenshots.spec.js` is 6 real screenshot tests with visibility assertions; `Makefile.prod` + `.env.prod.example` are consistent; `dependabot.yml` targets 8 live ecosystems; `Makefile logs` tails real files; `shared_config.yaml` remains an honest reference (S136).

---

## R135 — backend Dockerfiles + compose hub/staging + docs drift (S303–S305)

**S303 (High) — Fixed in R151.** The `exchange_simulator` Docker image is dead on arrival on every build path. Every compose file and `deploy.yml`/`build-all.bat` builds it with `context: ./exchange_simulator`, and the Dockerfile's `COPY . .` flattens the package *contents* into `/app` (`__main__.py`, `exchange.py`, …). The `CMD python -m exchange_simulator` then looks for `/app/exchange_simulator/` — which doesn't exist → `ModuleNotFoundError` → `restart: unless-stopped` crash-loop. Nothing rescues it: `pyproject.toml` is ruff/pytest config only (no packaging), `requirements.txt` has no `-e .`, and no volume mounts the source tree. The dev/staging/prod/hub compose paths and every pushed ghcr image all produce a simulator that never starts — the entire Docker data path (sim → ai-bot → hft → web-ui) has never worked. Same root cause as S215/S220's host-side `python -m`, different mechanism (flat `COPY` inside the image).

**S304 (Medium) — Fixed in R162. `docker-compose.hub.yml` runs the dev-image binary path against the prod image.** `:81` `command: ./build/hft_trade_bot config/config.yaml` — but hub pulls ghcr images built by `deploy.yml` from `Dockerfile.prod`, which copies the binary to `/app/hft_trade_bot`; `/app/build/` doesn't exist in the runtime stage → the hft service can't start via hub. Staging's identical `./build/hft_trade_bot` (:104) is *correct* because it builds the dev `Dockerfile`, which preserves `/app/build/` (Dockerfile:50) — the path is right there and wrong in hub. Also folded: staging's header advertises "Signal Engine V3 enabled (HMM regime detection)" (:6) while nothing in the file enables it (V3 is the hft C++ feature whose config keys are dead per S245), and staging-grafana gets no provisioning mounts → an empty Grafana under a "Prometheus + Grafana monitoring" claim.

**S305 (Info) — Fixed in R162. CONTRIBUTING/CHANGELOG residue.** `CONTRIBUTING.md:105,:241` still point at `start.bat` — a file CHANGELOG itself records as removed ("covered by no-docker.bat"); `:470` describes `docker-compose.prod.yml` as "(+ PostgreSQL, Redis, Prometheus, Grafana)" — postgres/redis don't exist in that file (phantom-dependency claim, S205 class). `CHANGELOG.md` is a version graveyard: only `[v4.0]`/`[v4.1]` at the bottom are versioned, ~2,200 lines of `[Unreleased]` entries carry internal v4.2–v5.3 sprint labels, and `package.json` declares 2.2.0 — the version regressed and no changelog entry maps to any shipped version. 5 sites total.

**Dedup:** ai-bot Dockerfile `run_logger` crash — S207; compose WS_URL/env plumbing — S210/S263; host-side `python -m` sites — S215/S220; CONTRIBUTING:256 — S217; Makefile v1-compose calls — S302.

**Verified clean:** both ai-signal-bot Dockerfiles are correct multi-stage builds (S207 aside); both hft Dockerfiles keep builder/runtime on bookworm for ABI stability with correct binary paths for their own layouts; staging ports/healthchecks/limits are consistent; hub's `depends_on: service_healthy` chains are right; the sim `--export` flags are real (`__main__.py:208-210` → `DataExporter`); SECURITY.md and the .github templates are honest.

---

## R136 — repo-wide residue sweep (S306)

**S306 (Info) — Fixed in R162.** `WEB_UI.md` third dead-launcher site corrected.
...
## R140 — grafana dashboards query↔metric cross-check (0 findings — honest zero round #3)

**Scope:** all 5 `monitoring/grafana/dashboards/*.json` (929 lines) — extracted every PromQL `"expr"`, 40 unique metric names, resolved each against the real emitters (`ws_prometheus.py`/`ws_metrics.py`, `MetricsExporter` in `metrics.py`, hft C++ metrics) plus datasource/provisioning consistency.

**No new findings.** Every queried metric name is emitted by live code — the `ai_signal_bot_*` series matches `MetricsExporter` Gauge/Counter declarations 1:1 (metrics.py:180-228), the `exchange_*`/`exchange_simulator_*` series are all in the sim's prometheus/metrics modules, and all `hft_*` names (including the three `shm_*_queue_depth` gauges) come from the C++ monitor. The only dead-query panels are `exchange_orders_filled_total`/`exchange_orders_rejected_total` — eternal-zero metrics already covered by open S281. The duplicate `ai_signal_bot_*` emissions in `metrics_server.py`'s never-served `render()` are S272's dead sink. Datasource `Prometheus` name-matches `datasources.yml` with `isDefault: true`; panel UIDs are unique; the single `"-- Grafana --"` datasource reference is a legitimate builtin panel.

---

## R141 — ci.yml full leaf-read + changelog reverse-drift (S309)

**Scope:** `.github/workflows/ci.yml` end-to-end (all 621 lines — the largest workflow, previously only edge-audited via S211/S265/S269/S271); CHANGELOG «removed X» claims verified against the live tree; `run_backtest.py` argparse vs documented invocations; stray untracked-file sweep.

**S309 (Medium) — Fixed in R182.** `docker-smoke` is permanently red, and `test-summary` propagates it to every push. `ci.yml:344` runs `docker compose up -d --wait --timeout 60` — the exchange-simulator container is DOA at startup (S303: `python -m exchange_simulator` against a flattened `/app`), so its :8775 healthcheck never comes up, `--wait` times out, and the `:348` `curl :8775/health` fails regardless. The job **cannot ever pass**. Then `test-summary` (`:500-539`) requires `success` from all 15 jobs including `docker-smoke` — so **every push shows a red pipeline**. A gate that is red by construction becomes decorative background and trains ignoring failures — worse than no gate. Meanwhile `build-docker` stays green (the image builds fine; the CMD only dies at runtime), giving CI the illusion of docker-path coverage.

**Verified clean:** all other jobs are honest — `test-python`/`test-cpp` (gcc-14 + clang-17)/`test-windows`/`test-e2e` (`VITE_MOCK_MODE` real flag) matrixes are real; `test-count` per-language floors are actually satisfiable (≈126 py / 12 doctest / ~12 ctest / ~150 js test files vs floors 20/10/6/30) — a genuine deletion-guard; `security-codeql` [python, javascript] doesn't overlap `codeql.yml` (cpp-only); concurrency group + read-only perms correct; S211's dead audit-gate and S265's unpinned websocketpp clone remain their own findings. `run_backtest.py` flags (`--db/--symbol/--candles/--balance/--plot/--optimize/--output-dir`) match the docs; `--no-visualizer` exists; localhost defaults are env-overridable. All CHANGELOG removal claims verified true against the tree (`test_ws_connection_pool.py`, `test_untested_modules.py`, `market_replay.py`/`timescaledb_client.py` [S286], `database/*.py`, `collaboration/`, `web-ui/src/exchanges/` — all absent; `helpers.py` itself was fully removed so the CircuitBreaker/RateLimiter claims aren't stale). `deploy/k8s/secrets.enc.yaml` is correctly gitignored; zero stray untracked files.

---

## R142 — web-ui vitest suite vacuity scan (0 findings — honest zero round #4)

**Scope:** `web-ui/src/test/` — all 153 `*.test.*` files (11,654 lines). The last un-scanned test tree: e2e specs were covered in R137 and this suite was only *counted* in R141's floor check, never read.

**No new findings.** The suite is genuinely healthy: 1,973 real `expect` calls (~13/file), zero `it.skip`/`it.todo`/`xdescribe`, zero `expect(true)`-class trivial asserts, zero snapshot tests. The 827 presence asserts (`toBeInTheDocument`/`toBeTruthy`) are a normal React render-smoke ratio backed by 459 `fireEvent`/`userEvent`/`act` interaction calls. All 23 `vi.mock` sites mock *dependencies* (hooks, stores, `lightweight-charts`) — never the unit under test — and mock shapes are faithful (the `useLocalStorage` mock returns the real `[value, setValue, remove]` triple). The cluster of ten identical 15-line files (`abTesting`, `colocation`, `deployStatus`, `hyperoptUI`, `onChainAnalytics`, `retrainingPipeline`, `scenarioSim`, `strategyVersionControl`, `teamCollab`, `widgetSDK`, `apiPlayground`) turned out to be deliberate NoDataFeed-disclosure tests asserting panels show the honest "no feed — this data is not produced" text rather than fabricating data — the exact anti-fake guarantee this audit demands. `useWebSocket.test` drives a real MockWebSocket class through the behavioral contract (auth frame before subscribe, `send` returns false when down, buffer tracking); `useExchangeData.test` is a 639-line protocol test (snapshot/fill/arbitrage message parsing); `useTradingStore.test` exercises the live Zustand store via `getState`/`setState`. All 150 unique relative imports resolve to existing modules — no S286-class zombie tests. `vitest.config` is honest (happy-dom, `isolate: true`, v8 coverage with real 40% thresholds scoped to utils+hooks) and `setup.js` registers jest-dom + per-test cleanup + localStorage/timer reset.

---

## R143 — helm chart leaf-read (S310–S312)

**Scope:** `helm/` — all 17 files (Chart.yaml, values.yaml, 10 templates, vendored `files/` copies of alerts/alertmanager/dashboards). The last untouched infra tree — never read in any round.

**S310 (Medium) — Fixed in R151 (dup of S203).** 2 of 3 PodDisruptionBudgets select zero pods. `pdb.yaml:26` selects `app.kubernetes.io/component: exchange-simulator` (hyphen) but the Deployment labels pods `exchange_simulator` (underscore — exchange-simulator.yaml:9). `pdb.yaml:43` selects `component: hft-trade-bot`, but hft is a **sidecar** inside the ai-signal-bot pod (ai-signal-bot.yaml:100-148) — no pod carries that label. Both PDBs render and look healthy in `kubectl get pdb` while protecting nothing — a fake disruption-budget guarantee. Only the ai-signal-bot PDB works (and accidentally covers the hft sidecar via pod membership). Compounding trap: `minAvailable: 1` on a 1-replica workload blocks drains forever.

**S311 (Medium) — Fixed in R151.** Image defaults are unreachable; a default `helm install` ImagePullBackOffs all four app images. `values.yaml` uses `hft-exchange_simulator:v2.0.0`, `hft-ai-signal-bot:v2.0.0`, `hft-trade-bot:v2.0.0`, `hft-web-ui:v2.0.0` — but `deploy.yml:75` pushes `ghcr.io/ezpectus/hft-tradebot--lite-version/<service>` and `:129-131` strips the leading `v` (published tag `2.0.0`, not `v2.0.0`). Wrong registry namespace AND wrong tag format — no repo path ever produces these names. The chart has nice `fail` guards for wsExchange/wsSignals/adminPassword but nothing forces image overrides.

**S312 (Low) — Fixed in R151 (dup of S203).** Ingress `/grafana` subpath is broken by construction. `ingress.yaml:42-48` routes `/grafana` to grafana:3000, but grafana.yaml sets neither `GF_SERVER_ROOT_URL`/`serve_from_sub_path` nor a rewrite annotation — Grafana answers with root-path redirects/assets, so the subpath 404s/redirect-loops the first time someone enables `ingress.enabled`.

**Verified clean — this is an unusually literate chart:** `fail` guards on the required values (WS URLs, grafana adminPassword); hft correctly deployed as a SHM-sharing sidecar with `shareProcessNamespace` + Memory-backed emptyDir; kill-switch relocated to a writable logs volume under `readOnlyRootFilesystem`; env comments document exactly the loopback traps (EXCHANGE_WS_HOST for probes, WS_URL/HFT_EXCHANGE_WS_URL for pod-to-pod dials); `/live`+`/ready` probes hit real endpoints (health_server.py:143-144, kept unauthenticated for kubelet); vendored `helm/files/` (alerts.yml, alertmanager.yml, all 5 dashboards) are **byte-identical** to `monitoring/`; network-policy is default-deny with an honest "can't select by DNS, allow 443" comment; `hft-trade-bot.yaml` is a deliberate comment-stub explaining the sidecar design. The sim Deployment pulls the S303-broken image — that CrashLoop is S303's blast radius, not a helm defect.

---

## R144 — residual tail: monitoring/alerts + .github templates + python dead-pin sweep (S313)

**Scope:** `monitoring/alerts/` dir, `.github/ISSUE_TEMPLATE`×3 + `PULL_REQUEST_TEMPLATE.md`, and the cross-cut never run before: every `requirements*.txt` pin vs actual imports.

**S313 (Info) — Fixed R170.** Zero `import numpy`/`np.` hits across the whole component — the GBM market simulator runs on stdlib `random`/`math`. The pin still gets installed on every CI run and baked into the sim Docker image (~50MB wheel plus supply-chain surface for zero benefit). Same class as S241's dead npm deps.

**Deduped / verified clean:** `monitoring/alerts/` is an empty untracked directory — git-invisible filesystem residue, not a finding. `exchange_simulator/tests/requirements.txt` dead+harmful → already **S199**. `vcpkg/` at root → gitignored local clone. `.github` templates are honest — the PR template names real ctest binaries (`test_signal_engine`, `test_signal_engine_v2`) and correct pytest commands. All other pins are live: ai-signal-bot 7/7 used (numpy in 22 files, matplotlib→backtest plots, tabulate, aiohttp, prometheus-client as declared optional); sim's websockets/aiohttp/msgpack/orjson/yaml all imported (msgpack/orjson via guarded optional-imports while being required pins — belt-and-suspenders, honest); dev pins (pytest-asyncio/cov/ruff/bandit/hypothesis) are tool-deps used via config/CLI, not imports.

---

## R145 — terraform/ tree + root stragglers (S314–S317)

**Scope:** `terraform/` — all 8 files (the last untouched IaC tree); plus root stragglers never read: `shared_config.yaml`, `web-ui/netlify.toml`, the three `monitor.py` tools, all four `.dockerignore` files.

**S314 (Info) — Fixed R170 (duplicate of S273 — files deleted in R167).** Both dev and prod examples ask the user to set `db_password`, but no `.tf` file declares `variable "db_password"` and the tree contains zero `aws_db_*`/RDS resources — `apply` emits an undeclared-variable warning while teaching users to "secure a database" that doesn't exist. Residue from a DB-era version of the modules.

**S315 (Medium) — Fixed in R151 (dup of S204).** The eks module is apply-breaking and exposes nodes publicly. `modules/eks/main.tf:48` pins `version = "1.28"` — past standard support (ended Nov-2024); the EKS API rejects new-cluster creation on EOL versions, so `terraform apply` dies on the first resource. And `eks/main.tf:116` feeds `var.subnet_ids` straight to the node group while both environments pass `concat(public_subnet_ids, private_subnet_ids)` — workers land in public subnets where `map_public_ip_on_launch` (vpc:45) gives them public IPs. Trading-infrastructure nodes publicly addressable.

**S316 (Medium) — Fixed R170.** The only consumer is `scripts/test_config_consistency.py:29`, which reads only the `symbols` list. The `risk`, `timeframe`, `account`, `websocket`, and `system` sections (~60 lines) are dead authority — an operator editing `max_risk_per_trade_pct` here changes nothing in any component. Bonus drift: `system.version: "3.0.0"` — a fifth version namespace on top of package.json 2.2.0 / py 1.0.0 / CHANGELOG v4–5 / helm appVersion 2.0.0 (S307 class).

**S317 (Info) — Fixed R170.** All 12 build contexts are per-component (`./exchange_simulator` etc. in 3 compose files + deploy.yml + ci build.sh) — no root-context build exists, so the file's 35 exclusion lines (`web-ui/dist`, `**/Dockerfile`, `*.bat`) affect nothing. It even lists `exchange_simulator/exchange_simulator/__pycache__` — the deleted nested-package layout. S308 dead-config class.

**Verified clean:** vpc module is competent (multi-AZ public/private, IGW+NAT, correct route tables/associations, outputs); s3 module has the right defaults (versioning, AES256 SSE, full public-access block, lifecycle tiers); both envs use proper s3+dynamodb state backends; `netlify.toml` is a real SPA config (history-API redirect, immutable assets, no-cache sw/manifest, security headers); the three `monitor.py` files are honest local tools with correct endpoints (ws :8766, hft log tail, SHM heartbeat); the three component-level `.dockerignore`s are honest and scoped to real contexts.
...
**S318 (Info) — Fixed in R180.** `CONFIGURATION_GUIDE.md:59-118` documents four `shared_config.yaml` sections deleted in S316/R170: `system:` (name/version/mode), `default_exchange:`, `timeframe:`/`timeframe_seconds:`, `account:` — plus "50 cryptocurrency pairs" (real: 49). The guide describes a file shape that no longer exists.

**S319 (Info) — Fixed in R180.** `TRADING_STRATEGIES.md` has detailed sections for only 3 of 6 wired strategies — TrendFollowing/MeanReversion/FFTCycle. `MarketMakingStrategy`, `MLEnsembleStrategy`, `SentimentStrategy` (all wired at `bot_helpers.py:54-63`) get no section; Sentiment is mentioned only in passing in the overview.

**S320 (Info) — Fixed in R180.** `PERFORMANCE.md` benchmark methodology is un-runnable + measures the nonexistent: `./hft_trade_bot --config <path>` (config is positional `argv[1]` — `bot_setup.cpp:58`), `--enable-latency-histograms` (no such flag — yaml `latency_histogram_enabled`), `cmake -DENABLE_PROFILING=ON` (no such option — real is `-DUSE_PGO=ON` + `-DCMAKE_BUILD_TYPE=Profile`). Latency table has a "REST API (exchange simulator) 5-20ms / ~10ms measured" row — the sim has no REST API (S074/S283). "5 strategies" ×2 — 6 wired. The "Measured" column is produced by nothing; `benchmark_suite.py` (S214 toy-theater) is still cited as the source.
...
## R192 — domain-math audit: backtesting/risk/portfolio/pricing/strategies (S328–S329)

Leaf-read of the trading-math stack — lookahead bias, zero-cost fills, NaN propagation, unit confusion. Two findings: one live correctness bug, one dead parallel stack.

**S329 (High) — Open.** `backtesting/backtester.py:130-132` — the loop builds `window = candles[start:i+1]` (includes bar `i`), passes it to `strategy.analyze(symbol, window)` (:224/:239), then `_open_position` fills at `candles[i]["close"]` slippage-adjusted (:253/:269). The strategy decides using the close it is filled at — impossible in live trading. Every consumer of this engine is flattered: UI StrategyBacktest, `run_backtest.py` CLI, the nightly regression gate, walk-forward, `compare_backtests`. Fees and slippage are real — the defect is timing, not cost modeling. No test pins bar timing, so the fix (`analyze` on `candles[start:i]`, fill at bar `i`) won't fight pinned expectations.

**S328 (Medium) — Fixed in R203.** `backtesting/backtest_engine.py` (330 lines) + `backtesting/pnl_calculator.py` (251 lines) — a second complete backtest stack (`BacktestEngine`, `BacktestConfig`, `BacktestTrade`, `PnLCalculator` with futures/options/funding PnL) referenced only by each other, `__init__.py` re-exports, and `test_backtest.py`. Zero production instantiation — the live path is `Backtester`+`results.py`. The one prod touchpoint makes it worse, not better: `backtest_requests.py:170` builds the dead module's `BacktestResult` while `BacktestComparison.add` is annotated for the live `results.BacktestResult` (`backtest_comparison.py:16`) — two same-named, divergent contracts for one concept. Fix: either wire the richer engine into prod (it has sortino/calmar fields the live result lacks) or delete ~580 lines.

**Checked, clean (not findings):** `mean_reversion` / `sentiment` / `trend_following` / `ml_ensemble` / `statistical_arbitrage` / `fft_cycle` / `funding_arb_detector` — real indicator math, warmup + `math.isnan` guards, ATR-derived SL/TP, cointegration+Kalman hedge, deterministic seeded event noise, no label leak in the ml_ensemble train path. `kelly` / `cvar` / `var` / `position_sizing` / `risk_manager` — real percentile/parametric/Monte-Carlo VaR, Kelly caps, zero-division guards. `volatility_surface` — real SVI/SABR calibration with sane fallbacks. `backtest_metrics` — correct Sharpe annualization. `markowitz` — annual `risk_free_rate=0.02` vs per-period returns is latent only: the request handler defaults rf to 0.0 and `PortfolioOptLab.jsx` never sends it, so no live unit mix; per-period `expected_return` is displayed unlabeled but consistent. `strategies.py` — honest re-export shim.

---

## R193 — exchange_simulator fill-model + JS backtest twin (S330–S333)

Leaf-read of the simulated exchange's fill/position/liquidation math — the counterparty that decides every fill in the system — plus the same lookahead check on the web-ui rule backtester.

**S332 (High) — Open.** `web-ui/src/utils/backtestEngine.js` — twin of S329: `evaluateConditions(candles, i, indicators, rules)` (:216) reads `candles[i].close` via `price_above`/`price_below` (:96/:99), and triggered rules fill at `candles[i].close` slippage-adjusted (:222/:237). Decide-on-close + fill-at-same-close is unfillable live — every rule-builder backtest in the UI is systematically flattered. The precomputed indicators are causal (`ema`/`rsi`/`avgVolume` indexed at `[i]` depend only on data ≤ i), so the defect is purely decision-vs-fill timing. Fix: evaluate on bar `i-1`, fill at bar `i`.

**S330 (Medium) — Fixed in R202.** `exchange_advanced_orders.py:162-190` — `_check_iceberg_orders` fills a slice on every tick with no marketability gate: `fill_price = order.price` when set (:177-178) prints fills at a stale limit price regardless of where the market trades (a sell-iceberg at 50000 keeps filling at 50000 while the market is 40000 — free money); when `price` is unset it drains one slice per tick at mid — a hidden TWAP, not an order resting in a book. The slice model alongside is dead: `IcebergOrder.on_fill`, `slice_size`, `slices_remaining`, `current_slice_filled`, `get_visible_quantity` are never used by the live path (`_execute_iceberg_slice` works on `hidden_quantity`/`visible_quantity` directly), and `_create_order` never sets `slice_size` → `slices_remaining` stays 0 forever while `to_dict` broadcasts `slices_remaining: 0`/`current_slice_filled: 0` as static fake fields. Fix: marketability gate (buy slice only when `current <= price`, sell when `current >= price`; unpriced → treat as TWAP explicitly) and either wire or delete the slice bookkeeping + its wire fields.

**S331 (Medium) — Fixed in R202.** `exchange_liquidation.py:113-150` — `_handle_partial_liquidation` is a shadow fill path that bypasses `submit_order` entirely: `fee=0.0` written into the ClosedTrade (:133), `account.total_fees` untouched, `filled_price = current_price` with no slippage (:148), no audit events at all (every other path logs ORDER_FILLED/POSITION_CLOSED/BALANCE_CHANGE), and a hand-built `ord-{N:08d}` id inconsistent with the `08x` order counter (:141-143). Full liquidation goes through `submit_order` and pays fee + slippage + audit; partial gets a free mid-price fill — the model flatters precisely the levered losing positions it liquidates. Fix: route partial liquidation through `submit_order(force_close=True)` with a partial quantity.

**S333 (Info) — Open.** Fill-path precision nits: `round(·, 2)` is the hardcoded tick everywhere — `exchange_order_submission.py:363,449`, `exchange_advanced_orders.py:236,251,298`, `market_simulator.py:323-347` — harmless for the majors list, silently collapses any future symbol priced under ~$0.005 into zero-price positions. Also `_TYPICAL_VOLUME = 500.0` is defined twice (`exchange_order_submission.py:20`, `exchange_advanced_orders.py:16`) — the market-impact coefficient and the partial-fill-price model can drift apart silently. Fix: one shared constant, tick-aware rounding or plain float passthrough.

**Checked, clean (not findings):** `submit_order` validation chain (NaN/quantity-max/TIF-consistency/OCO-resolved/NO_PRICE_DATA rejects all real); margin accounting traced across all close branches — exact close, partial close, and residual-overshoot all balance; SL/TP trigger directions and trailing-stop buy/sell semantics correct; funding sign convention correct (positive rate → longs pay shorts, notional at current price); OCO sibling-cancel correct; GTD expiry is broadcast as a non-fill deliberately (`ws_broadcast.py:316-322`); `market_simulator` mid-price/order-book generation is an honest documented model; `_apply_partial_fill` is a misnomer (it's a price-deterioration model, not partial fills) but keeps accounting consistent.

---

## R194 — hft-trade-bot C++ core: shm/position/execution/signal (S334–S336)

Leaf-read of the C++ engine's correctness core — lock-free IPC, position accounting, order wire path.

**S334 (Medium) — Fixed in R202.** `position_manager.h:105-117` — the REDUCED branch of `apply_fill` realizes `slice_pnl - fee` into `realized_pnl_total_` and also adds the fee to `it->fees_paid` (:112). `Position::update_pnl` (types.h:87-91) nets `fees_paid + funding_paid` off the surviving position, so when the position finally closes, `realized_pnl_total_ += unrealized_pnl` subtracts that same partial-close fee a second time. Effect: `realized_pnl_total_` is understated by every partial-close fee — conservative direction, but the total no longer reconciles with per-fill fees. Fix: on REDUCED either skip `fees_paid += fee` or don't subtract `fee` from the realized slice — not both.

**S335 (Medium) — Fixed in R202.** `position_manager.h:160-183` — `sync_position` adopts exchange-reported positions and refreshes qty/entry on tracked ones but deliberately never removes ("fills own removals"). That assumption breaks whenever a fill is missed — disconnect gap, dropped `fills_batch`, manual exchange-side close: the local ghost position persists forever, `has_position()` blocks new trades on that symbol indefinitely, and `check_sl_tp` keeps evaluating a non-existent position (can fire close orders for ghosts). Fix: remove local positions absent from the account broadcast, or age-out after N syncs unseen.

**S336 (Info) — Open.** `order_executor.h:54-56,466` — the asio open-handler writes non-atomic `connection_` then sets `connected_`; the watchdog thread reads `connection_` after a relaxed `connected_` load (:439). No formal happens-before — benign on x86, formally a data race on weak-memory targets. Fix: release-store `connected_` after writing `connection_`, or guard the hdl under `client_mtx_`.

**Checked, clean (not findings):** `shm_ring_buffer.h` — textbook SPSC (relaxed own-index loads, acquire/release cross-side, wraparound-correct `head - tail`, loud magic/capacity/element-size validation on open; creator-init races fail loudly, never silently corrupt). Three signal engines all genuinely wired — V3 wraps V2 (`bot_loop.cpp:163-166`), V1 is an explicit disabled-only fallback whose synthetic-book path logs an explicit "unrealistic" warning once (`bot_loop.cpp:289-303` — disclosed, not a hidden fake). `order_executor` — snprintf truncation guards on every wire path, auth precedes orders, arb sell-leg failure unwinds the buy leg with a critical alert, watchdog + cancelable reconnect. `apply_fill` open/increase/close branches otherwise correct; `closing_since_` close-retry staleness window (S248) present.

---

## R195 — ai-signal-bot safety gates + remaining src dirs (S337–S339)

Leaf-read of the last unaudited ai-signal-bot surface — `llm_engine`, `signal_validation`, `data_collection`, `database`, `technical_analysis`, `communication` — hunting fake features and unwired risk controls. The round's theme: **gates that don't gate anything**.

**S337 (High) — Open.** `communication/circuit_breaker.py` + `signal_publisher.py:308` + `run.py:571-584` — the CircuitBreaker is a real CLOSED/OPEN/HALF_OPEN machine wired into `broadcast_signal`, but **it can never trip**: `record_failure`/`record_success` have zero production callers (only `test_e2e_pipeline`/`test_comm_circuit_breaker`), so `consecutive_failures` stays 0 and `allow_signal` returns True forever. And even if it tripped, it would only silence the WS broadcast — `run.py:571` ignores `broadcast_signal`'s result and `_execute_paper_order`/`_execute_live_order` (:578-584) execute regardless; the SHM signal feed to the hft bot (:572) isn't breaker-gated either. The one thing a circuit breaker exists to stop — orders — is the one thing it doesn't touch. Fix: record failures on order/broadcast errors AND gate the order + SHM paths on breaker state.

**S338 (Medium) — Open.** `run.py:578-584` — `exchange.is_trading_active` gates only `_execute_paper_order`; the live path `_execute_live_order` has no halt check at all — in live mode orders keep firing during a sim halt (real-money blast radius). Separately, `_hft_kill_active` (:389/:572) pauses only the SHM signal feed — neither order path checks it. Fix: halt gates must cover the order path, not just the feed.
...
## R202 — sim fill-model + C++ position book (S330, S331, S334, S335 closed)

- **S330 — iceberg gate + live slice model.** Priced iceberg slices filled every tick at a stale limit (sell@51000 filling into a 50000 market); now gated on marketability (buy `current<=price`, sell `current>=price`, fill at limit). `slice_size` wired at creation + `on_fill` per slice — `slices_remaining`/`current_slice_filled` broadcast real values instead of static zeros.
- **S331 — partial liquidation through submit_order.** The shadow path (fee=0, no slippage, no audit, fake `ord-{N}` id) is deleted; partial liquidation goes through `submit_order(force_close=True)` like LIQUIDATION/SL/TP.
- **S334 — fee double-count on partial close.** REDUCED realized `slice_pnl - fee` AND kept the fee in `fees_paid` (netted again at final close). Fee now lives only in the realized slice.
- **S335 — ghost positions.** `sync_position` never removed: missed close fills left permanent ghosts that blocked symbols and could fire phantom close orders. New `reconcile_positions` drops a position absent from 3 consecutive account broadcasts (age-out prevents fill-vs-snapshot flap), per-exchange, logged by caller.
- Board: 9 open — Medium: S322 (venue-runner copy-paste), S328 (dead backtest stack ~580 ln); Low/Info: S323–S327, S333, S336.

## R203 — venue-runner dedup + dead-stack deletion (S322, S328 closed)
...
## R207 — ai-signal-bot/src full sweep (audit-only, 5 new findings)

Static audit of the largest uncovered area (66 files, ~12.9k lines). One defect family dominates — live code whose only data producer is never called:

- **S342 — closed-trade accounting absent (Medium).** `db.close_trade` and `tracker.record_trade` have zero production callers; `RealAccount.set_fill_callback`/`start_user_data_stream` are likewise dead. Fills persist as `OPEN`/`FILLED` rows forever → dashboard `trades_closed`/`win_rate`/`total_pnl` and the Prometheus `ai_signal_bot_win_rate`/`pnl_total`/daily gauges are structurally zero while real fills flow.
- **S343 — zombie strategy feeds (Medium).** `SentimentStrategy.on_news_event` and `MarketMakingStrategy.on_fill`/`update_inventory`/`update_toxicity` have zero callers — both strategies can only return NEUTRAL. Sentiment is `enabled: true` by default; the simulator broadcasts `news_event` fields that move prices, and the bot ignores them entirely.
- **S345 — live-adapter market-data read path dead (Medium).** `RealExchangeAdapter` starts a real Binance/OKX/Bybit feed that fills caches nobody reads (all `get_*` methods have zero prod callers; only `place_order` is used). Latent proof the path was never exercised: the `@aggTrade` stream is subscribed "for `last`" but never parsed — `last` stays 0.0 forever.
- **S346 — contaminated walk-forward validation (Medium).** `run_backtest.py` selects best params via `grid_search` over the full candle set, then "validates" them on windows inside the same set — an in-sample number printed as out-of-sample. `optimizer.walk_forward`'s `train_size` is only a skip offset, not a fitting window.
- **S344 — circuit-breaker Prometheus reporting half-dead (Low).** `record_circuit_breaker_trip` has zero callers (counter pinned at 0); `set_circuit_breaker_state` only runs while a WS client is connected — the gauge goes stale exactly when the breaker trips with no UI attached.

Clean: all strategy/risk/portfolio math, indicators (both numpy and pure paths), the SHM stack, WS request handlers, observability, monitoring, backtesting, llm_engine, and data-collection write paths verified as real implementations.

## R208 — slop-fix — all 5 R207 findings closed (board empty)

- **S342 — closed-trade accounting (Medium).** The sim's authoritative `trade_history` is now consumed per-tick via a `total_trades` cursor (`run.py:399-444`): new closes write `status='CLOSED'` rows, feed `PerformanceTracker.record_trade`, and hit the trade CSV — dashboard `trades_closed`/`win_rate`/`total_pnl` and the Prometheus gauges now reflect real closes (paper + live-signal + C++ fills on the sim account). `get_stats.total_fees` scoped to CLOSED rows to avoid double-counting execution-row fees. Live ccxt accounts expose no `trade_history` — that path stays honestly unwired.
- **S343 — zombie strategy feeds (Medium).** `ws_client` now exposes the sim's `news_event`; `_route_news_event()` converts it (intensity→magnitude, direction→sign) and dedupes rebroadcasts before calling `SentimentStrategy.on_news_event` — which also no longer flattens pre-scored sentiment for untyped events. `MarketMakingStrategy` gets real inventory: `_sync_mm_inventory()` diffs signed account positions into `on_fill` calls (opens, closes, flips all covered). `update_toxicity` remains unwired — no order-flow data source exists.
- **S345 — dead live-adapter read path (Medium).** The real exchange market-data feed now starts lazily on the first read instead of at adapter connect — order-only adapters open zero sockets. The `@aggTrade` stream is finally parsed and merged with `bookTicker` into per-symbol ticker state, so `last` is a real price rather than a permanent 0.0.
- **S346 — contaminated walk-forward (Medium).** `run_backtest.py` now splits candles chronologically (60% fit / 40% OOS): `grid_search` only sees the fit segment, `walk_forward` evaluates on the untouched tail. `walk_forward` itself is honest — `train_size` is now a real strictly-past context window (warmup), `test_size` the evaluated segment; the redundant `warmup` param was removed.
...
## R211 — slop-audit — `exchange_simulator` remainder — 3 findings

Fresh ground after the R210 clean web-ui sweep: the entire non-`exchange_*` simulator stack (~4.6k lines) — `ws_*` modules, market/options simulators, arbitrage, audit/data-export/config services, models, visualizers, `__main__`.

- **S347 — Dead WS-layer Prometheus metrics (Medium).** Three `/metrics` gauges are structural zeros: `record_broadcast_latency` and `record_delta_update` have zero production callers, and `compressed_size` is never passed — so `exchange_simulator_broadcast_latency_p95_ms`, `delta_update_ratio`, and `compression_ratio` can never move. Worse, `record_message`/`client_count` only run inside `_send_json` — the hot broadcast paths (`_broadcast_market_data`, `_broadcast_fills_batch`, `_broadcast_audit_events`) bypass them, so `messages_total`/`bytes_sent`/`clients_connected`/`message_size_*` only count connect-time sends while the tick loop is invisible. Any Grafana panel or alert on these is dead. Same family as S344.
- **S348 — Floating `create_task` ack sends (Info).** Four `asyncio.create_task(websocket.send(...))` calls (ws_message_handler:400,408,410,589) hold no task reference — GC can collect them mid-flight (asyncio docs warn explicitly) and send failures vanish. The `replay_state` push at :400 is the client's only pause-sync when speed=0 is set; losing it desyncs `replayPaused`.
- **S349 — `close_reason` misattribution on batch closes (Info).** `ws_exchange_events` reads `trade_history[-1].reason` for every closed order in a tick — with multiple closes in one tick (SL on BTC + TP on ETH), all get the last entry's reason. Wrong `close_reason` propagates to AlertWebhook's liquidation/sl_tp/fill classification and the trade CSV `CLOSED_{reason}` status.

Clean: `ws_message_handler` (per-message try, rate limiting, auth gate on control types, idempotent dedup, NaN/TIF validation), `ws_broadcast` (per-encoding variants, per-client subscriptions, seq numbers, delta computation, isolated tick loop), the full wire schema vs the web-ui consumer (every emitted type has a consumer), `websocket_server` (SHM seqlock, health/live/ready, graceful shutdown), `ws_metrics` histogram, `market_simulator` (GBM + correlated shocks + OU venue deviation), `options_simulator` (Black-Scholes + put-call parity), `arbitrage`, `audit_logger`, `config_validator`, `data_export`, `models`, `exchange`, `__main__`.

**R212 resolution — all 3 fixed:**
- **S347** — new `_send_tracked()` feeds size + per-client send latency (in `finally`, so backpressured failures count) on every broadcast path: `_broadcast_market_data` (main + arb payload), `_broadcast_fills_batch`, `_broadcast_audit_events`, `_broadcast_to_clients`. `record_delta_update` is fed at the per-key full-vs-delta decision in `_build_orderbook_data`. `clients_connected` now reads `len(self.clients)` at scrape. The unmeasurable compression surface was removed (`compressed_size` param, `compression_ratio` field, `exchange_simulator_compression_ratio` gauge) — permessage-deflate lives inside the websockets transport, so the app can never observe wire size; one eternal-zero gauge left `/metrics`.
- **S348** — `_handle_set_speed`/`_handle_update_config` are `async def`, dispatched with `await`, and all four acks go through `_send_json` — delivered reliably, encoded per client negotiation (msgpack clients previously got unparseable TEXT), and counted in metrics.
- **S349** — the reason now travels on the order: `Order.close_reason` (emitted by `to_dict`) is stamped in `_close_triggered_position`, `ClosedTrade.order_id` is written at close, and the trade stamp joins by `order_id` instead of `[-1]` position. The WS layer reads `order.close_reason`; batch closes and non-trade fills can no longer inherit each other's reason.

## R214 — slop-audit — `hft-trade-bot` remainder — 3 findings

Fresh ground after R194's fill-model core sweep: the full remaining C++ stack (~8.9k lines across 44 files) — monitoring/health, watchdog, kill switch, risk manager, the whole `core/` orchestrator, signal receiver + handlers + data store, all four IPC modules, pressure model, inline indicators, OBI utils, signal engines v1/v2/v3+HMM, adaptive order selectors, position manager, order executor, config parser+validator, logger, low-latency utils.

- **S350 — `test_integration_shm.cpp` cannot compile on any platform (Medium).** The file references a phantom API that never existed: `ExchangeId::Binance`/`SymbolId::BTCUSDT`/`Side::Buy` (enums are `BINANCE`/`BTC`/`BUY`), `fill.quantity`/`fill.order_id` (field is `qty`; no `order_id`), `producer.init(name, cap)` (`init()` takes no args), `ShmFillProducer::unlink`/`ShmSignalConsumer::unlink` (no such statics), `producer.push` (`push_fill`), `consumer.init` (`start(cb)`), `has_pending()` (`pending()`), and asserts `sizeof(SignalMsg)==64`/`FillMsg==48`/`MarketSnapshotMsg==64` (actual: 32/28/28). It's wired into every POSIX build via `if(NOT WIN32)` (CMakeLists:448) — Linux CI's `make -j` (ci.yml:162, gcc-14 + clang-17) compiles every test target, so the job is red. On Windows the guard silently skips it — which is how it rotted unnoticed since `5965b95`. Phantom coverage plus a broken build.
- **S351 — Dead and misleading IPC declarations (Info).** Zero references in `src/` or tests: `AlignedOrderBookLevel` (aligned_types.h:17), `RoutingDecision` (:231), `SymbolId` 49-entry enum (shm_protocol.h:85), `Action` (:147), `Side` (:154), `ExchangeId` (:139). `ExchangeId` also contradicts the wire: enum says `SIMULATOR=0`/`BYBIT=3`, while the `FillMsg.exchange_id` doc comment and the producer (`f.exchange_id = 3`, signal_receiver_handlers.h:57) treat 3=Simulator — anyone wiring `ExchangeId::SIMULATOR` would tag fills "Binance" per the wire doc. Latent today (Python discards `ex_id`), a trap tomorrow.
- **S352 — `HealthStatus.signal_engine_active` is structurally permanent-true (Info).** `is_healthy()` (system_monitor.h:308-310) includes the bit, but `update_health_status` (bot_loop.cpp:396-397) sets it from `engine_v2`/`engine_v1` pointers that `init_signal_engines` constructs unconditionally before the loop (bot_setup.cpp:156,179) — it can never be false, so the check is decorative. `cpu_usage_pct` (system_monitor.h:305) is a dead field: zero writers, zero readers, absent from both `is_healthy()` and `format_json`.

Clean: `SystemMonitor` (all counters→/metrics live, real histogram), `HealthServer` (thread, timeouts, 503-on-unhealthy), `Watchdog` (stale→terminate→reconnect), `KillSwitch` (file+SHM+programmatic, real cancel/close/notify), `RiskManager` (on_fill/reduce_exposure wired at bot_setup.cpp:341-344), `bot_loop`/`bot_setup`/`main` orchestration (fill→position→risk→balance chain, SL/TP, arb, graceful shutdown), `SignalReceiver` (msgpack negotiate, per-venue keying, delta application, fill→SHM producer with drop counting, watchdog+mutex-guarded hdl), `signal_receiver_data` (primary-venue fast arrays), IPC (`ShmRingBuffer` SPSC acquire/release, `ShmMarketData` seqlock, `ShmHeartbeat` write-order, `ShmSignalConsumer`), `PressureModel`/`inline_indicators`/`obi_utils` (real OBI/microprice/toxicity/Wilder math), `SignalEngineV2` (real 6-indicator composite, honest "no tape → renormalize" S247), `SignalEngineV3` (real log-space online HMM + regime gating), V1 engine (real FFT + 6-vote), `AdaptiveOrderSelectorV2`/`OrderTypeSelector` (live via order_executor.h:143), `PositionManager` (S334/S335-hardened), `OrderExecutor` (S154 TIF serialization, arb unwind), `config_parser`/`config_validate` (env expansion, hard-throw validation incl. fraction-vs-percent drawdown), `logger` (real rotating sinks + error-count sink), `low_latency` (SPSC/spinlock/histogram/thread-affinity all real).

**Fixed in R215:**

- **S350** — rewritten against the real API: wire-size contract asserts (32/28/28/16), `ShmFillProducer`→ring consumer roundtrip with field asserts, full-ring failure, `ShmRingBuffer<SignalMsg>`→`ShmSignalConsumer::start(cb)` thread delivery, `KillSwitchMsg` roundtrip. Bonus defect fixed: `shm_fill_producer.h` used `spdlog::error` without its own `#include <spdlog/spdlog.h>` — survived on include order, now self-contained.
- **S351** — `SymbolId`/`AlignedOrderBookLevel`/`RoutingDecision` deleted (drift hazard + zero refs); `ExchangeId` reordered to the wire doc (`BINANCE=0,OKX=1,BYBIT=2,SIMULATOR=3`) and wired into the producer along with `ipc::Side` (fill side) and `ipc::Action` (signal decode) — dead declarations became the live protocol contract. Wire bytes unchanged.
- **S352** — `signal_engine_active` is now a real liveness bit: `BotContext::last_engine_eval_ms` is stamped at both eval sites (`generate_signal` v2/v3, `engine_v1->analyze` v1); the gate is "engine exists AND (warmup OR eval <60s)" — a stalled engine loop now trips `/health`. Dead `cpu_usage_pct` field removed.

## R217 — slop-audit — infra-layer re-sweep — 1 finding

Post-R216 (board 0, zero verify debt) fresh sweep of the infrastructure layer: `terraform/` (574 lines, 6 files), `monitoring/` (~650: alerts/alertmanager/prometheus/grafana provisioning/ebpf/test_alerts), `scripts/` (~2.5k unaudited: ci/*, hooks, pre-commit-check gate, walk_forward_ci, benchmark_suite, test_config_consistency, report.py), `helm/templates` (~950).

- **S353 — Stale docstrings claim capabilities the code doesn't have (Info).** Two sites, same defect pattern (doc-claim outlived code):
  - `monitoring/ebpf_monitor.py:153` — `_report` docstring: "Log current stats as JSON **and update Prometheus metrics**" — the `prometheus_client` Gauge block was added in Пачка HH then deleted (git-confirmed); the docstring survived the removal. Function only logs JSON.
  - `scripts/benchmark_suite.py:3-4` — module docstring: "Latency measurement for **all HFT components**... for each pipeline stage" — all six benches are inline toy loops (`json.loads` on a canned dict, dict-swap book update, toy RSI, literal validation); zero imports from real pipeline code. `docs/PERFORMANCE.md:25-29` already honestly discloses "inline toy loops, not the real pipeline" — but the script's own header still inflates.

Clean: terraform end-to-end (pinned provider, encrypted/locked S3 backends, EKS KMS+private endpoint+audit logs, private worker subnets, textbook VPC, versioned+encrypted S3; single-NAT is a documented cost trade-off); `monitoring/alerts.yml` — all 17 metric refs resolve to live emitters (`exchange_*` via ws_prometheus, `_CountingOrderHistory` makes orders counters real, `ai_signal_bot_*` via MetricsExporter, job names match prometheus.yml); `alertmanager.yml` honestly documents receiver-less default; grafana provisioning consistent; `ebpf_monitor.py` is real eBPF (paired sys_enter/sys_exit tracepoints); `test_alerts.py` honest structural checks; `scripts/ci/*` all real and wired via Makefile:85; `pre-commit-check.py` is an honest gate (all_ok→exit, correct staged-mode narrowing, loud [SKIP] notes); both git hooks really invoke the gate (incl. WindowsApps-python-stub guard); `walk_forward_ci.py`/`test_config_consistency.py`/`report.py` real; helm: prometheus `hft-trade-bot` job targeting `ai-signal-bot:9091` is CORRECT (hft is a sidecar in that pod; Service exposes 9091); vendored `helm/files/*` byte-identical to monitoring originals; Makefile targets all resolve.

**Fixed in R218:**

- **S353** — both docstrings corrected to match reality: `ebpf_monitor._report` now says "no metrics export — standalone tool" (rejected restoring the Gauge code — it exposed nothing without a `/metrics` endpoint, which is why it was deleted); `benchmark_suite` header now says "synthetic proxy micro-benchmarks (NOT the real pipeline)" + points at `docs/PERFORMANCE.md`. `py_compile` clean on both.

## R219 — slop-audit — docs-vs-reality sweep — 2 findings

Claim-level audit of the six docs never systematically verified (`TESTING.md`, `ADVANCED_ORDER_TYPES.md`, `RISK_MANAGEMENT.md`, `QUICK_START.md`, `TRADING_GUIDE.md`, `ARCHITECTURE.md` — ~2.4k lines) plus the remaining root scripts (`no-docker.{sh,bat}`, `install-deps.bat`, `build-all.bat`, `.pre-commit-config.yaml`).

- **S354 — TESTING.md systematic numeric + coverage rot (Medium).** Nearly every count is wrong and the doc contradicts itself: Python 118 (:23,:95) / 126 (:88) → actual **120**; C++ 26 (:23) / 25 (:89,:167) → **23**; JS 159 (:23) / ~158 (:90) / 162 (:194) → **159**; totals 303 (:23) / ~307 (:84) / 304 (:91) → **302**+5 e2e. Phantom test files: `test_trading_flow.py` (:113-116, listed as 1 of "4 integration files" — 3 exist), `test_order_types.py` in `ADVANCED_ORDER_TYPES.md:349` (same rot pattern; actual: `test_advanced_order_types.py`/`test_exchange_advanced_orders.py`). False coverage claims: :230 says `test_alerts.py` validates "metric names match exports" — it's structure-only, never compares names; :241-242 says `test_integration.py` verifies `:8080/health` + `:9090/metrics` — zero health asserts there (they live in `tests/unit/test_health_server.py`); :240 `:8775/health` in `test_websocket_server.py` — comment only. Stale counts: exchange_simulator "29+1" (:123) → 31; "15 security tests" (:158) → 17.
- **S355 — RISK_MANAGEMENT.md phantom-API tour (Medium).** Nearly every code example fails on copy-paste. Phantom module+class: :245-259 documents `src/risk/var_stress_test.py` + `RiskAnalyzer` — neither exists. Wrong method names: `calculate_historical_cvar` (:139) → actual `calculate_cvar`/`calculate_expected_shortfall`; `size_by_volatility` (:208) → `calculate_position_size`; `run_covid_crash` (:241) → `covid_crash_scenario` (different args); `update_stop_loss` (:302) → `init_position`+`update`. Wrong kwarg: `calculate(entry=65000)` (:176) → `entry_price`. Phantom test file: `test_risk_modules.py` (:344-345, listed twice). Verified accurate: `VaRCalculator` ctor + `calculate_historical_var`, `RiskConfig` fields, config keys, C++ `precheck_order` wiring, all 6 module files.

Clean: `QUICK_START.md` (49 symbols, 3 exchanges, `.env.mock`, `prod-up`, C++20, all ports/commands resolve — only a stale self-contradictory comment at :176); `TRADING_GUIDE.md` (all 8 hotkeys resolve in `useAppShortcuts.js`, `VITE_MOCK_MODE` real); `ARCHITECTURE.md` (every file path resolves incl. `signal_engine_v2_finalize.h`/`watchdog.h`, `run_logger.py` absence honestly disclosed, panel counts 278/271 exact, S230/S232/S235 audit annotations honest); `ADVANCED_ORDER_TYPES.md` (all classes/fields/enum/semantics match `models.py` — only the test filename rots); root scripts all honest (real error propagation, S220-aware `python -m` from repo root, correct Dockerfile/npm/cmake invocations); `.pre-commit-config.yaml` honestly delegates to the canonical hook.
...
## R225 — slop-audit — CLI surface + hft scripts — 0 findings

Two-part sweep: (a) every `argparse.add_argument` across all entry points (~15: run.py, run_backtest.py, exchange_simulator `__main__`, pre-commit-check.py, walk_forward_ci.py, tools/*, monitor scripts, hft scripts) cross-checked against every `--flag` invoked in tracked docs/scripts/compose/Makefile/CI; (b) first read of `hft-trade-bot/scripts/{monitor,run}.py` (131 lines — build.py was covered R113).

- **CLI consistency: clean.** All invoked flags resolve (`--no-visualizer`/`--metrics`/`--dashboard`/`--config`/`--backtest`/`--export*`/`--staged`/`--lint`/`--msg-file`/`--quick`/`--fix`/`--full`/`--all`/`--tests`/`--csv`/`--output`/`--baseline`/`--threshold`/`--window-days`/`--step-days`/`--days`/`--db`/`--symbol`/`--candles`/`--plot`/`--optimize`/`--output-dir`/`--url`/`--duration`/`--target`/`--verbose`/`--scenario`/`--pid`/`--interval`/`--follow`/`--since`/`--exchange`/`--debug`/`--clean`/`--jobs`). Phantom invocations (`--strategy`/`--period`/`--walk-forward`/`--benchmark` on run_backtest, `--benchmark` on hft_trade_bot) exist **only in gitignored `docs/theory/`** — out of repo weight per S080. `--paper` on `scripts/run.py` was the one real fake-flag — already found+fixed as **S225** (R165). `--lite-version` grep hits are the GHCR image name, not a flag.
- **hft scripts: clean.** `monitor.py` reads `/hft_heartbeat` as `[u64 ts_ms][4×i64 counters]` — byte-identical to the `shm_heartbeat.h` producer layout, which itself documents the contract ("must stay in sync with monitor.py's struct.unpack offsets") and the write-order rationale (counters-then-timestamp). Windows verbatim-tag vs POSIX `/dev/shm` split matches the C++ sides. `run.py` does real binary discovery (Debug/Release/.exe fallbacks) + config-path check + honest errors.

## R226 — slop-audit — async-task lifecycle sweep — 1 finding

Concurrency pass over all 13 `asyncio.create_task`/`ensure_future` sites in `ai-signal-bot/src`, `run.py`, `exchange_simulator/` — for each: is the task referenced, cancelled on shutdown, and do exceptions propagate?
...
## R236–R238 — S364 extensions + verify — board still 0 open

S364 grew to 18 sub-sites across the full pipeline (`sim tick → encode → wire → C++ ingest → store → signal → risk → order → wire`): encode-once broadcast cache (measured **8.3x** per tick at 8 clients), `pack_into`/`unpack_from` direct mmap IO (**1.3x**), amortized candle trim (**1.6x**), scratch map keys, zero-alloc WS sends, single-lock SHM sweep, post-commit notify. `slop-verify` R237 re-checked all 18 claims against live code — all hold. Residual micro-sites closed in R238 (`now_ns` hoisted per tick, health-status throttled to 1s). ai-signal-bot swept clean (encode-once broadcast, `pack_into` SHM writer already correct).

## R239 — playwright e2e unblocked — 1 finding fixed (S365)

The e2e suite was recorded as env-blocked — actually `playwright.config` `webServer` auto-spawns `dev:mock`. First real run: 32 pass / 3 fail, ~9.5k console errors per load. **S365 (Medium)** — four runtime defects + two test/UX defects: `OptionsStrategySimulator` NaN SVG coordinates (`spot=0` → `0/0` in `priceAtExpiry` and flat P&L range), `IndicatorBuilder` setState-during-render (`onIndicatorsChange` inside `useMemo` → moved to `useEffect`), `OpenInterestTracker` per-render crash (orphaned `.filter().sort()` ASI-glued onto `return null` — dead code, OI comes from candle volume), `PerfAreaChart` `setData` assertion throw (mixed/duplicate times via `p.time || i` → normalized to strictly-increasing finite times), toast region physically covering the tab bar (`pointer-events-none` region, interactive children `auto`), ambiguous `getByRole('alert')` (narrowed to the DEMO MODE banner). New regression tests: 3 files / 9 tests — the `spot=0` case fails pre-fix. **e2e 35/35, vitest + eslint + tsc clean.**
...
## R242 — console-error gate in mount sweep — 1 finding fixed (S368)

The mount sweep now captures `console.error`/`console.warn` during every panel mount and fails on real React dev warnings (test-harness `act` noise filtered) — duplicate keys, setState-in-render, and invalid-attribute warnings are now a hard gate across all 271 panels. It immediately caught **S368 (Low)**: `BotStatus`'s activity feed keyed rows `${symbol}-${time}`, so a signal and a fill for the same symbol on the same tick produced identical keys. Key now `${type}-${symbol}-${time}-${i}`. Sweep 271/271 clean.

## R244 — interaction pass in mount sweep — 1 finding fixed (S369)

The mount sweep proved rendering but never exercised handlers. It now fires `click` on every rendered `button` and `change` (`'1'`) on every rendered number/text input for each of the 271 panels, settles triggered effects, then re-asserts zero real console warnings and zero `NaN`. First run caught **S369 (High)** — two interaction-only defects: `PortfolioOptLab` stored a `Set` in `selected` while `chosen` consumers call `.includes`/`.length`/`.map` → `TypeError` on the first asset-chip click; `VariationalAutoencoder`'s decoder backprop indexed `Wout` transposed (`[hidden][input]` vs its `[inputDim][hiddenDim]` layout) and `bout` over the wrong axis — square default dims masked it, while `windowSize=4` crashed and `hiddenDim=4` wrote `NaN` into weights → `NaN` SVG geometry. Both fixed at the root; 3 new regression tests; sweep **271/271**, full vitest **172 files / 1440 tests green**.

## R245 — all-control + second-tick pass — 1 finding fixed (S370)

The mount sweep now exercises every control type (buttons; `change '1'` on number/text/textarea; checkbox/radio clicks; range mid-value; select second option) fired twice — covering conditionally-revealed controls and both toggle directions — then re-renders each panel with a second-tick context (appended candle per series, fresh fill/signal) to drive update-path `setData`/memo/effect re-runs. It caught **S370 (High)** — two numerical-stability defects invisible to mount-only coverage: `BurgersEquation` documented Lax-Friedrichs but implemented unconditionally-unstable FTCS advection, and its combined explicit diffusion was Nyquist-unstable (|G(π)|=1+4d) — a rough histogram IC blew `|u|` to 5.8e+35 in one step, producing an effectively infinite substep loop and `NaN` paths; fixed via operator splitting (conservative LF advection + separate diffusion substep under per-term CFL bounds, dt clamped). `RecurrentNeuralNetwork`'s learning-rate input had no upper clamp — `lr=1` diverged training into `NaN` DOM output; now clamped plus a non-finite-loss early stop keeps predictions finite. 3 new regression tests; sweep **271/271**, full vitest **173 files / 1443 tests green**.

## R246 — selection-switch + empty-context pass — 1 finding fixed (S371)

The mount sweep now re-renders each panel with an alternate `selectedExchange`/`selectedSymbol` pair (per-selection paths) and with a fully-empty context matching the real pre-snapshot state. It caught **S371 (High)** — `BayesianPricePredictor` called `betaPath`'s `useMemo` **below** its `if (!data)` early return, so any transition into the no-data branch (symbol switch to an unlisted pair, empty feed, startup race) dropped a hook → React `Rendered fewer hooks than expected` crash of the panel subtree. The memo now runs unconditionally above the early return. Root cause of the class: `eslint-plugin-react-hooks` was never wired into the flat config — `rules-of-hooks` is now an `error`-level gate over all of `src` (verified non-vacuous on a seeded violation; this was the only live instance). 2 new regression tests (populated→empty→populated hook-count cycle); sweep **271/271** across all five phases, full vitest **174 files / 1445 tests green**.

## R247 — boundary-data + lifecycle pass — clean (0 findings)

Two final lifecycle phases added to the panel sweep: a minimal-data re-render (exactly one candle per series, one fill, one signal — the boundary where `length < N` guards degrade but unguarded single-element math could NaN) and an unmount→remount cycle (cleanup paths — intervals, chart-lib teardown, subscriptions — with localStorage cleared so remount must render as for a new user). All **271/271 panels clean across all seven phases**. The only initial failure, `onboarding`, was investigated and found to be by-design (dismissal persists in localStorage; clearing storage before remount verifies the harder path and passed). The panel gate now covers the complete externally-reachable lifecycle: mount → all-control interaction → data tick → selection switch → empty → minimal → remount.

## R248 — hft ctest unblocked via clang-MinGW — 1 finding fixed (S372)

The standing "MSVC-blocked" note only covered prebuilt ASan exes from the S:-drive machine — the host actually has LLVM-MinGW clang 22.1.8 + cmake 4.4 + ninja. Enabling the suite surfaced **S372 (High)** — `tests/doctest.h` is a hand-rolled 156-line stub ("replace with the full doctest.h"), missing `CHECK_THROWS*`/`REQUIRE_THROWS*`/`WARN`/`MESSAGE`/`INFO`, with `SUBCASE` running subcases sequentially instead of re-running the prefix and `REQUIRE`==`CHECK`. `test_doctest_order_type_selector` includes `"doctest.h"` (always resolves to the stub) and uses `CHECK_THROWS_AS` — **registered in ctest but never compiled anywhere: a dead test file**. The stub gained the THROWS/NOTHROW family, and CMake gained `HFT_TESTS_ONLY` (deps discovered QUIET, main exe skipped, each dep-requiring test gated on its `TARGET` — full hosts unchanged, dep-less hosts build the dep-free set). First real execution caught three defects: `InlineRSI` returned ~100 on a perfectly flat series (gain=loss=0 fell into the no-losses branch — now 50); `OnlineHMM` annualized its per-observation volatility proxy (`·252`, ~16x mis-scale — VOLATILE out-scored every state, so TRENDING/RANGING detection was dead on live data) and RANGING's return variance (5e-5, σ≈0.7%) was wide enough to absorb trend drifts (now 1e-7, σ≈0.03%); and `test_doctest_pressure_model`'s fixture claimed a 1bps spread but built 2bps with a TIGHT expectation (fixture parametrized; real 0.4bps→TIGHT, 2bps→NORMAL). Also fixed a latent copy-paste linking `test_doctest_hft_config` inside the signal_receiver section. **ctest: 13/13 binaries, 221 cases — all green** (first local run ever); remaining targets are dep-blocked (fmt/spdlog/yaml/json, POSIX-only shm/signal_flow), not env-blocked.


## R250 — ctest registration completeness — 1 finding fixed (S373)

Cross-checked test files on disk vs CMake registrations: **S373 (Medium)** — the v2 quartet (`test_v2_engine`/`indicators`/`infra`/`pressure_adaptive`) existed on disk but was never registered — four dead assert-based suites, all dep-free. Registered via `add_hft_test` with `signal_engine_v2.cpp` (out-of-line `SignalEngineV2Params::validate`). First real execution caught a rotten test: `test_signal_engine_v2_toxicity_penalty` used `toxic_penalty=0.8` where the residual (0.96×0.28≈0.27) still saturates above `pressure_threshold=0.2`, making `sig_toxic < sig_clean` impossible; fixed to `toxic_penalty=1.0` so the penalty crosses the threshold and normalization exposes the reduction. **ctest: 17/17 binaries — all green**; file↔registration parity 26/26.


## R251 — dep-gated ctest targets unblocked via vendored deps — 1 finding fixed (S374)

Vendored gitignored `deps/` (reproducible via `hft-trade-bot/fetch-test-deps.sh`): nlohmann/json single-header, spdlog header-only, fmt headers, standalone asio, prebuilt static yaml-cpp — CMake's tests-only mode now mints the canonical `*::*` targets so the `if(TARGET …)` gates light on dep-less hosts. **S374 (Medium)** — first execution of the formerly gated targets caught three fixture defects: `test_integration_kill_switch_monitor` was written against a different API (`KillSwitch(trigger, 0.5)`, `get_counter(Counter::…)`, map-style `snapshot()`, auto-clear semantics) — rewritten to the real contract including sticky activation (FILE_TRIGGER deletes the file; only `deactivate()` re-enables); config test fixtures used phantom yaml keys `ws_url:`/`default:` (real: `websocket_url`/`default_exchange`), which also made the "missing ws_url" negative test pass vacuously; and `test_signal_engine`'s spectral fixture oscillated at freq 0.3 (~21-candle period → inside the impl's low band — split at n_fft/8), so a genuinely cycle-dominated signal needed freq=1.0. **ctest: 22/22 binaries green.** `test_doctest_signal_receiver` remains gated on real OpenSSL headers (asio's TLS path parses them even when unused) — documented in the gate. Correction: R250's "v2 quartet never registered" was wrong — it sat behind `if(TARGET fmt::fmt)` via foreach-variable names; the record is fixed.


## R252 — final dep-gated target unblocked via vendored OpenSSL — 1 finding fixed (S375)

`test_doctest_signal_receiver` was the last dep-blocked suite: asio's TLS path parses `openssl/*.h` even when unused and OpenSSL source trees only ship `.in` header templates. Solved by vendoring the msys2 `ucrt64` prebuilt OpenSSL 3.6.4 package (UCRT ABI-compatible with LLVM-MinGW) — real headers + static `libssl.a`/`libcrypto.a`, fetched by `fetch-test-deps.sh` via the `zstandard` python package. **ctest: 23/23 binaries green — every Windows-compatible test file executes.** The only remaining un-run files are `test_shm`/`integration_signal_flow`, POSIX `shm_open`-only by design.


## R253 — clang-tidy hot-path sweep — 1 finding fixed (S376)

First static-analysis pass over the trading-loop TUs (clang-tidy 22.1.8, driver-derived sysroot): **S376 (High)** — `bugprone-exception-escape` caught `SignalEngineV2::analyze_incremental` (`noexcept`) reaching `unordered_map::emplace(std::string, …)` — a `bad_alloc` under memory pressure would have called `std::terminate` on the trading thread; the same noexcept lie existed in `SignalEngineV3::analyze`/`analyze_incremental` via `get_or_create_hmm_state`. Both now degrade gracefully through noexcept `try_get_*` helpers (neutral signal / V2 base without the regime layer). Also: dead `now_ms` param dropped from `set_long_signal`; `test_fixtures.h` helpers made `inline`; and the "cooldown blocks consecutive signals" doctest was **vacuous** — it called the stateless `analyze()` that never consults the gate — rewritten on `analyze_incremental` with a real `now_ns()` base and a fires→blocked→fires-again assert. **ctest: 23/23 green.** Wire-layout enum-size warnings left untouched (SHM ABI is a byte contract).
...
## R257 — production exe build unlocked + real-loop audit — 1 finding fixed (S378)

**S378 (Medium):** `signal_latency_hist` was scoped to the whole per-symbol iteration — it swallowed candles-fetch, book-prep, and the nested risk/exec timers, and `continue`-skips still recorded samples. Now wraps `generate_signal()` only; risk/exec timers are siblings. Zero runtime cost (same clock pair per computed signal).

**Build unlock:** the production `hft_trade_bot` exe was never compiled on dep-less hosts (Boost `REQUIRED` → vcpkg-only). Dep resolution restructured: full-build finds are `QUIET`, the vendored `deps/` minting is shared by both build modes, and a post-check re-enforces REQUIRED semantics. Zero `boost::` usage in `src/` — `pch.h`'s `boost/system/error_code.hpp` is now `ASIO_STANDALONE`-guarded and vendored asio replaces boost-asio entirely. First-ever `hft_trade_bot.exe` build+run on this host (smoke: banner → config-validated 49 symbols → SIMULATOR mode). Full-build warnings on production code: **zero** (dead `process_sl_tp` balance param dropped; MSVC pragma guards; `[[maybe_unused]]` on POSIX-only members; redundant aggregate init). **ctest: 23/23 green.**
...
**Deeper root cause found via live Docker run:** even with all send paths counted, `fill_rate` read 4.73 — the simulator executes its own auto-arb orders on the *same exchange account* and broadcasts those fills on the shared connection. HFT counted foreign fills as its own. Fix: every HFT order path now submits a `client_order_id` with an `hft_` prefix (market/limit orders, `hft_close_*`, `hft_arb_buy/sell_*`, `hft_arb_unwind_*`); the sim echoes it through `Order.client_order_id` → `to_dict()` → fill frames (close-position handler now propagates the request's cid). `FillCallback` carries the cid; `bot_setup` increments FILLED/CANCELLED/REJECTED only for `hft_`-prefixed ids. All fills still feed `apply_fill` — the exchange account is shared and the local book must reflect real account activity — but only own-order events feed metrics/risk bookkeeping. Live result: `fill_rate` 0.87–0.96 across runs.
...
**S397 (Medium):** `o.value("client_order_id", "")` threw `json::type_error` when the field was present but `null` — sim auto-arb orders carry `client_order_id: None` → every foreign fill threw, got counted in `ERRORS` (928 errors in ~3 min live), and the fill was dropped *before* `apply_fill`, so the position book silently missed real account activity. Null-safe extraction via `contains + is_string`; regression test feeds a batch order with explicit `client_order_id: null` and asserts the fill reaches `on_fill` with an empty id. Audited all other `.value()` reads against `Order.to_dict()` — `price`/`rejection_reason`/`oco_group_id`/`expire_ts` are the only other nullable fields and none are read by the receiver.
...
**Verify:** ctest 23/23 Windows + 26/26 Linux, pytest 446, both prod exes rebuilt, receiver tests cover batch fills + null cid + cancel count. **Live Docker:** 7/7 healthy; `fill_rate` 0.87–0.96, `rejection_rate` 0, `errors_total` 0, `risk_rejected` populated separately; foreign fills visible in logs as `id=-`, own orders as `id=hft_*`.
