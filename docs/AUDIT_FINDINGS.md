...
**S297 (Info) — rollback restores 2 of 4 backed-up artifacts.** Backup takes config tar + exchange `data` + ai-signal-bot `data` + `audit` logs (`deploy.sh:48–59`, `deploy.bat:49–56`); `rollback` restores only config and exchange data (`deploy.sh:284–304`, `deploy.bat:241–253`). `ai_data_$TIMESTAMP` and `audit_$TIMESTAMP` accumulate on disk every deploy and are never read by any path — a rollback silently drops the AI bot's SQLite/WAL signal/trade database while reporting `Rollback completed`. Two write-only backup streams in both scripts.
