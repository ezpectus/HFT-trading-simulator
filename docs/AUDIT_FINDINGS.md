...
**S302 (Info) — Makefile residue: `dev-exchange` is the 5th broken-from-inside site + 4 EOL `docker-compose` calls.** `Makefile:15` `dev-exchange` runs `cd exchange_simulator && python -m exchange_simulator` — the guaranteed ModuleNotFoundError already recorded for deploy.sh:148 (S215) and no-docker.* (S220); the documented dev target was born dead. `:12,:50,:53,:87` (`dev`/`docker-up`/`docker-down`/`docker-hub`) all call the EOL `docker-compose` v1 binary.
...
**S306 (Info) — `WEB_UI.md` points users at the deleted launchers, with a claim that was never true.** `docs/WEB_UI.md:498` — "Use `start.bat` (Windows) or `start.sh` (Linux/Mac) to launch all 4 services + 4 monitors in 8 terminal windows". Both files were deleted (CHANGELOG:62, "covered by no-docker.bat"), and the "8 terminal windows" figure was wrong even historically — `start.bat` opened 6. Third site of the deleted-launcher-pointer class: S305 covered `CONTRIBUTING.md:105,:241` + `install-deps.bat:140`; the adjacent `WEB_UI.md:495-496` monitor rows are S209's gitignored-components site — this line sat one paragraph below and survived every pass. The actual live paths (`no-docker.{bat,sh}`, Docker compose) are never named here.
