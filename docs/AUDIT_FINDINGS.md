...
**S310 (Medium) — 2 of 3 PodDisruptionBudgets select zero pods.** `pdb.yaml:26` selects `app.kubernetes.io/component: exchange-simulator` (hyphen) but the Deployment labels pods `exchange_simulator` (underscore — exchange-simulator.yaml:9). `pdb.yaml:43` selects `component: hft-trade-bot`, but hft is a **sidecar** inside the ai-signal-bot pod (ai-signal-bot.yaml:100-148) — no pod carries that label. Both PDBs render and look healthy in `kubectl get pdb` while protecting nothing — a fake disruption-budget guarantee. Only the ai-signal-bot PDB works (and accidentally covers the hft sidecar via pod membership). Compounding trap: `minAvailable: 1` on a 1-replica workload blocks drains forever.

**S311 (Medium) — image defaults are unreachable; a default `helm install` ImagePullBackOffs all four app images.** `values.yaml` uses `hft-exchange_simulator:v2.0.0`, `hft-ai-signal-bot:v2.0.0`, `hft-trade-bot:v2.0.0`, `hft-web-ui:v2.0.0` — but `deploy.yml:75` pushes `ghcr.io/ezpectus/hft-tradebot--lite-version/<service>` and `:129-131` strips the leading `v` (published tag `2.0.0`, not `v2.0.0`). Wrong registry namespace AND wrong tag format — no repo path ever produces these names. The chart has nice `fail` guards for wsExchange/wsSignals/adminPassword but nothing forces image overrides.

**S312 (Low) — ingress `/grafana` subpath is broken by construction.** `ingress.yaml:42-48` routes `/grafana` to grafana:3000, but grafana.yaml sets neither `GF_SERVER_ROOT_URL`/`serve_from_sub_path` nor a rewrite annotation — Grafana answers with root-path redirects/assets, so the subpath 404s/redirect-loops the first time someone enables `ingress.enabled`.
