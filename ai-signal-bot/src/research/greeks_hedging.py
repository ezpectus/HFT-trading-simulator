...
class GreeksHedgingSimulator:
    ...
    def print_summary(self, result: HedgeSimulationResult) -> None:
        ...
        logger.info("%s", '='*60)
        logger.info("Final P&L:          $%10.2f", result.final_pnl)
        logger.info("Option P&L:         $%10.2f", result.option_pnl)
        logger.info("Hedge P&L:          $%10.2f", result.hedging_pnl)
        logger.info("Transaction Costs:  $%10.2f", result.transaction_costs)
        logger.info("Rebalances:         %10d", result.n_rebalances)
        logger.info("Avg Hedge Error:    %10.4f", result.avg_hedge_error)
        logger.info("Max Hedge Error:    %10.4f", result.max_hedge_error)
        logger.info("%s", '-'*60)
        logger.info("P&L Decomposition:")
        for k, v in result.pnl_decomposition.items():
            logger.info("  %-20s $%10.2f", k, v)
        logger.info("%s", '='*60)
