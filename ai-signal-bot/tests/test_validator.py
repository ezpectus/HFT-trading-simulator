...
class TestSignalValidator:
    ...
    async def test_valid_signal_passes(self):
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
    ...
    @pytest.mark.asyncio
    async def test_low_rr_rejected(self):
    ...
    @pytest.mark.asyncio
    async def test_max_positions_rejected(self):
        await self.validator.update_position_count(3)
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert not result.passed
    ...
    @pytest.mark.asyncio
    async def test_neutral_rejected(self):
    ...
    @pytest.mark.asyncio
    async def test_drawdown_rejected(self):
        await self.validator.update_pnl(-900)  # 9% of 10000
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert not result.passed
...
class TestDuplicateCooldown:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_signal_within_cooldown_rejected(self):
        sig = make_signal(symbol="BTC/USDT")
        result1 = await self.validator.validate(sig, account_balance=10000)
        assert result1.passed
        # Same symbol immediately after → rejected
        sig2 = make_signal(symbol="BTC/USDT")
        result2 = await self.validator.validate(sig2, account_balance=10000)
        assert not result2.passed
        assert "Duplicate" in result2.reason or "cooldown" in result2.reason.lower()

    @pytest.mark.asyncio
    async def test_different_symbol_not_blocked_by_cooldown(self):
        sig1 = make_signal(symbol="BTC/USDT")
        await self.validator.validate(sig1, account_balance=10000)
        sig2 = make_signal(symbol="ETH/USDT")
        result2 = await self.validator.validate(sig2, account_balance=10000)
        assert result2.passed

    @pytest.mark.asyncio
    async def test_cooldown_expires_after_5_minutes(self):
        sig1 = make_signal(symbol="BTC/USDT")
        await self.validator.validate(sig1, account_balance=10000)
...
class TestResetDaily:
    ...
    @pytest.mark.asyncio
    async def test_reset_daily_clears_pnl(self):
        await self.validator.update_pnl(-500)
...
class TestUpdatePnl:
    ...
    @pytest.mark.asyncio
    async def test_positive_pnl_does_not_trigger_drawdown(self):
        await self.validator.update_pnl(500)  # Profit, not loss
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert result.passed

    @pytest.mark.asyncio
    async def test_pnl_accumulates(self):
        await self.validator.update_pnl(-300)
        await self.validator.update_pnl(-300)
    ...
    @pytest.mark.asyncio
    async def test_pnl_accumulates_to_drawdown(self):
        await self.validator.update_pnl(-400)
        await self.validator.update_pnl(-500)
...
class TestPositionCount:
    ...
    @pytest.mark.asyncio
    async def test_below_max_passes(self):
        await self.validator.update_position_count(2)
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert result.passed

    @pytest.mark.asyncio
    async def test_at_max_rejected(self):
        await self.validator.update_position_count(3)
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert not result.passed
    ...
    @pytest.mark.asyncio
    async def test_above_max_rejected(self):
        await self.validator.update_position_count(5)
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert not result.passed

    @pytest.mark.asyncio
    async def test_zero_positions_passes(self):
        await self.validator.update_position_count(0)
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert result.passed
...
class TestCustomConfig:
    ...
    @pytest.mark.asyncio
    async def test_custom_min_rr(self):
    ...
    @pytest.mark.asyncio
    async def test_custom_max_drawdown(self):
        validator = SignalValidator(max_drawdown_pct=5.0)
        await validator.update_pnl(-600)  # 6% of 10000
        sig = make_signal()
        result = await validator.validate(sig, account_balance=10000)
        assert not result.passed
        assert "drawdown" in result.reason.lower()
    ...
    @pytest.mark.asyncio
    async def test_custom_max_positions(self):
        validator = SignalValidator(max_open_positions=1)
        await validator.update_position_count(1)
        sig = make_signal()
        result = await validator.validate(sig, account_balance=10000)
        assert not result.passed
...
class TestValidationResult:
    ...
    @pytest.mark.asyncio
    async def test_result_has_signal_field(self):
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert result.signal is sig

    @pytest.mark.asyncio
    async def test_result_passed_true_for_valid(self):
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
        assert result.passed is True

    @pytest.mark.asyncio
    async def test_result_passed_false_for_invalid(self):
        sig = make_signal(confidence=10)
        result = await self.validator.validate(sig, account_balance=10000)
        assert result.passed is False

    @pytest.mark.asyncio
    async def test_result_reason_is_string(self):
        sig = make_signal()
        result = await self.validator.validate(sig, account_balance=10000)
...
class TestShortSignals:
    ...
    @pytest.mark.asyncio
    async def test_valid_short_signal_passes(self):
    ...
    @pytest.mark.asyncio
    async def test_short_low_rr_rejected(self):
        sig = make_signal(direction=SignalDirection.SHORT,
...
class TestZeroBalance:
    ...
    @pytest.mark.asyncio
    async def test_zero_balance_no_drawdown_division_error(self):
        await self.validator.update_pnl(-500)
        sig = make_signal()
        # Should not raise ZeroDivisionError
        result = await self.validator.validate(sig, account_balance=0)
