...
class TestTripAndRecovery:
    ...
    def test_auto_recovers_after_cooldown(self, cb):
        ...
        with patch('src.strategies.circuit_breaker.time.monotonic', return_value=cb._trip_time + 2.0):
            assert cb.is_tripped is False
        assert cb.consecutive_losses == 0
    ...
    def test_does_not_recover_before_cooldown(self, cb):
        ...
        with patch('src.strategies.circuit_breaker.time.monotonic', return_value=cb._trip_time + 0.5):
            assert cb.is_tripped is True
