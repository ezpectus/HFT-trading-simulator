...
> - `.cascade/personal-prompt.md` — компактный быстрый старт (~300 строк): цикл, портирование, анти-луп
> - `.cascade/prompts.md` (этот файл) — справочник: 100 ролей, качество, сценарии, делегирование
>
> **Файлы контекста:**
> - `.cascade/context_cache.md` — компактный контекст (20 строк, читай ВМЕСТО future_development.md)
...
- Функция ≤ 40 строк (мягкий лимит — 45 с ясной логикой ОК, >60 и сложная — рефактори)
...
- `CHANGELOG.md` — что изменено, почему
...
```
1. CEO: читает context_cache.md, выбирает следующую модель без ✅ DONE
...
```
1. CEO (01)       → читает context_cache.md, выбирает следующую модель
...
6. Code Reviewer (29) → read_file созданного кода, проверка качества, коммит
7. Tech Writer (41)→ обновляет MATH_MODELS.md, CHANGELOG.md, отмечает ✅ DONE
...
- office-board.md используется для коммуникации (макс 50 строк, шаблон в sprint_template.md)
- context_cache.md читается ВМЕСТО future_development.md (20 строк вместо 858)
...
> **Полные 10 шагов автономного режима — в `.cascade/personal-prompt.md` БЛОК 7.**
> Здесь — только краткая сводка и таблица.
...
### ПОЛНАЯ ТАБЛИЦА АВТОНОМНОГО ЦИКЛА

> Детальные 10 шагов — в `.cascade/personal-prompt.md` БЛОК 7.

```text
Ты — AI оркестратор для HFT Trading System.
Прочитай .cascade/personal-prompt.md и следуй ему.
Прочитай .cascade/context_cache.md — следующая модель уже определена.
Реализуй → тесты → коммит → ✅ DONE → следующая модель.
```
...
|------|-----|-----------|-----------|
| Контекст | CEO (01) | Читает context_cache.md, office-board.md | Какая модель следующая |
| Выбор | CTO (02) | Выбирает модель, определяет файлы | Задача на office-board.md |
| Разработка | Quant Dev (07) / ML Eng (09) | Реализует модель в Python | Код + тесты |
...
- **Если добавил фичу** — добавь в CHANGELOG, MATH_MODELS.md. README/ARCHITECTURE — только если новая структура проекта

---

*100 ролей. 20 отделов. IT-офис квант-трейдинг компании. РАЗРАБОТКА первична. Аудит вторичен. AI берёт следующую модель из context_cache.md, реализует, тестирует, коммитит, обновляет context_cache.md, идёт дальше. Если future_development.md завершён — САМ ПЛАНИРУЕТ новые задачи через code_search/grep_search/list_dir. ПРОГРЕСС, НЕ ЦИКЛ. НИКОГДА НЕ СТОП. future_development.md = главный драйвер. Никакого AI slop. Principal engineer спит спокойно.*
