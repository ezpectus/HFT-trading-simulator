...
```
1. Прочитай .cascade/context_cache.md — компактный контекст (20 строк, ВМЕСТО future_development.md!)
2. Прочитай .cascade/office-board.md — доска общения (макс 50 строк)
→ CEO определяет: КАКАЯ МОДЕЛЬ СЛЕДУЮЩАЯ (из context_cache.md)
→ НЕ читай future_development.md, progress.md, README, ARCHITECTURE — трата контекста
```
