...
> Сам находит проблемы, сам приоритизирует, сам исправляет, сам коммитит.

### АЛГОРИТМ АВТОНОМНОЙ РАБОТЫ — 7 ШАГОВ
...
**ШАГ 2: ПОИСК ПРОБЛЕМ — ГДЕ ИСКАТЬ**
```
1. grep_search "TODO" — все TODO комментарии в коде
2. grep_search "FIXME" — все FIXME
3. grep_search "HACK" — все костыли
4. grep_search "XXX" — все проблемные места
5. grep_search "NotImplementedError" — нереализованные функции
6. grep_search "pass  #" — пустые заглушки
7. grep_search "raise NotImplementedError" — заглушки
8. grep_search "type: ignore" — подавленные ошибки типизации
9. grep_search "noqa" — подавленные lint предупреждения
10. grep_search "assert " — assertions в production (должны быть tests)
11. list_dir по каждому модулю — найти пустые __init__.py
12. find_by_name "test_*" — найти модули без тестов
13. Прочитать .cascade/bug_log.md — неисправленные баги
14. Прочитать docs/9_DAY_DEVELOPMENT_PLAN.md — невыполненные пункты
```

**ШАГ 3: ПРИОРИТИЗАЦИЯ — ЧТО ДЕЛАТЬ ПЕРВЫМ**

| Приоритет | Что | Роль |
|-----------|-----|------|
| 🔴 P0 | Баги из bug_log.md (crash, data loss, money loss) | Bug Hunter (31) → Bug Fixer (32) |
| 🔴 P0 | NotImplementedError / пустые заглушки | Соответствующий разработчик |
| 🟠 P1 | FIXME / HACK — технический долг | Tech Debt (49) → Bug Fixer (32) |
| 🟠 P1 | Модули без тестов | QA (27) → Test Auto (28) |
| 🟡 P2 | TODO — незавершённые фичи | PM (05) → разработчик |
| 🟡 P2 | `type: ignore` / `noqa` — подавленные ошибки | Static Analyst (30) |
| 🟢 P3 | Документация — отсутствующие docstrings | Tech Writer (41) |
| 🟢 P3 | Архитектура — нарушения SRP, большие файлы | Refactoring (47) |
| 🔵 P4 | Performance — медленные функции | Performance (24) |
| 🔵 P4 | Улучшения из development plan | Tech Planner (45) |

**ШАГ 4: ФОРМИРОВАНИЕ СПРИНТА**
```
1. Выбери 3-5 задач из P0-P1 (не больше 5 за спринт)
2. Для каждой задачи определи роль по таблице выше
3. Запиши спринт в .cascade/progress.md:
   "СПРИНТ [дата]: [задача1] → [роль], [задача2] → [роль], ..."
4. Начинай с P0 — баги и заглушки первыми
```

**ШАГ 5: ВЫПОЛНЕНИЕ — КАЖДАЯ ЗАДАЧА**
```
...
7. Обнови документацию (CHANGELOG, progress, bug_log)
8. Коммит: git add -A; git commit -m "<type>: <description>"
9. Запиши результат в .cascade/progress.md
10. Делегируй следующей роли если нужно
```
...
4. Если P0 остались — следующий спринт с ними
5. Если P0 пуст — переходи к P1, P2, P3, P4
```

**ШАГ 7: ЦИКЛ — ПОВТОРЯЙ**
```
1. После каждого спринта — новое сканирование (ШАГ 2)
2. Новые TODO/FIXME могут появиться от предыдущих изменений
3. Новые баги могут быть найдены при рефакторинге
4. Продолжай пока есть задачи любого приоритета
5. Когда все задачи исчерпаны — сообщи пользователю:
   "Автономный режим завершён. Все найденные задачи выполнены.
    P0: 0 активных. P1: 0 активных. P2: 0 активных.
    Спринтов выполнено: N. Коммитов: N.
...
Ты сам сканируешь проект, находишь проблемы, приоритизируешь, исправляешь, коммитишь.

АЛГОРИТМ:
1. Прочитай контекст: bug_log.md, progress.md, notes.md, CHANGELOG.md
2. Сканируй код: grep TODO, FIXME, HACK, NotImplementedError, type: ignore
3. Приоритизируй: P0 (баги) → P1 (долг) → P2 (фичи) → P3 (docs) → P4 (perf)
4. Сформируй спринт: 3-5 задач
5. Выполни каждую: роль → план → код → тесты → docs → коммит
6. Проверь результаты
7. Повторяй пока есть задачи

ПРАВИЛА:
- Коммит после КАЖДОГО изменения
- 10 вопросов планирования перед кодом
- Тесты для каждой новой функции
- Обновляй CHANGELOG, progress, bug_log
- Не создавай новые проблемы — минимальный diff
- Если задача слишком крупная — разбей на подзадачи

ЗАДАЧА: АВТОНОМНЫЙ РЕЖИМ — найди и исправь все проблемы в проекте.
```

### ЧТО AI ДЕЛАЕТ АВТОНОМНО — ПРИМЕРЫ

| Сценарий | Что AI найдёт | Что AI сделает |
|----------|---------------|----------------|
| Пустая функция | `raise NotImplementedError` | Реализует, напишет тест, коммит |
| TODO в коде | `# TODO: add VaR calculation` | Реализует VaR, тест, docs, коммит |
| Модуль без тестов | `src/risk/var_calculator.py` без `test_var_*` | QA (27) напишет тесты, коммит |
| FIXME костыль | `# FIXME: this is wrong` | Bug Hunter (31) найдёт, Fixer (32) исправит |
| Большой файл | Файл > 500 строк | Refactoring (47) разобьёт на модули |
| `type: ignore` | `x: Any  # type: ignore` | Static Analyst (30) исправит типизацию |
| Баг из лога | `bug_log.md: "division by zero in VaR"` | Bug Fixer (32) исправит, тест, коммит |
| Нет docstring | Функция без документации | Tech Writer (41) добавит docstring |
| План не выполнен | `9_DAY_DEVELOPMENT_PLAN.md: Day 3 — не done` | PM (05) → разработчик → коммит |
...
- **Не рефактори** рабочие тесты — только добавляй новые
...
*100 ролей. 20 отделов. Один большой IT-офис квант-трейдинг компании. Каждый — специалист. Каждый — с правилами. Каждый — знает свои границы. Командная работа. Автономный режим — AI сам находит и решает задачи. Никакого AI slop. Principal engineer спит спокойно.*
