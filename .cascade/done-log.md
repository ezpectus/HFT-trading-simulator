...
- **Extended2 — ingest path:** (g) `update_prices`/`update_orderbook_deltas`/`update_candles`/`update_orderbooks`/`inject_snapshot` each built a fresh `ex|sym` map key per message under `data_lock_` — all five reuse `key_scratch_` (same serialization guarantee: every caller holds `data_lock_`); (h) `update_candles` ran `erase(begin, end-200)` — an O(n) memmove — on EVERY candle once over cap, in both `candle_history_` and `candles_by_id_` — now amortized: trim fires at >256, keeping ≥200 (no consumer depends on the exact 200 bound; vector storage retained so pointer/reference stability semantics unchanged between trims).
- **Extended3 — egress + dispatch:** (i) all 7 `OrderExecutor` sends wrapped the stack buffer in `std::string(buf,n)` — a heap alloc per order on the submission path — now pass `buf, len` to websocketpp's `send(void const*, size_t, ...)` overload; (j) `handle_market_data` ran `contains()`+`operator[]` per key — two object-map walks ×5 keys per message — now single `find()` per key, `timestamp` still lazy.
- **Extended4 — tick tail:** (k) `process_ai_signals` deep-copied the order book per AI signal via by-value `get_order_book` — now fills shared `ctx.ob_buf` (is_connected checked first, miss→cleared); (l) `update_risk_state` ran `gmtime`+`system_clock` every tick to catch a once-a-day rollover — now rechecked ≤1/min (boundary lands ≤60s late, keeps the limit active LONGER — safe direction); (m) `handle_market_data` flagged `has_new_data_`+notify BEFORE committing updates — consumers could observe pre-update state — now flags after all updates, matching `inject_snapshot_impl`; (n) `get_all_prices_into` still did `k.substr(pos+1)` temp per qualified key per tick — now `k.c_str()+pos+1` (zero-alloc char* key); (o) `poll_shm_market_data` re-took `data_lock_` per symbol per tick via `inject_snapshot` — now `inject_snapshots_scoped` batches the whole sweep under one lock (also makes the sweep an atomic cross-symbol snapshot); (p) `inject_snapshot` copied the whole OrderBook (vectors) into `order_books_` every inject — keyed entry now updated in place; (s) `run_v2_signal_loop` read `now_ns()` per symbol — hoisted above the loop (one tick = one timestamp; client_order_id still unique via `<sym>`); (t) `update_health_status` built the full HealthStatus + took health_mtx_ every tick for a polled endpoint — throttled to 1s.
- **Extended5 — Python feed side:** (q) `_broadcast_market_data` rebuilt AND re-encoded the full candles+books+accounts JSON **per client** — now `encoded_cache` keyed by (encoding, frozenset(subs)); identical variants encode once per tick (arb_data already had `_encoded_variants`, the main message didn't); (r) `_publish_shm_snapshot` sliced `buf[a:b]` + `struct.unpack(bytes(...))` per symbol per tick — now `struct.unpack_from`/`pack_into` directly on the mmap buffer, zero copies. Verified: sim suite 446/446 green.
- **Files:** `hft-trade-bot/src/core/bot_loop.cpp:283-316,420-428`; `src/communication/signal_receiver_data.h:45-61,114-131,185-191,248`; `src/communication/signal_receiver_handlers.h:98-113,140-149,201-221,227-247`; `src/execution/order_executor.h:68,179,237,268,297,344,364,379`; `src/position/position_manager.h:274,283`; `src/core/bot_loop.cpp:116-127,360-380`.
...
- **Files:** `web-ui/src/components/OptionsStrategySimulator.jsx:114-137,282-296`; `web-ui/src/components/IndicatorBuilder.jsx:61-63`; `web-ui/src/components/OpenInterestTracker.jsx:12`; `web-ui/src/components/performance/PerfAreaChart.jsx` (mapPoint/setData guard); `web-ui/src/components/Toast.jsx` (region pointer-events); `web-ui/e2e/trading.spec.js` (banner selector); new `src/test/{OptionsStrategySimulator,OpenInterestTracker,IndicatorBuilder}.test.jsx` (9 regression tests — spot=0 NaN case fails pre-fix).
- **Verified:** playwright **35/35** green (was 32/35); vitest 9/9 new + touched suites green; eslint+tsc clean via pre-commit gate. `5185eb2`.
...
- **Files:** `web-ui/src/components/BayesianStructuralTimeSeries.jsx:97-110,161-181,215`; `web-ui/src/components/GaussianProcessRegression.jsx:182-207,270`; `web-ui/src/components/IndicatorFormulaParser.jsx:148-151,211-217,233-238`; new `src/test/{BayesianStructuralTimeSeries,GaussianProcessRegression,IndicatorFormulaParser}.test.jsx` (10 tests — the SVG-NaN check failed pre-fix on the Kalman defect).
- **Verified:** vitest 10/10 new green (Kalman NaN gone after the math fix); eslint clean; e2e console-errors test green; `kalmanMath.js` audited — textbook-correct forms, BSTS was the only deviant. `974522d`.

### S366-ext — MultiLegOptions same-class NaN (folded into S366)
- Sweep of unguarded range-division `(v−min)/(max−min)` across all 118 SVG components: `MultiLegOptions` was the live site — `xScale` divided by `maxSpot − minSpot = 0.6·spot`; typing `0`/junk into the Spot input → `0/0`/`NaN·x` → NaN path in production (identical class to S365(a) in its sibling `OptionsStrategySimulator`). Fixed: `safeSpot` sanitized once in the memo (NaN/≤0 → flat domain + sanitized option prices — raw spot also leaked into `estimateOptionPrice`/`totalCost` text cells), scales clamp non-finite to flat fallbacks, x falls back to uniform index spacing.
- Audited-clean siblings: `CumulativeVolumeDelta`/`PnLAttributionChart`/`TradeHistory`/`EhlersSuperSmoother`/`WaveletDecomposition` (length gates ≥2 before `len−1` division), `OrderFlowHeatmap` (`Math.max(len−1,1)`), `TickSpeedAnomaly` (zMax const), `EmpiricalDynamicModeling` (libSize ≥20), ASI-orphan sweep (3 hits, all legal `return arr\n.chain()`), effect-cleanup sweep (0 leaks), fixed-overlay sweep (SessionReplay modal + App button — legit).
- **Files:** `web-ui/src/components/MultiLegOptions.jsx:45-51,61-77,104-118`; new `src/test/MultiLegOptions.test.jsx` (3 tests — spot=0 and spot='abc' produce finite geometry).
- **Verified:** full web-ui vitest **166 files / 1156 tests — all green**; eslint+tsc clean. `875f586`.
...
- **Bugs:**
  ...
  (b) `RecurrentNeuralNetwork` — TWO crash bugs: `LSTMCell.Wf` was allocated `initMatrix(4, concat)` (4 rows) but indexed `Wf[gate*hiddenSize + i]` (needs `4*hiddenSize` rows) → `TypeError` on the very first forward pass — the panel could never have rendered; and `lstmTrain` referenced `this.inputSize` inside a module-scope arrow (`this` === undefined in ESM) — the BPTT-through-time loop was dead code that would throw once reached. Fixed both: `initMatrix(4*hiddenSize, concat)`, `lstm.inputSize`.
  (c) `OrderFlowAbsorption` — destructured order-book levels as `[price, qty]` tuples (`[_p, q]`), but the wire format — and every other consumer (`OrderBook`, `OrderBookHeatmap`, `DepthChart`) — is `{price, quantity}` objects → `object is not iterable` TypeError whenever a real book was present → dead panel.
...
## R244 — interaction pass in mount sweep — S369

### S369 — interaction-triggered defects: 2 panels broken on click/change, invisible to mount-only coverage (High) ✅
- **Context:** the mount sweep proved rendering but never exercised handlers — a whole defect class (bad state writes, transposed math reached only via a control) stayed invisible. The sweep now fires `click` on every rendered `button` and `change` on every rendered number/text input (value `'1'`, components' own clamps apply), lets triggered effects settle, then re-asserts zero real console warnings and zero `NaN`. First run: 269/271 clean, 2 real interaction-only defects.
- **Bugs:**
  (a) `PortfolioOptLab` — `toggleAsset` stored a `Set` in `selected` (`setSelected(next)`) while every consumer (`chosen.includes`, `chosen.length`, `chosen.map`) treats it as an array → `chosen.includes is not a function` on the very first chip click → dead panel after one interaction. Fixed: `setSelected([...next])`.
  (b) `VariationalAutoencoder` — the decoder backprop in `trainStep` indexed `Wout` as `[hidden][input]` although it is allocated `[inputDim][hiddenDim]` (`initWeights(hiddenDim, inputDim)` returns `[out][in]`), and indexed `bout[i]` over `hiddenDim` though it is `inputDim`-long. Square defaults (window 8, hidden 8) masked both. Any asymmetric combo — `windowSize=4` → `Wout[4..7]` undefined → `TypeError` crash; `hiddenDim=4` → `Wout[i][j≥4]` undefined → NaN weights → NaN `reconErrors`/`anomalyThreshold` → `NaN` SVG `y1/y2/y` — user-reachable via two number inputs. Fixed: correct `dWout[j][i] = dxHat[j]·h2[i]` / `bout[j] -= lr·dxHat[j]` output-layer update (the bogus `dh2`/`z` term was dimensionally wrong anyway — `Wout` consumes `h2`, not `z`).
- **Files:** `web-ui/src/components/PortfolioOptLab.jsx:68`; `web-ui/src/components/VariationalAutoencoder.jsx:111-131`; `web-ui/src/test/panelsMount.test.jsx` (fireEvent click/change pass + settle + re-assert); new `src/test/variationalAutoencoder.test.jsx` (2 tests — asymmetric dims crash/NaN regression); `src/test/portfolioOptLab.test.jsx` +1 test (chip toggle).
- **Verified:** both defects reproduced pre-fix by the sweep (TypeError stack / NaN-attribute warnings); sweep **271/271** post-fix with interactions on; focused 9/9 green; full vitest **172 files / 1440 tests — all green**; eslint clean on all touched files.

## R245 — all-control-types interaction + second-tick update pass — S370

### S370 — numerical-instability defects surfaced only by interaction + data update (High) ✅
- **Context:** sweep extended two ways: (1) interactions now cover every control type — `button` clicks, `change '1'` on number/text/textarea, `click` on checkbox/radio, mid-range `change` on range, second-option `change` on select — fired twice so toggles exercise both directions and conditionally-revealed controls get covered; (2) a second-tick context re-render (`ctxTick` — one appended candle per series + fresh fill/signal, new array identities) drives the update path where chart `setData`/memo/effect re-runs live. First combined run: 269/271 clean, 2 real defects.
- **Bugs:**
  (a) `BurgersEquation` — the solver was documented as Lax-Friedrichs but implemented FTCS advection (unconditionally unstable); worse, adding an explicit Laplacian to LF's Nyquist-marginal average is `|G(π)| = 1+4d > 1` — verified by von Neumann: the sawtooth histogram IC is a pure Nyquist mode → exponential blowup (`umax 1 → 5.8e+35` in one step → `sub = 6.6e36` → UI thread hang → NaN paths). dt input also had no upper clamp. Fixed: operator splitting — conservative LF advection (flux `u²/2`, TVD under CFL) then a separate explicit diffusion substep, each under its own bound; `dt` clamped ≤0.5; `sub` capped 500 as a cost bound. Node-verified stable for ν∈{0,0.01,1} × dt∈{0.01,0.5} — u decays monotonically.
  (b) `RecurrentNeuralNetwork` — `lr` input had no upper clamp (`Math.max(0.0001, +v)`); `lr=1` diverged training within the visible epochs → weights→Inf→NaN → `predPrice.toFixed` wrote `NaN` into the DOM. Fixed: clamp `lr ≤ 0.1` + honest early-stop — a non-finite loss returns the pre-divergence history (y overflows while weights are still finite, so predictions stay finite).
- **Files:** `web-ui/src/components/BurgersEquation.jsx:18-22,40-84,208`; `web-ui/src/components/RecurrentNeuralNetwork.jsx:136-140,372`; `web-ui/src/test/panelsMount.test.jsx` (all-control interact()×2 + ctxTick re-render); new `src/test/BurgersEquation.test.jsx` (2 tests — the Δt=0.5 case hung pre-fix); `src/test/RecurrentNeuralNetwork.test.jsx` +1 test.
- **Verified:** both reproduced pre-fix by the sweep (NaN paths / infinite loop — the focused Burgers test timed out pre-fix, passes in 259ms post-fix); sweep **271/271**; focused 6/6; full vitest **173 files / 1443 tests — all green**; eslint clean.

## R246 — selection-switch + empty-context pass — S371

### S371 — hooks-order violation + missing rules-of-hooks gate (High) ✅
- **Context:** the sweep now re-renders each panel twice more after the interaction/tick phases: once with a different `selectedExchange`/`selectedSymbol` pair (`ctxAlt` — per-selection code paths: different candle series, books, price lookups) and once with a fully-empty `ctxEmpty` (the real pre-snapshot state — empty candles/prices/orderbooks/fills/signals). First run: 270/271 clean — `bayesian-predictor` threw `Rendered fewer hooks than expected`.
- **Bug:** `BayesianPricePredictor` — the `betaPath` `useMemo` sat **below** the `if (!data) return …` early return. Populated mount runs all hooks; a re-render where `selectCandles` yields <30 candles (symbol switch to an unlisted pair, empty feed, startup race) drops the memo → hook-count mismatch → React throws — this escapes the error boundary as a subtree unmount, worse than a broken panel.
- **Root-cause class fix:** the reason it shipped is that `eslint-plugin-react-hooks` was never wired into `eslint.config.js` — `rules-of-hooks` now runs as `error` over all of `src`, statically gating the whole conditional-hook class (verified non-vacuous on a seeded violation; real tree: 0 remaining).
- **Fix:** `betaPath` memo moved above the early return (it already self-guards `if (!data) return ''`).
- **Files:** `web-ui/src/components/BayesianPricePredictor.jsx:289-305`; `web-ui/src/test/panelsMount.test.jsx` (ctxAlt selection-switch + ctxEmpty degenerate re-render phases); `web-ui/eslint.config.js` + `package.json` (`eslint-plugin-react-hooks@6.1.1`, `rules-of-hooks: error`); new `src/test/bayesianPricePredictor.test.jsx` (2 tests — populated→empty→populated hook-count cycle).
- **Verified:** reproduced pre-fix by the sweep ("Rendered fewer hooks than expected"); sweep **271/271** post-fix across all five phases; focused 2/2 green; full vitest **174 files / 1445 tests — all green**; eslint clean including the new rule.

## R248 — hft ctest unblocked via clang-MinGW — S372

### S372 — vendored doctest was a 156-line stub; enabling execution surfaced 3 real defects (High) ✅
- **Context:** `tests/doctest.h` is a hand-rolled 156-line stub ("replace with the full doctest.h"), not doctest 2.4.11. Missing: `CHECK_THROWS_AS`/`CHECK_THROWS`/`CHECK_NOTHROW`/`REQUIRE_THROWS*`, `WARN`/`MESSAGE`/`INFO`/`CAPTURE`, `doctest::Context`; `SUBCASE` runs subcases sequentially without prefix re-run; `REQUIRE`==`CHECK`. `test_doctest_order_type_selector.cpp` uses `"doctest.h"` (always resolves to the stub) + `CHECK_THROWS_AS` → **the file never compiled anywhere — a dead-but-registered ctest target**. Host has LLVM-MinGW clang 22.1.8 + cmake 4.4 + ninja — the "MSVC-blocked" note only covered prebuilt ASan exes from the S:-drive machine.
- **Infrastructure fix:** stub gained the THROWS/NOTHROW family (honest semantics — typed catch, wrong-type and no-throw both fail); `CMakeLists.txt` gained `HFT_TESTS_ONLY` (deps QUIET, main exe skipped, each dep-requiring test gated on its `TARGET` existing — full hosts still build everything, dep-less hosts build the dep-free set). Also fixed a latent copy-paste: the signal_receiver section's FMT/SPDLOG_LIBRARY fallbacks linked `test_doctest_hft_config` instead of `test_doctest_signal_receiver`. `.gitignore` `hft-trade-bot/build/` → `build*/`.
- **Defects found by the first real execution:**
  (a) `InlineRSI` — flat series (gain=loss=0) fell into the no-losses branch → `rs=1e12` → **RSI≈100 on a dead tape** (should be 50 — no directional movement). Fixed: three-way — losses>0 → formula; only gains → 100; flat → 50.
  (b) `OnlineHMM` — `vol_proxy = sqrt(vol_ewma·252)` annualized the per-observation EWMA while all emission params are per-observation → vol channel mis-scaled ~16× → VOLATILE out-scored every state → TRENDING/RANGING detection was dead. Additionally RANGING's return variance (5e-5, σ≈0.7%) was so wide its mean-0 Gaussian absorbed trend drifts; tightened to 1e-7 (σ≈0.03%) — ranging now means sub-0.03% moves. Trending-up/down tests pass; `current_volatility()` keeps ×252 (reporting accessor, plausible intent).
  (c) `test_doctest_pressure_model` — fixture comment claimed "1bps spread" but `half_spread = mid·1e-4` produces 2bps, and TIGHT requires `<1.0` — rotten expectation. Fixture gained a `half_spread_frac` param; the TIGHT test now builds a real 0.4bps book and also asserts the default 2bps fixture → NORMAL (thresholds shared with `adaptive_order_selector_v2`).
- **Files:** `hft-trade-bot/tests/doctest.h:144-200`; `hft-trade-bot/src/strategies/inline_indicators.h:105-115`; `hft-trade-bot/src/strategies/signal_engine_v3.h:124-131,156-161`; `hft-trade-bot/tests/test_doctest_pressure_model.cpp:11-15,98-108`; `hft-trade-bot/CMakeLists.txt` (HFT_TESTS_ONLY + per-target gating); `.gitignore`.
- **Verified:** `cmake -B build-mingw -G Ninja -DHFT_TESTS_ONLY=ON -DCMAKE_CXX_COMPILER=clang++` → 26/26 build steps clean → **ctest 13/13 binaries, 221 test cases, all green** (was: never ran locally). Dep-gated targets documented: fmt/spdlog/yaml/json tests + POSIX-only shm/signal_flow remain dep-blocked, not env-blocked.

### R248-cont — stub replaced with real doctest 2.4.11
...
### S373 — 4 v2 test files dep-gated on fmt they never include + rotten assert (Medium) ✅
- **Context (corrected R251):** my R250 claim "never registered" was wrong — the quartet sits inside `if(TARGET fmt::fmt)` via a `foreach(v2test …)` variable expansion my literal grep couldn't see. They were dep-gated (conservatively — the files don't include fmt at all); vendoring deps in R251 lit the gate. They do need `src/strategies/signal_engine_v2.cpp` (out-of-line `SignalEngineV2Params::validate`) — already wired by the existing foreach.
- **Fix:** none needed for registration (existing gate lit once fmt resolved); R250's temporary duplicate block was removed on discovery.
...
### S374 — 5 test targets sat dep-gated; first execution found API-drifted + phantom-key fixtures (Medium) ✅
- **Context:** `test_doctest_kill_switch`, `test_doctest_hft_config`, `test_doctest_signal_receiver`, `test_integration_config`, `test_integration_kill_switch_monitor` were gated on fmt/spdlog/yaml/json — none ever ran. Vendored into gitignored `deps/` (reproducible via `hft-trade-bot/fetch-test-deps.sh`): nlohmann/json 3.11.3 single-header, spdlog 1.15.3 header-only (bundled fmt), fmt 11.1.4 headers (`FMT_HEADER_ONLY`), asio 1.30.2 standalone, yaml-cpp 0.8.0 prebuilt `libyaml-cpp.a` (clang+ninja). CMake tests-only mode creates the canonical `*::*` targets from `deps/` when real packages are absent — the `if(TARGET …)` gates light automatically. `signal_receiver` additionally needs real OpenSSL headers (asio TLS path parses them even unused) — stays gated on `deps/openssl/openssl/ssl.h`, documented in the gate + fetch script.
- **Defects found by enabling them:**
  (a) `test_integration_kill_switch_monitor` was written against a *different API entirely*: `KillSwitch(trigger, 0.5)` (ctor takes strings, poll ms is a `start_monitoring` arg), `monitor.get_counter(Counter::…)` (real API is `get(Metric::…)`), `snapshot()` treated as a map (real: struct), and expected auto-clear after file removal — but `activate(FILE_TRIGGER)` deletes the file itself and stays active until `deactivate()` (sticky by design). Rewritten to the real contract incl. a sticky-activation assertion.
  (b) `test_doctest_hft_config` + `test_integration_config` fixtures wrote `ws_url:`/`default:` keys the parser never reads (`websocket_url:`/`default_exchange:`) — every positive-load case failed validation; also meant the "missing ws_url fails validation" negative test was passing vacuously.
  (c) `test_signal_engine`'s `spectral_trend_score` fixture oscillated at freq 0.3 → period ~21 candles → spectral peak bin ~6, which is INSIDE the impl's low band (split at n_fft/8=16) — only cycles faster than ~8 candles are "cycle-dominated". Fixture gained a freq param; the test uses 1.0 (peak bin ~20 → high band). Impl verified correct, not changed.
...
## R253 — clang-tidy hot-path sweep (first static-analysis pass on hft src) — S376

### S376 — noexcept hot-path functions could std::terminate via bad_alloc; vacuous cooldown test (High) ✅
- **Context:** first clang-tidy 22.1.8 pass over the trading-loop TUs (proper sysroot via driver include query — compile_commands.json didn't carry implicit libc++ paths for clang-tidy). `bugprone-exception-escape` flagged `SignalEngineV2::analyze_incremental` (noexcept): its `get_cache` → `unordered_map::emplace(std::string(symbol), …)` allocates → `bad_alloc` under OOM → **std::terminate on the trading thread**. Same pattern found manually in `SignalEngineV3::analyze`/`analyze_incremental` via `get_or_create_hmm_state` (analyzer missed it — same class of lie in `noexcept`).
- **Fix:** noexcept `try_get_cache`/`try_get_hmm_state` return nullptr on alloc failure; `analyze_incremental` degrades to a neutral "Cache alloc failed" signal, v3 falls back to the V2 base signal (regime layer skipped) — the bot survives OOM in the map path. Public `get_cache` delegates and throws `bad_alloc` for non-noexcept callers. Prepended `#include <new>`.
- **Secondary fixes while in the file:** `set_long_signal` took a dead `now_ms` param (cooldown stamps in `finalize_signal`, asymmetric with `set_short_signal`) — dropped; `test_fixtures.h` helpers `static`→`inline` (per-TU copies + unused-function warnings gone).
- **Vacuous test found + fixed:** `test_doctest_signal_engine` "cooldown blocks consecutive signals" called `analyze()` — the *stateless* path that never consults cooldown — so `sig2==NEUTRAL` passed for the wrong reason. Rewritten on `analyze_incremental` with `FastSignal::now_ns()` base (tiny fake timestamps land "inside cooldown" vs `last_signal_ms=0` epoch — subtle trap), proven fixture, 3-phase assert: fires → blocked → fires after expiry.
- **Files:** `src/strategies/signal_engine_v2.h` (try_get_cache + noexcept-honest call), `src/strategies/signal_engine_v3.h` (try_get_hmm_state ×2 sites), `src/strategies/signal_engine_v2_finalize.h` (dead param), `tests/test_fixtures.h` (inline), `tests/test_doctest_signal_engine.cpp` (honest cooldown test).
- **Verified:** `bugprone-exception-escape` clean; build warnings 0 on touched TUs; **ctest 23/23 green**. Remaining clang-tidy output is stylistic noise (swappable-params on math helpers, enum-size on wire-layout ABI types — deliberately untouched: SHM wire structs are a cross-process byte contract).


## R254 — clang-tidy sweep #2: header-TU pass (SPSCQueue noexcept lie + IWYU) — S377

### S377 — SPSCQueue::push noexcept-lie on Signal; missing direct includes (Medium) ✅
- **Context:** second clang-tidy pass, this time over header TUs via wrapper .cpp files (headers alone parse as C → bogus diagnostics). Two real defects amid noise:
  1. `src/utils/low_latency.h` `SPSCQueue<Signal,16>::push(const T&)` declared `noexcept` but `buffer_[head] = item` runs `Signal`'s copy-assign — its `std::string symbol/exchange/reason` members can allocate → `bad_alloc` → **std::terminate** in the AI-signal handoff path (same class as S376). `push(T&&)`/`pop` are fine — `std::string` move-assign is noexcept. Fixed: `if constexpr (std::is_nothrow_copy_assignable_v<T>)` fast path + try/catch fallback returning false — OOM degrades to the caller's existing "queue didn't take it" drop path (`bot_setup.cpp:282/314` already handle false). Zero cost for trivially-assignable T; noexcept stays honest.
  2. `src/core/bot_setup.cpp` used `fmt::join` at :31 without `#include <fmt/ranges.h>` — compiled on vcpkg hosts only via spdlog's transitive includes; `-fsyntax-only` against the vendored header-only fmt proved it fails standalone → added the direct include.
- **Also in this pass:** `health_server.h` gained a direct `#include <spdlog/spdlog.h>` (it called `spdlog::info/error/warn` relying on consumer include order — same IWYU class as S350) and its HTTP response assembly was reworked off chained `+` temporaries (observability-plane cleanup, not hot path).
