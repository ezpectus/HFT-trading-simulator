...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 762 reliability findings (R1-R762: ... + exchange_factory GOOD (3 modes Protocol interface fallback health check runtime switching) + API keys plaintext MEDIUM + simulator hardcoded prices Low + db.py GOOD (WAL 3 tables 3 indexes parameterized queries Windows-safe close) + new connection per op MEDIUM + no timeout Low + no migration tracking Low + main.cpp GOOD (sequential init error checking comprehensive loop latency tracking graceful shutdown) + no SIGINT/SIGTERM handler MEDIUM + no exception handling MEDIUM + config.h GOOD (60+ fields defaults per-exchange IPC/SHM FIX DB Redis metrics risk limits) + API keys plaintext std::string MEDIUM + metrics_host 0.0.0.0 Low + bot_loop.cpp GOOD (SL/TP arb AI signals V2/V1 loops adaptive orders latency tracking status) + arb_lock not exception-safe Low + synthetic spread hardcoded Low + has_arb_opportunity after unlock Low + code reduction: duplicate OB synthesis ~10 lines). Updated CODE_AUDIT.md (§7+§8.1-8.772), RELIABILITY_PLAN.md (R1-R762), office-board.md (220 gap items), interview-prep.md (75 bad vs good code examples). Code reduction ~8260+ lines | ✅ Done | — |
