...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 898 reliability findings (R1-R898: ... + signal_engine_v3 EXCELLENT (OnlineHMM 4-state log-space regime gating per-symbol HMM) + get_or_create_hmm_state heap alloc noexcept MEDIUM + adapt_parameters only means Low + market_making_v2 EXCELLENT (Avellaneda-Stoikov inventory skew adverse selection no heap) + t_remaining always T Low + no per-symbol state MEDIUM + simd_indicators GOOD (AVX2 EMA/RSI/SMA/VWAP scalar fallback) + ema_array vector Low + has_avx2 compile-time Low + obi_utils EXCELLENT (3 OBI functions single-pass noexcept no heap) + fix_client GOOD (FIX 4.4 persistent seq nums checksum verify gap recovery) + password plaintext debug MEDIUM + seq file tempfile Low + no reconnect Low + _pending_messages unbounded Low + ws_client GOOD (optional msgpack/orjson compression reconnect trading state) + no reconnect on listen exit Low + ws_connection_pool GOOD (pool max 10 stale eviction health checks asyncio.Lock) + fire-and-forget tasks Low + _health_loop no error handling Low + simd hsum 2× Info ~10 lines). Updated CODE_AUDIT.md (§7+§8.1-8.908), RELIABILITY_PLAN.md (R1-R898), office-board.md (242 gap items), interview-prep.md (83 bad vs good code examples). Code reduction ~8655+ lines | ✅ Done | — |
