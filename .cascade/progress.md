...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 623 reliability findings (R1-R623: ... + inline_indicators EXCELLENT (5 streaming, O(1), Wilder's, noexcept, constexpr, transparent hash) + no period validation Low + obi_utils EXCELLENT (3 functions, single-pass, proximity weighting, noexcept) + IExchange EXCELLENT (11 pure virtual, DIP/SOLID, latency, toxic) + R593 CORRECTION: latency exists in interface Info + price_predictor GOOD (LSTM+Transformer, attention, 11 features, ONNX, early stop) + hard-imports torch Low + model_registry EXCELLENT (5 statuses, semver, A/B testing, rollback, file persistence) + not thread-safe Low + _save not atomic Low + db.py GOOD (3 tables, 3 indexes, WAL, parameterized, Windows-safe) + new connection per operation MEDIUM + no FK on signal_id Low + risk_parity GOOD (marginal risk, equal risk contribution, budgeting) + portfolio_return hardcoded 0 Low + rebalancing GOOD (3 triggers, turnover, skip threshold, transaction cost) + no min trade size Low). Updated CODE_AUDIT.md (§7+§8.1-8.633), RELIABILITY_PLAN.md (R1-R623), office-board.md (198 gap items), interview-prep.md (67 bad vs good code examples). Code reduction ~8000+ lines | ✅ Done | — |
