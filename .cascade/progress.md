...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 796 reliability findings (R1-R796: ... + signal_engine_v2 EXCELLENT (6 indicators no heap alloc incremental cache cooldown composite adaptive SL/TP branchless) + heap alloc in get_cache MEDIUM + stack arrays 8KB Low + cooldown not per-symbol MEDIUM + pressure_model EXCELLENT (multi-level OBI weighted OBI trade flow toxicity microprice queue pos price impact) + compute_obi dead code Info + position_manager GOOD (mutex update-on-duplicate SL/TP active_symbols) + linear search Low + no qty validation Low + signal.h GOOD (9 fields convenience methods R:R) + NEUTRAL side() returns BUY Low + code reduction: position_manager V1 vs V2 ~130 lines + pressure_model dead compute_obi ~10 lines). Updated CODE_AUDIT.md (§7+§8.1-8.806), RELIABILITY_PLAN.md (R1-R796), office-board.md (229 gap items), interview-prep.md (77 bad vs good code examples). Code reduction ~8420+ lines | ✅ Done | — |
