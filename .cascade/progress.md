...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 811 reliability findings (R1-R811: ... + signal_engine_v3 EXCELLENT (HMM 4-state regime detection online learning log-space per-symbol regime gating no heap alloc) + heap alloc in get_or_create_hmm_state MEDIUM + trans_sum 128B stack Low + manual string ops Low + mean_reversion_v2 EXCELLENT (Kalman filter OU estimation z-score half-life 6 actions ring buffer alignas(64) no heap) + no per-symbol state MEDIUM + 32KB stack per instance Low + socket_transport GOOD (non-blocking UDP 1MB buffer 127.0.0.1 binary parsing packet stats) + start_receive_loop blocks thread MEDIUM + no packet validation Low + notifier GOOD (Telegram+Discord 6 event types remote commands chat ID validation graceful stop) + bot token plaintext MEDIUM + no rate limiting MEDIUM + no retry Low + code reduction: duplicate emoji_map ~8 lines). Updated CODE_AUDIT.md (§7+§8.1-8.821), RELIABILITY_PLAN.md (R1-R811), office-board.md (235 gap items), interview-prep.md (78 bad vs good code examples). Code reduction ~8438+ lines | ✅ Done | — |
