...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 349 reliability findings (R1-R349: ... + markowitz GOOD (efficient frontier, scipy, div-by-zero guard) + 5× PortfolioOptimizer duplication HIGH + rebalancing GOOD (3 triggers, turnover) + exchange_factory EXCELLENT (Protocol adapter, 3 modes, lazy imports) + real_exchange_client GOOD (3 exchanges, HMAC-SHA256, usedforsecurity=False, testnet) + api_key as attr + model_registry EXCELLENT (5 statuses, A/B testing, rollback, file persistence) + no file lock + feature_store GOOD (Redis, in-memory fallback, TTL, batch) + catch-all Exception + price_predictor GOOD (LSTM+Transformer, ONNX export) + technical_analysis 25 files HIGH + indicators GOOD (8 indicators, NumPy optional) + alerting GOOD (3 severity, 3 channels, rate limiting, parallel) + list slice not deque + aiohttp session leak MEDIUM + health_server GOOD (4 endpoints, registerable) + 4× health check MEDIUM + backtester GOOD (candle replay, RiskManager) + pnl_calculator EXCELLENT (3 asset types, PnLBreakdown, DI) + 60-file TA+research overlap HIGH + dual metrics MEDIUM). Updated CODE_AUDIT.md (§7+§8.1-8.359), RELIABILITY_PLAN.md (R1-R349), office-board.md (156 gap items), interview-prep.md (48 bad vs good code examples + reliability talking points). Code reduction ~7000+ lines (incl. 60 TA+research modules + 22× compute_returns + 5× PortfolioOptimizer + 3× CircuitBreaker + 3 engines + duplicate risk + 3x CircuitBreaker + tracing + RateLimiter + web-ui dead components + 3 adapter duplication + App.jsx extraction + NETWORK_BPF dead code + EMA/RSI duplication + 200+ math panels feature flag + registry import generation + triple metrics consolidation + dual health consolidation + dual metrics consolidation + 4× health consolidation) | ✅ Done | — |
