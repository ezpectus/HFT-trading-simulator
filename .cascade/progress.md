...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 587 reliability findings (R1-R587: ... + bot_loop.cpp GOOD (8 functions: SL/TP, arb, AI signals, V2/V1, SHM poll, shutdown) + arb atomic set without lock Low + hardcoded 0.5 max arb qty Low + bot_setup.cpp EXCELLENT (SIGINT+SIGTERM handlers, thread pinning, 15-param risk init) + R518 CORRECTION: SIGTERM handler EXISTS in bot_setup.cpp Info + signal_handler only sets flag Low + strategies.py GOOD (3 strategies, EnsembleVoter, NaN guards) + noqa E402 Low + statistical_arbitrage EXCELLENT (OLS+ADF+half-life+Kalman+z-score+correlation) + market_making EXCELLENT (Avellaneda-Stoikov, inventory skew, adverse selection) + inventory not thread-safe Low + sentiment GOOD (10 event types, sentiment/volatility maps, decay) + ml_ensemble EXCELLENT (3 ML libs, HMM regime, IsolationForest, walk-forward) + HMMRegimeDetector not thread-safe Low + indicators GOOD (8 indicators, NumPy+scalar fallback, NaN-padded) + EMA not fully vectorized Low + risk_manager.py GOOD (4 features, ATR-based, 12-field state) + not thread-safe MEDIUM + no config validation Low). Updated CODE_AUDIT.md (§7+§8.1-8.597), RELIABILITY_PLAN.md (R1-R587), office-board.md (198 gap items), interview-prep.md (66 bad vs good code examples). Code reduction ~8000+ lines | ✅ Done | — |
