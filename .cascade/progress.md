...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 545 reliability findings (R1-R545: ... + bot_context GOOD (24 includes, SimExchange adapter, SymbolEntry, ArbOpportunity) + SimExchange holds reference Low + bot_loop GOOD (7 functions, header/impl separation) + types.h GOOD (3 enums, inline helpers, Candle, OrderBook) + string_to_side defaults to BUY Low + signal.h GOOD (10 fields, is_long/short/actionable, side, rr_ratio) + NEUTRAL side() returns BUY Low + aligned_types EXCELLENT (alignas64, static_assert, FastSignal fixed-size, 7 scores) + symbol_map GOOD (FNV-1a constexpr, bidirectional, uint16_t IDs) + get_id allocates string Low + risk_manager EXCELLENT (V1+V2 params, blacklist, per-symbol, 8 codes) + pre_trade_risk EXCELLENT (TokenBucket lock-free CAS, noexcept, relaxed) + TokenBucket refill race Low + portfolio_risk EXCELLENT (DrawdownTracker, VaR hist+param, CVaR, stress, zero-alloc) + exchange_advanced_orders GOOD (3 types, trigger logic, safe removal) + exchange_liquidation GOOD (3 triggers, leverage-aware, PnL before check) + hardcoded 0.005 margin Low + options_pricing GOOD (Black-Scholes, 5 Greeks, guards) + duplicate of options_simulator MEDIUM + price_feed_manager GOOD (multi-API, TTLCache, failover) + hard-imports msgpack Low + ws_metrics GOOD (deque maxlen, compression/delta, P95) + sorted() per query Low + visualizer GOOD (2 mixins, cross-platform, ANSI, tabbed) + strategies/__init__ GOOD (7 strategies, 4 configs, __all__) + missing CrossExchangeArb Low). Updated CODE_AUDIT.md (§7+§8.1-8.555), RELIABILITY_PLAN.md (R1-R545), office-board.md (195 gap items), interview-prep.md (64 bad vs good code examples). Code reduction ~8000+ lines | ✅ Done | — |
