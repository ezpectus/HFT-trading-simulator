...
| 66 | 2026-08-23 | Пачка AI — C++ safety: order_executor.h: replaced detached reconnect thread with member reconnect_thread_ joined in disconnect() (§8.117/452/774). health_server.h: stored server socket as member, closed in stop() to unblock accept() (§8.126). config.h: added clear_secrets() to zero API keys/db_dsn/redis_url, called in graceful_shutdown (§8.766). BinanceAdapter.h: added clear_secrets() to Config (§8.778). CODE_AUDIT §§8.117, 8.126, 8.452, 8.766, 8.774, 8.778 marked [FIXED]. Files: 7 changed (4 source + 3 docs) | ✅ Done | — |
