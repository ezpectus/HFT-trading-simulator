...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 289 reliability findings (R1-R289: race condition, DB timeout/pool/indexes/retention, socket buffers, SQL clean, deque clean, C++ correct + catch-all + smart pointers + memory ordering correct + signal handling good + FIX seq persistence good + kill switch excellent + position_manager_v2 good + order_executor detached thread risk + snprintf truncation + smart_order_router excellent + health_server accept blocking + health_server raw POSIX good + low_latency.h excellent + aligned_types excellent + IExchange DIP/SOLID + bot_context God struct + SPSCQueue+mutex mismatch + bot_loop clean + risk_manager dual V1+V2 good + check_order mutex + daily_pnl += race + pre_trade_risk token bucket excellent + blacklist not thread-safe + portfolio_risk VaR/CVaR good + simd_indicators AVX2 excellent + signal_receiver good + signal_engine_v2 6-indicator excellent + signal_engine_v3 HMM regime excellent + market_making_v2 Avellaneda-Stoikov excellent + shm_ring_buffer cross-process SPSC excellent + shm_heartbeat seq-guarded good + 3 signal engines version sprawl + adaptive_order_selector_v2 excellent + mean_reversion_v2 OU+Kalman excellent + stat_arb_v2 cointegration excellent + momentum_breakout_v2 good + inline_indicators O(1) excellent + system_monitor 11 atomic excellent + snprintf done right + types.h good + string_to_side silent default + pressure_model excellent + obi_utils good + signal.h NEUTRAL→BUY + ExchangeBase EWMA excellent + BinanceAdapter good + nested Spinlock + can_send_order TOCTOU + OKXAdapter good + BybitAdapter good + 3 adapters code duplication + binance_config constexpr excellent + web-ui setInterval cleanup good + 50+ components code reduction + App.jsx lazy loading good + Zustand store sync good + App.jsx God component + ExchangeContext good + usePerformance excellent + 4 Zustand stores good + usePanelContext bridge good + useToastStore setTimeout no cleanup + useUIStore 50 symbols duplicated + getFilteredSymbols not memoized + Helm values.yaml hardcoded passwords + resource limits good + VITE_WS localhost + ingress disabled good + shared_config localhost + 50 symbols good + hardcoded toxic threshold + Alertmanager hardcoded SMTP password + inhibition rules good + no silence rules + signal_publisher 6 catch-all + aiohttp session leak + alerts.yml 12 rules excellent + no HFT-specific alerts + ebpf_monitor good + NETWORK_BPF dead code + no Prometheus export + performanceMonitor good + metricsHistory unbounded + console.log in prod + Grafana 5 dashboards good + alertCallbacks unbounded + no SHM overflow alert + no drawdown alert + backtestEngine good + EMA/RSI duplicated + no borrow fee + no slippage + indicators excellent + O(n²) SMA + auditExport good + mockData good + only 5 of 50 symbols + indicators 579 lines + vite.config excellent + no esbuild.drop + hft-trade-bot config 50 symbols 3x + config localhost WS + config well-structured + PanelContainer good + registry 200+ lazy panels good + 200+ math panels reduction candidate + e2e Playwright good + no WS e2e tests + useExchangeData good + candle sort every update + useMockData good + useDetachablePanels good + no channel cleanup + db.py good + new connection per op + no data retention + no equity_curve index + no migration system + useWebSocket excellent + no max reconnect + console.error in prod + config_validator excellent + liquidation_engine_v2 excellent + ADL stub + fixed seed RNG + f-string logging + no thread safety + arbitrage excellent + unbounded closed_history + funding_rate excellent + f-string logging + latency_simulation excellent + market_microstructure excellent + spread_analytics good + order_book_realism excellent + recent_fills unbounded + options_simulator excellent + data_export good + __main__ clean + all modules seed=42 + health_checks excellent + no liveness depth + __import__ anti-pattern + logging excellent + tracing excellent + f-string logging + localhost endpoint + notifier good + token in URL + no rate limiting + no retry + llm_engine good + cache unbounded + no LLM response validation + f-string logging, aiohttp session leak (alerting + health check), Docker TCP healthchecks + Dockerfile TCP + Dockerfile.prod good, Helm probes missing + no PDB + no NetworkPolicy + hardcoded PG password + helpers good + Chart.yaml good + values.yaml hardcoded passwords + resource limits good + VITE_WS localhost + ingress disabled good, ErrorBoundary per-panel but no top-level, magic numbers, CORS, PropTypes disabled in ESLint, env secrets clean, docker secrets clean, no log rotation, float precision, no WS validation + no WS input schema, no auth, dep pinning good, Rust unwrap/expect/idempotency/string matching/panic=abort + FFI null safety good + 22 tests good, exchange_simulator clean + config_validator excellent + liquidation_engine_v2 excellent + arbitrage excellent + funding_rate excellent + latency_simulation excellent + market_microstructure excellent + spread_analytics good + order_book_realism excellent + options_simulator excellent + data_export good + __main__ clean + all modules seed=42 + global singletons, no network timeout config, Prometheus HFT path, no HFT alerts, CI/CD excellent + CI workflow excellent + deploy.yml excellent + codeql excellent, npm audit non-blocking, no config schema, Dockerfile security good, Docker Compose dev no limits + prod/staging has limits + prod excellent (mandatory secrets, network segmentation, SHM IPC) + hub pre-built good + dev Grafana admin/admin, Terraform hardcoded RDS password + S3 backend encryption+locking good + modular good, dead code tracing.py + RateLimiter + dpdk_transport.py source missing + NETWORK_BPF dead code, test coverage gaps, monitoring tests good, **no graceful shutdown (High)**, no WS keepalive, no backoff with jitter, 3x CircuitBreaker duplication + duplicate risk system + 3 signal engines + 3 exchange adapters duplication + EMA/RSI duplication in backtestEngine + 50 symbols 3x duplication, no asyncio task management, health check no depth + HealthChecker exists but not wired + liveness always alive, no retry on transient failures, code reduction ~4440 lines (incl. 3 engines + duplicate risk + 3x CircuitBreaker + tracing + RateLimiter + web-ui dead components + 3 adapter duplication + App.jsx extraction + NETWORK_BPF dead code + EMA/RSI duplication + 200+ math panels feature flag + registry import generation), f-string logging (7 modules), SHM no cleanup on crash + stale data on restart, SHM 1ms polling, dual metrics systems, no asyncio.Lock on shared state, DB migrations no runner + Makefile.prod runner exists but not idempotent + migrate.py idempotent but not used + db.py _init_db no migration, Alertmanager placeholder credentials + hardcoded SMTP password + no silence rules, shared_config + hft-trade-bot config + .env.prod + docker-compose.prod + Helm values + shared_config hardcoded localhost/passwords, Grafana 5 dashboards good + ebpf good but no Prometheus export + alerts 12 rules but no HFT-specific, Alertmanager inhibition rules good, CMake excellent, Cargo good, pre-commit good, Makefile no C++ tests + Makefile.prod good, deploy.sh no health exit + rm -rf + no retention + deploy.yml health no exit, Vite no CSP + no esbuild.drop, config.prod excellent, testnet good, dependabot excellent, SECURITY.md good but inaccurate WS claim, staging good, notifier good but token in URL + no rate limit + no retry, web-ui useWebSocket excellent + useTradingStore good + ExchangeContext good + usePerformance excellent + App.jsx lazy+Zustand good but God component + 4 Zustand stores clean + performanceMonitor good but unbounded+console.log + backtestEngine good but no borrow/slippage + indicators excellent but O(n²) SMA + auditExport good + mockData good but 5 symbols + PanelContainer good + registry 200+ lazy good but 684 lines + 200+ math panels + e2e good but no WS tests + useExchangeData good but candle sort every update + useMockData good + useDetachablePanels good but no channel cleanup, .env.prod placeholder passwords, build-all.bat good, CONTRIBUTING good, verify.bat good, reset_daily incomplete, snapshot not atomic, NEUTRAL→BUY silent default, nested Spinlock, can_send_order TOCTOU, hardcoded toxic threshold, 50 symbols 3x duplicated, getFilteredSymbols not memoized, alertCallbacks leak, metricsHistory unbounded, no SHM overflow alert, no drawdown alert, no borrow fee, no slippage, O(n²) SMA/Bollinger, mockData 5 symbols, config localhost WS, no esbuild.drop, 200+ math panels, no WS e2e tests, db.py new connection per op, no equity_curve index, no migration system, candle sort every update, BroadcastChannel no cleanup, useWebSocket no max reconnect, liquidation_engine ADL stub, no thread safety, arbitrage unbounded closed_history, order_book recent_fills unbounded, all modules seed=42, health_checks no liveness depth, notifier token in URL, llm_engine no response validation, tracing localhost endpoint). Updated CODE_AUDIT.md (§7+§8.1-8.301), RELIABILITY_PLAN.md (R1-R289), office-board.md (139 gap items), interview-prep.md (48 bad vs good code examples + reliability talking points) | ✅ Done | — |
