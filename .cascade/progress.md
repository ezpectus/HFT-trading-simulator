...
| — | 2026-08-22 | Full project grep audit — 26 grep findings + 859 reliability findings (R1-R859: ... + aligned_types EXCELLENT (5 aligned structs static_assert no heap alloc cache-line padding FastSignal FastOrder PressureResult RoutingDecision) + set_symbol/reason repeated 5× Info ~25 lines + confidence uint8_t precision Low + types GOOD (Candle OrderBook Order Position 3 enums) + string_to_side defaults SELL Low + Order ms vs FastOrder ns Low + symbol_map GOOD (FNV-1a hash runtime+compile-time bidirectional) + PerfectSymbolMap collision O(N) Low + get_id allocates string Low + tracing GOOD (OpenTelemetry+Jaeger noop fallback idempotent) + global singleton not thread-safe Low + logging EXCELLENT (structlog correlation IDs console/JSON contextvars library noise) + _configured not thread-safe Low + code reduction: copy_str 5× ~25 lines). Updated CODE_AUDIT.md (§7+§8.1-8.869), RELIABILITY_PLAN.md (R1-R859), office-board.md (238 gap items), interview-prep.md (81 bad vs good code examples). Code reduction ~8555+ lines | ✅ Done | — |
