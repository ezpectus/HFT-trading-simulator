...
Высшие чины проводят аудит, распределяют задачи.
Технические роли выполняют. Документация обновляется. Лишнее удаляется.
...
ШАГ 1: СБОР КОНТЕКСТА — CEO (01)
  Прочитай через read_file:
  1. .cascade/bug_log.md — известные баги
  2. .cascade/progress.md — что уже сделано
  3. .cascade/notes.md — контекст и TODO
  4. CHANGELOG.md — последние изменения
  5. docs/9_DAY_DEVELOPMENT_PLAN.md — план разработки
  6. .cascade/file_tracker.md — какие файлы просмотрены
...
ШАГ 2: ТЕХНИЧЕСКИЙ АУДИТ — CTO (02) + Principal (03)
  ...
  1. list_dir по каждому модулю — структура соответствует документации?
  2. grep_search "import" — есть ли circular dependencies?
  3. find_by_name "*.py" → read_file для проверки размеров
  4. Проверь: есть ли модули без __init__.py?
  5. Архитектурные нарушения → записать в .cascade/bug_log.md

  Principal (03) — quality аудит (ИСПОЛЬЗУЙ БЛОК 3 — COMMON SENSE):
  1. grep_search "TODO" → прочитай контекст, отфильтруй ложные
  2. grep_search "FIXME" → реальные FIXME
  3. grep_search "HACK" → реальные костыли
  4. grep_search "except:" → bare except в бизнес-логике (не CLI!)
  5. grep_search "except Exception" → широкий catch в бизнес-логике
  6. grep_search "from .* import \*" → star imports
  7. grep_search "print(" → проверь файл, CLI = OK, src/ = P1
  8. grep_search "global " → проверь, singleton = OK, mutable = P1
  9. grep_search "pass$" → пустые тела (проверь контекст!)
  → Все РЕАЛЬНЫЕ нарушения → .cascade/bug_log.md с P-уровнем
  → НЕ записывай ложные срабатывания (см. БЛОК 3)
...
ШАГ 3: АУДИТ ТЕСТОВ — QA (27)
  ...
  6. Модули без тестов → .cascade/bug_log.md как P1
...
ШАГ 4: АУДИТ ДОКУМЕНТАЦИИ — Tech Writer (41) + Audit (43)
  ...
  5. Проверь: есть ли docstrings во всех публичных функциях?

  Audit (43):
  6. Сравни docs/ARCHITECTURE.md с реальной структурой кода
  7. Сравни README.md с реальными возможностями
  8. Найди: что описано но не реализовано
  9. Найди: что реализовано но не описано
  → Отчёт: "Документация: N актуальна, M устарела, K отсутствует"

ШАГ 5: РАСПРЕДЕЛЕНИЕ ЗАДАЧ — VP Eng (04) + Eng Manager (53)
  VP Eng (04) — приоритизация:
  P0 — баги (crash, data loss, NameError, TypeError в production)
  P0 — NotImplementedError заглушки (не в except-блоках!)
  P1 — FIXME/HACK, bare except в бизнес-логике, import *
  P1 — модули без тестов, устаревшая docs
  P2 — TODO, type: ignore, noqa, большие файлы (>600 строк)
  P3 — архитектура (SRP нарушения), performance
  P4 — улучшения из development plan
  ...
  5. ЕСЛИ НЕТ P0-P2 ЗАДАЧ → СТОП, ФИНАЛЬНЫЙ ОТЧЁТ
...
ШАГ 10: ЦИКЛ ИЛИ ЗАВЕРШЕНИЕ
  1. Если есть задачи P0-P2 → следующий спринт (ШАГ 5)
  2. ЕСЛИ АУДИТ ЧИСТ 2 ЦИКЛА ПОДРЯД → СТОП
     Не придумывай проблемы. Не рефактори ради рефакторинга.
     "Всё чисто" = нормальный результат.
  3. Финальный отчёт: спринтов N, коммитов N, багов N
