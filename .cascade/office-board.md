...
| Всего находок | ~163 (S001–S163) |
| Закрыто | 147 |
| Открыто | **16** — 7 из R71 + 4 из R72 + 3 из R73 + 2 из R74 |

**Текущее состояние:** R74 (docs-vs-reality: WEBSOCKET_PROTOCOL.md field-level sweep + monitoring) нашёл **2 открытых** — S162–S163. Главное: протокол-док :8765 врёт в 7 местах — `config_update` не существует (реальный `update_config` + другая схема), `position`/`speed_change` — фиктивные broadcast'ы, `fills_batch.fills` → реально `orders` (S162); 6 из 8 latency-панелей Grafana кверят голое имя гистограммы без `_bucket` — вечно пустые (S163).
...
6. **S162** (Medium) — протокол-док :8765 врёт в 7 местах. Fix: переписать §8765 по диспетчеру — `update_config`+`updates`-схема, убрать `position`/`speed_change`/`error.code`, `fills`→`orders`, `server_name`→`server`, дописать `start_trading`/`stop_trading` и 5 candles-полей.
