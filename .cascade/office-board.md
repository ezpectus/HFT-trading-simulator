...
| Закрыто | 151 |
| Открыто | **18** — 5 из R71 + 2 из R72 + 3 из R73 + 2 из R74 + 2 из R75 + 3 из R76 + 1 из R77 |

**Текущее состояние:** R77 fix-раунд закрыл **4 High** (S148 kill-switch cancel, S149 client_order_id dedup, S155 sim control-plane auth, S156 container bind). Попутно починен скрытый sibling-дефект: покоящие LIMIT-ордера никогда не филлились (не было `_pending_limits` dict) — теперь трекатся и исполняются по limit-цене. Новые сообщения протокола: `auth`/`auth_ok`/`auth_failed`, `cancel_order`/`cancel_all_orders`, `order_cancelled`/`orders_cancelled`, `deduplicated`-флаг — всё задокументировано. Prod: `EXCHANGE_CONTROL_TOKEN`+`VITE_EXCHANGE_TOKEN` required; `EXCHANGE_WS_HOST=0.0.0.0` во всех compose. C++ изменения inspection-verified (тулчейна нет, CI скомпилит).
...
4. **S159** (Medium) — ~16 мёртвых конфиг-ключей в 3 сервисах. Fix-порядок: sim `metrics.*` — либо честный gate + отдельный `health.enabled`, либо удалить ключи (healthcheck зависит); `account.currency`/`exchanges.*.symbols` — пробросить в `SimulatedExchange` или дропнуть; hft `fft_*`/`fast_ema_enabled`/`obi_levels`-drift — свести к одному написанию и завести в `EngineParams`.
5. **S162** (Medium) — протокол-док :8765 врёт в 8 местах. (Doc поправлен в R74 + R77 дописал новые типы; строка остаётся контрольной точкой для slop-verify.)
6. **S167** (Medium) — 5 math-тестов тестируют inline-копии, не продакшен. Fix: вынести чистую математику в `src/utils/math/` и импортировать из обеих сторон, или удалить shadow-файлы; плюс S166 — заменить 19 `expect(true)` реальными рендер-assert'ами или удалить placeholder-тесты.
7. Периодически — `/slop-verify`: QA-проверка записей done-log по файлам/строкам.
