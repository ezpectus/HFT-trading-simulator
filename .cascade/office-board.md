...
2. **S183** (Medium) — UI дропает order-lifecycle: `order`-ack + `order_cancelled`/`orders_cancelled` → `default:`, нет pending-orders view и cancel-отправителя. Fix: `case 'order'`/`case 'order_cancelled'`/`case 'orders_cancelled'` → `openOrders` map в стор, pending-orders панель/секция, кнопка cancel шлёт `cancel_order`; «Order submitted» должен отражать ack, не send-success.
3. **S185+S186** (Medium) — мёртвый тест-слоёк: удалить mock_objects.py+test_trading_flow.py (моки тестируют моки) и hft mocks/mock_exchange.h; припаять 2 сиротских doctest-файла через `add_doctest_test` — они тестируют живой код и должны бежать.
