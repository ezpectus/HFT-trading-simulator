...
| **S183** | UI игнорит весь order-lifecycle протокола: `order`/`order_cancelled`/`orders_cancelled` падают в `default:` | `useExchangeData.js:38-167` — switch не имеет case для `order`-ack'а (sim шлёт `{type:'order', status:'pending'|'filled'}` каждому отправителю) и двух cancel-типов из R77-fix. `OrderForm.jsx:72-74` показывает «Order submitted» по send-success + фейковый 500ms спиннер — resting LIMIT уходит в никуда: **ни одной панели pending/open-orders в UI нет** (grep pending → только auth-state и backtest-флаги), cancel UI отсутствует хотя протокол его поддерживает. Юзер ставит лимитник → «submitted» → невидим до филла; увидеть/отменить из UI нельзя. Зеркало S179 на C++-стороне: обе стороны считают send == fill. | Medium | [ ] Open |
| **S184** | `HawkesProcess` хранит pending-timeout в `window.__hawkesTimeout` — глобал | `components/HawkesProcess.jsx:293-294` пишет setTimeout-id в `window.__hawkesTimeout`, cleanup читает его же (:148). Два mount'а панели (detach+main, или повторное добавление) шарят один глобал: второй `clearTimeout(window.__hawkesTimeout)` убивает чужой pending-таймер → его `setPending(false)` никогда не придёт → вечный «waiting». Состояние на `window` вместо `useRef`. | Low | [ ] Open |
| **S185** | Mock-testing-mock: `mock_objects.py` + `test_trading_flow.py` — замкнутый круг без продакшена | `ai-signal-bot/tests/mocks/mock_objects.py` (185 строк: MockExchange/MockWebSocket/MockSHM/MockDatabase) имеет **единственного** потребителя — `tests/integration/test_trading_flow.py` (~130 строк, «Integration tests — end-to-end flow tests using mocks»), который импортирует **ноль** продакшен-модулей: `place_order` возвращает хардкод `"filled"` → тест assert'ит `"filled"`; MockSHM try_push/try_pop тестируют свою же dict-обёртку. ~315 строк скаффолдинга, тестирующего сам себя, под именем «integration». Рядом настоящие e2e (`test_e2e_pipeline.py` гоняет SimulatedExchange+CircuitBreaker) — контраст делает theatre видимым. | Medium | [ ] Open |
| **S186** | hft: мёртвый mock + 2 сиротских doctest-файла — тесты живого кода никогда не компилируются | `tests/mocks/mock_exchange.h` (164 строки, `hft::test::MockExchange`) — **0 includers** в тестах/CMake. `tests/test_doctest_cpp_optimizations.cpp` (125 строк: LatencyHistogram/SPSC/spinlock) + `tests/test_doctest_hft_config.cpp` (127 строк: config defaults/YAML) — **нет add_doctest_test-таргета** в CMakeLists и нет в CI → файлы никогда не собираются и не запускаются: регрессии в `low_latency.h` и `config.h` проходят молча, а дерево выглядит покрытым. | Medium | [ ] Open |
...
| **S189** | `wget -qO-` вместо сохранения ключа — lint-cpp и clang-17 leg test-cpp мертвы | `ci.yml:42-43` и `:140-141` — `wget -qO- https://apt.llvm.org/llvm-snapshot.gpg.key` стримит ключ в **stdout** (`-O-`), файл `llvm-snapshot.gpg.key` никогда не создаётся → `gpg --dearmor < llvm-snapshot.gpg.key` падает на несуществующем файле (bash -e) → apt-репозиторий llvm не ставится → `clang-format-18`/`clang-17` недоступны → обе джобы красные при любом запуске. Правильная форма: `wget -qO llvm-snapshot.gpg.key` или `wget -qO- URL \| gpg --dearmor \| sudo tee`. | Medium | [ ] Open |
| **S190** | codeql.yml: C++ билд под `|| true` → пустая база анализа при зелёной джобе + дубль со ска-нjом в ci.yml | `codeql.yml:65` `cmake --build build ... || true` — сломанный C++ билд всё равно доходит до analyze: CodeQL-C++ требует build-tracing, пустая/частичная БД → ноль C++-находок при зелёном чеке. Плюс python/javascript сканируются дважды на пуш: `codeql.yml` matrix и `ci.yml:402 security-codeql` — дублирующий pipeline, двойное время. | Low | [ ] Open |
---
...
3. **S170+S171** (Medium) — dead-code sweep ai-signal-bot: удалить `cross_exchange_arb.py`/`marketplace.py`/`helpers.py` + их тесты (~740 строк); wired strategies-CircuitBreaker в `EnsembleVoter` (run.py:118) или удалить класс + `EnsembleVoter.analyze`/`.strategies` — adapter в backtest_requests уже содержит рабочий вариант.
4. **S162** (Medium) — протокол-док :8765 врёт в 8 местах. (Doc поправлен в R74 + R77 дописал новые типы; строка остаётся контрольной точкой для slop-verify.)
5. **S167** (Medium) — 5 math-тестов тестируют inline-копии, не продакшен. Fix: вынести чистую математику в `src/utils/math/` и импортировать из обеих сторон, или удалить shadow-файлы; плюс S166 — заменить 19 `expect(true)` реальными рендер-assert'ами или удалить placeholder-тесты.
6. **S183** (Medium) — UI дропает order-lifecycle: `order`-ack + `order_cancelled`/`orders_cancelled` → `default:`, нет pending-orders view и cancel-отправителя. Fix: `case 'order'`/`case 'order_cancelled'`/`case 'orders_cancelled'` → `openOrders` map в стор, pending-orders панель/секция, кнопка cancel шлёт `cancel_order`; «Order submitted» должен отражать ack, не send-success.
7. **S185+S186** (Medium) — мёртвый тест-слоёк: удалить mock_objects.py+test_trading_flow.py (моки тестируют моки) и hft mocks/mock_exchange.h; припаять 2 сиротских doctest-файла через `add_doctest_test` — они тестируют живой код и должны бежать.
11. Периодически — `/slop-verify`: QA-проверка записей done-log по файлам/строкам.
