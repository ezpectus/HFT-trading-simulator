...
| Закрыто | 89 (+S124) |
| Открыто | 2 (S125 dead-module cluster ai-bot ~1900 строк; S126 web-ui dead residue) |
...
| **S125** | ai-bot dead-module cluster — ~1900 строк test-only | Zero-importer sweep (модули не импортируемые prod-кодом, живут только через свои тесты): `communication/shm_*` остров — ring_buffer(327)+signal_producer(99)+market_data_writer(124)+fill_consumer(91) — задуманный SHM-канал hft↔bot, но run.py ничего не инстанцирует; `monitoring/alerting.py` (271, AlertSystem); `risk/{cvar(179), position_sizing(198), stress_test(202)}` — читает только `risk/__init__.py` re-export; `strategies/funding_arb_detector.py` (269, FundingRateArbitrageDetector); `technical_analysis/{hawkes_funcs(113), hawkes_model(91)}`. + их тест-файлы. Тот же класс что S117 — но это 11 модулей. Wire-vs-delete по доменам: shm = hft-IPC фича, risk = API, alerting = notifications, hawkes = TA. | **High** | [ ] Open |
| **S126** | web-ui dead residue — ~6 файлов + тесты | Zero-importer sweep web-ui/src: `ExchangeSelector.jsx` — 0 импортеров, НЕ в registry (не монтируется); `useInterval.js`+`useInterval.ts` — ОБА твина мёртвы; `usePerformance.js`; `utils/auditExport.js`; `utils/cn.js` (3-строчный classnames-helper, никем не используется). Каждый живёт только через свой test-файл. Delete-кандидаты вместе с тестами. | Medium | [ ] Open |
