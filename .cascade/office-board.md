...
| Всего находок | ~124 (S001–S123 + переименованные) |
| Закрыто | 86 (+S116–S117) |
| Открыто | 2 (S122 stale smoke-test — fix-or-delete; S123 one-shot codemods) |

**Главный вывод:** R40 закрыл последние находки (S116 auth e2e, S117 portfolio WS API — обе wire). R41 — slop-verify 7 claims, все VERIFIED. R42 — аудит infra/CI/monitoring: CI↔compose↔prometheus↔alerts полностью согласованы, но найдены `docker-smoke-test.{sh,bat}` (мёртвый + стейл-порты, всегда фейлит здоровый стек) и два одноразовых codemod-скрипта.
...
| **S122** | `docker-smoke-test.{sh,bat}` — мёртвый И стейл | 0 refs: ci.yml имеет свой inline docker-smoke с ПРАВИЛЬНЫМИ портами (8775/9090/9091/3000 — совпадает с compose healthchecks), а скрипт curl-ит `http://localhost:8765/health` + `:8766/health` — чистые WS-порты, HTTP /health там нет (health живёт на port+10=8775 и :9090). На здоровом стеке всегда "failed health check" → отправляет дебажить не то. `.bat`-твин те же порты. Fix-or-delete: либо порты 8775/9090, либо удалить пару файлов. | Medium | [ ] Open |
| **S123** | Одноразовые codemod-скрипты забыты | `scripts/fix_eslint_unused.py` + `scripts/fix_fstring_logs.py` — одноразовые миграции из S101 (eslint codemod 257 сайтов), единственные "refs" — упоминания в AUDIT_FINDINGS. Отработали и остались лежать. Тот же класс что S104 dead-хуки. Delete. | Low | [ ] Open |
