...
|----|---------|--------|-----------|--------|
| **S196** | `config.prod.yaml` — `risk.kill_switch.trigger_file` shadow'ит env-aware `ipc.kill_switch.trigger_file`; `ipc.kill_switch.shm_name` мёртв | В prod yaml два блока kill_switch: `ipc.kill_switch` (:53-58: `shm_name` + `trigger_file: "${HFT_KILL_SWITCH_FILE:-/tmp/kill_switch}"` + `poll_interval_ms`) и `risk.kill_switch` (:123-124: literal `"/tmp/kill_switch"`). Оба парсера пишут `cfg.kill_switch_trigger_file`; `parse_prod_risk` идёт ПОСЛЕ `parse_prod_ipc` (config.cpp:30 vs :46) и перезаписывает безусловно → env-переменная `HFT_KILL_SWITCH_FILE` (добавленная специально под k8s readOnlyRootFilesystem, см. коммент :56) никогда не доходит — file-trigger всегда `/tmp/kill_switch`, на k8s kill-switch по файлу недостижим. Также `ipc.kill_switch.shm_name` (:54) не парсится — имя захардкожено в `bot_setup.cpp:180`. | Medium | [ ] Open |
