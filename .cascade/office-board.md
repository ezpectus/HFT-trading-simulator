...
|----|---------|--------|-----------|--------|
| **S342** | Closed-trade accounting absent end-to-end | `src/database/db.py:148` — `close_trade()` is the only writer of `status='CLOSED'`+`pnl`; **0 prod callers**. `src/monitoring/tracker.py:48` — `record_trade()` **0 prod callers**. Fills persist as `status:"FILLED"` (run.py:374) / `"OPEN"` (run.py:661) forever → `get_stats()` (db.py:176-178) + `get_daily_pnl()` (db.py:195) count CLOSED rows that never exist → dashboard `trades_closed`/`win_rate`/`total_pnl` (tracker.py:152-153) + Prometheus `bot_win_rate`/`bot_pnl_total`/`daily` (run.py:763,769-770) structurally zero while fills flow. `RealAccount.set_fill_callback`/`start_user_data_stream` (real_account.py:361-395) — the natural realized-fill feed — also dead. | Medium | [ ] Open |
| **S343** | Zombie strategy feeds: sentiment (default-on) + market_making | `src/strategies/sentiment.py:96` — `on_news_event()` **0 prod callers**; sim broadcasts `news_event` in every market msg (`ws_broadcast.py:154,383,421`) and it moves prices (`market_simulator.py:134-186`), but bot prod code has zero `news` references outside sentiment.py → `analyze()` always exits at :155-160 NEUTRAL. **`sentiment.enabled: true` by default** (settings.yaml:126, comment "React to news events from simulator"). `market_making.py:69,73,160` — `update_inventory`/`update_toxicity`/`on_fill` **0 prod callers** → inventory pinned at 0 → :238-249 only reachable branch is NEUTRAL "Quoting" (quotes never become orders either). Both built via `bot_helpers.build_strategies` (bot_helpers.py:53-61). | Medium | [ ] Open |
| **S344** | CB Prometheus reporting half-dead | `src/monitoring/metrics.py:30` — `circuit_breaker_trips_total` declared; `record_circuit_breaker_trip()` (metrics.py:257) has **0 callers anywhere** → counter pinned at 0 forever. `signal_publisher.py:386-391` — `set_circuit_breaker_state()` called only inside `if not self._clients: continue` → `ai_signal_bot_circuit_breaker_state` gauge goes stale when breaker trips with no UI connected — exactly when the alert matters. | Low | [ ] Open |
| **S345** | Live-adapter market-data read path dead end-to-end | `src/data_collection/exchange_factory.py:332` — `initialize()` starts `RealMarketDataFeed` → real Binance/OKX/Bybit WS connections + queue/backpressure normalizing into `_tickers`/`_candles`/`_orderbooks` caches; read methods `get_ticker`/`get_candles`/`get_positions`/`get_balance`/`get_orderbook`/`get_health` (:342-385) — **0 prod callers**; only `place_order` used (run.py:701). Latent proof the path was never exercised: `market_data_feed.py:160` subscribes `@aggTrade` for `last` (comment :181 "updated by aggTrade") but `_handle_binance_msg` :174-204 has **no aggTrade branch** → `NormalizedTicker.last` = 0.0 forever. | Medium | [ ] Open |
| **S346** | Walk-forward "validation" is in-sample | `run_backtest.py:129-169` — `grid_search` selects best params over the **full** `candles` set (all validation windows included), then `walk_forward` "validates" them on subsets of the same data → printed "Walk-forward validation" is in-sample dressed as OOS. Also `optimizer.walk_forward` docstring (`optimizer.py:193`) claims "train on window, test on next window" but `train_size` is only a skip offset — no per-window parameter fitting happens. | Medium | [ ] Open |
...
2. **R207 slop-audit** — 5 новых: S342→S345 — одна семья "мёртвые producer-фиды" (closed-trade accounting, zombie-стратегии, live-adapter read-path, CB-метрики), S346 — contaminated walk-forward. Порядок фикса: S342 (reporting zeros) → S343 (default-on zombie) → S345 (dead feed + latent data bug) → S346 → S344.
